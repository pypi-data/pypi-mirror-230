...
def wait_for_eventbus(conf: Dict[str, Any]) -> None:
    ...
    while True:
        try:
            status = requests.get(endpoint, headers=headers).status_code
            if status == 200:
                break
            logging.debug('EventBus not ready yet, got %d status code.', status)
        except Exception:
            logging.debug('EventBus not ready, could not reach yet.')
        if time.monotonic() - start > HEALTHCHECK_DELAY:
            logging.error(
                'EventBus not ready for %d seconds, aborting.', HEALTHCHECK_DELAY
            )
            sys.exit(1)
        time.sleep(EVENTBUS_WARMUP_DELAY)
...
def main():
    ...
    logging.info('Waiting for EventBus...')
