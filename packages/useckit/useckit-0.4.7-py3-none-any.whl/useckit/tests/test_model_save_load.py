...
class TestUseckit(unittest.TestCase):
    ...
    def test_distance_scikit_prediction_model(self):
        ...
        save_path = os.path.join('delete_me', 't_d_s_p_m.persisted')
        kwargs = {'output_dir': ''}
    ...
    def test_anomaly_scikit_prediction_model(self):
        ...
        data = make_dataset(shape=(100, 100))
        os.makedirs('delete_me', exist_ok=True)
        save_path = os.path.join('delete_me', 't_a_s_p_m.persisted')
        kwargs = {'output_dir': ''}
        args = {}
        self.fit_save_load_eval(model_class, data, save_path, kwargs, args, dict())
    ...
    def _test_scikit(self, model_class, name):
        data = make_dataset(shape=(100, 100))
        os.makedirs('delete_me', exist_ok=True)
        save_path = os.path.join('delete_me', name)
        kwargs = {'output_dir': ''}
    ...
    def test_encoders(self):
        ...
        os.makedirs('delete_me', exist_ok=True)
        save_path = os.path.join('delete_me', 't_enc.persisted')
