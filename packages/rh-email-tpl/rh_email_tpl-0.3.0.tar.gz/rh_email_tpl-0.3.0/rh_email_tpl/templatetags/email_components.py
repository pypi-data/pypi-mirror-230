...
class EmailBlockTag(dj_template.Node):
    ...
    def render(self, context):
        ...
        with context.push(values):
            output = template.render(context.flatten())
...
class EmailTextHtmlBlockTag(EmailBlockTag):
    ...
    def __init__(
        self, nodelist, extra_context=None, template_html=None, template_text=None
    ):
        super(EmailTextHtmlBlockTag, self).__init__(
            nodelist=nodelist, extra_context=extra_context, template_html=template_html
        )
...
def do_email_content(parser, token):
    ...
    {% emailcontent %}Some other content, may be text or additional elements{% endemailcontent %}
    """
    return generate_block_tag(
        parser,
        token,
...
def do_email_paragraph(parser, token):
    ...
    {% emailparagraph %}Some other content, may be text or additional elements{% endemailparagraph %}
    """
    return generate_block_tag(
        parser,
        token,
...
def do_email_small(parser, token):
    ...
    {% emailsmall %}Some small content, may be text or additional elements{% endemailsmall %}
    """
    return generate_block_tag(
        parser,
        token,
...
def do_email_medium(parser, token):
    ...
    {% emailmedium %}Some medium sized content, may be text or additional elements{% endemailmedium %}
    """
    return generate_block_tag(
        parser,
        token,
...
def do_email_center(parser, token):
    ...
    {% emailcenter %}Some centered content, may be text or additional elements{% endemailcenter %}
    """
    return generate_block_tag(
        parser,
        token,
...
def do_email_eyecatcher(parser, token):
    ...
    {% emaileyecatcher image="img/email/som/eyecatcher_impressions.png" subline="Awesome subline text"%}
...
def do_email_list(parser, token):
    ...
    Accepts arguments:
        ordered (optional, default: False) - define if it should be an ordered or unordered list
...
def do_email_table(parser, token):
    """
    table base element that should contain extaclty one of emailtable_thead and/or emailtable_tbody element
    """
    return generate_block_tag(
        parser,
        token,
...
def do_email_link(parser, token):
    ...
    Accepts arguments:
        ...
        protocol (optional) - mailto|tel etc., used to hide these protocols in the text variant
        remove_underline
    """
    return generate_block_tag(
        parser,
...
def do_email_del(parser, token):
    ...
    {% emaildel %}Some small content, may be text or additional elements{% endemaildel %}
    """
    return generate_block_tag(
        parser,
        token,
...
def do_email_salutation(parser, token):
    ...
    Accepts arguments:
        service_employee (optional) - user instance of the service employee, will fall back to generic salutation
    """
    return generate_block_tag(
        parser,
        token,
