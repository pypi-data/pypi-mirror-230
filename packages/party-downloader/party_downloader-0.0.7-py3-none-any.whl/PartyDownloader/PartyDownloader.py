...
class PartyDownloader:
    ...
    def download_coomer_files(self, model, *, skip_scraping=False):
        full_path, model = get_model_url(model)
...
def json_to_url(model_json: dict) -> tuple[str, str]:
    if model_json['service'] == 'onlyfans':
        return f"https://coomer.party/onlyfans/user/{model_json['name']}", model_json['name']
    elif model_json['service'] == 'fansly':
        return f"https://coomer.party/fansly/user/{model_json['id']}", model_json['name']


def get_model_url(model: str) -> tuple[str | None, str | None]:
