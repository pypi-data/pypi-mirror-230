...
def _is_mlflow_requirement(requirement_string):
    """
    Returns True if `requirement_string` represents a requirement for mlflow (e.g. 'mlflow_by_johnsnowlabs==1.2.3').
    """
    try:
...
def _generate_mlflow_version_pinning():
    ...
    return f"mlflow_by_johnsnowlabs=={current_major_version}.{current_minor_version}"


def _contains_mlflow_requirement(requirements):
    """
    Returns True if `requirements` contains a requirement for mlflow (e.g. 'mlflow_by_johnsnowlabs==1.2.3').
    """
...
def _get_pip_install_mlflow():
    ...
    "pip install mlflow_by_johnsnowlabs=={mlflow.__version__} 1>&2".
    """
    mlflow_home = os.getenv("MLFLOW_HOME")
    if mlflow_home:  # dev version
        return f"pip install -e {mlflow_home} 1>&2"
    else:
        return f"pip install mlflow_by_johnsnowlabs=={VERSION} 1>&2"
