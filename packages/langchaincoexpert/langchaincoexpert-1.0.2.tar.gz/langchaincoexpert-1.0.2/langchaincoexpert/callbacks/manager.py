...
from langchaincoexpert.callbacks.base import (
    BaseCallbackHandler,
    BaseCallbackManager,
    Callbacks,
    ChainManagerMixin,
...
from langchaincoexpert.callbacks.tracers import run_collector
from langchaincoexpert.callbacks.tracers.langchain import langchaincoexpertTracer
from langchaincoexpert.callbacks.tracers.langchain_v1 import langchaincoexpertTracerV1, TracerSessionV1
from langchaincoexpert.callbacks.tracers.stdout import ConsoleCallbackHandler
from langchaincoexpert.callbacks.tracers.wandb import WandbTracer
from langchaincoexpert.schema import (
    AgentAction,
    AgentFinish,
    Document,
    LLMResult,
)
from langchaincoexpert.schema.messages import BaseMessage, get_buffer_string
from langchaincoexpert.schema.output import ChatGenerationChunk, GenerationChunk
...
tracing_callback_var: ContextVar[
    Optional[langchaincoexpertTracerV1]
...
tracing_v2_callback_var: ContextVar[
    Optional[langchaincoexpertTracer]
] = ContextVar(  # noqa: E501
...
def _get_debug() -> bool:
    return langchaincoexpert.debug
...
def tracing_enabled(
    ...
    """Get the Deprecated langchaincoexpertTracer in a context manager.
    ...
    Returns:
        TracerSessionV1: The langchaincoexpertTracer session.

    Example:
        >>> with tracing_enabled() as session:
        ...     # Use the langchaincoexpertTracer session
    """
    cb = langchaincoexpertTracerV1()
...
def tracing_v2_enabled(
    ...
    *,
    ...
    """Instruct langchaincoexpert to log all runs in context to LangSmith.
    ...
    Example:
        ...
        ...     # langchaincoexpert code will automatically be traced
    """
    if isinstance(example_id, str):
        example_id = UUID(example_id)
    cb = langchaincoexpertTracer(
...
class CallbackManager(BaseCallbackManager):
    """Callback manager that handles callbacks from langchaincoexpert."""
...
class AsyncCallbackManager(BaseCallbackManager):
    """Async callback manager that handles callbacks from langchaincoexpert."""
...
def _configure(
    ...
    tracing_enabled_ = (
        env_var_is_set("langchaincoexpert_TRACING")
        or tracer is not None
        or env_var_is_set("langchaincoexpert_HANDLER")
    )
    wandb_tracing_enabled_ = (
        env_var_is_set("langchaincoexpert_WANDB_TRACING") or wandb_tracer is not None
    )

    tracer_v2 = tracing_v2_callback_var.get()
    tracing_v2_enabled_ = (
        env_var_is_set("langchaincoexpert_TRACING_V2") or tracer_v2 is not None
    )
    tracer_project = os.environ.get(
        "langchaincoexpert_PROJECT", os.environ.get("langchaincoexpert_SESSION", "default")
    )
    ...
    if (
        ...
        if tracing_enabled_ and not any(
            isinstance(handler, langchaincoexpertTracerV1)
            for handler in callback_manager.handlers
        ):
            if tracer:
                callback_manager.add_handler(tracer, True)
            else:
                handler = langchaincoexpertTracerV1()
        ...
        if tracing_v2_enabled_ and not any(
            isinstance(handler, langchaincoexpertTracer)
            for handler in callback_manager.handlers
        ):
            if tracer_v2:
                ...
                try:
                    handler = langchaincoexpertTracer(project_name=tracer_project)
                    callback_manager.add_handler(handler, True)
                except Exception as e:
                    logger.warning(
                        "Unable to load requested langchaincoexpertTracer."
                        " To disable this warning,"
                        " unset the  langchaincoexpert_TRACING_V2 environment variables.",
                        e,
                    )
