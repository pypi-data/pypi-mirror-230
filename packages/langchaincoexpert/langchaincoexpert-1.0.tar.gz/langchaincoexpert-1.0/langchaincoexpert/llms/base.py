...
from langchaincoexpert.load.dump import dumpd
from langchaincoexpert.prompts.base import StringPromptValue
from langchaincoexpert.prompts.chat import ChatPromptValue
from langchaincoexpert.pydantic_v1 import Field, root_validator, validator
...
from langchaincoexpert.schema.language_model import BaseLanguageModel, LanguageModelInput
from langchaincoexpert.schema.messages import AIMessage, BaseMessage, get_buffer_string
from langchaincoexpert.schema.output import GenerationChunk
from langchaincoexpert.schema.runnable import RunnableConfig
from langchaincoexpert.schema.runnable.config import get_config_list

logger = logging.getLogger(__name__)


def _get_verbosity() -> bool:
    return langchaincoexpert.verbose
...
def get_prompts(
    ...
    for i, prompt in enumerate(prompts):
        if langchaincoexpert.llm_cache is not None:
            cache_val = langchaincoexpert.llm_cache.lookup(prompt, llm_string)
...
def update_cache(
    ...
    for i, result in enumerate(new_results.generations):
        ...
        if langchaincoexpert.llm_cache is not None:
            langchaincoexpert.llm_cache.update(prompt, llm_string, result)
...
class BaseLLM(BaseLanguageModel[str], ABC):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if langchaincoexpert.llm_cache is None or disregard_cache:
            if self.cache is not None and self.cache:
                raise ValueError(
                    "Asked to cache, but no cache found at `langchaincoexpert.cache`."
                )
            run_managers = [
