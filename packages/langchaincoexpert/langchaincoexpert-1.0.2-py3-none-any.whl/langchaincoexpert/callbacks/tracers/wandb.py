...
from langchaincoexpert.callbacks.tracers.base import BaseTracer
from langchaincoexpert.callbacks.tracers.schemas import Run
...
class RunProcessor:
    """Handles the conversion of a langchaincoexpert Runs into a WBTraceTree."""
    ...
    def process_span(self, run: Run) -> Optional["Span"]:
        """Converts a langchaincoexpert Run into a W&B Trace Span.
        :param run: The langchaincoexpert Run to convert.
        ...
        try:
            ...
            if PRINT_WARNINGS:
                self.wandb.termwarn(
                    f"Skipping trace saving - unable to safely convert langchaincoexpert Run "
    ...
    def _convert_llm_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a langchaincoexpert LLM Run into a W&B Trace Span.
        :param run: The langchaincoexpert LLM Run to convert.
        :return: The converted W&B Trace Span.
        """
    ...
    def _convert_chain_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a langchaincoexpert Chain Run into a W&B Trace Span.
        :param run: The langchaincoexpert Chain Run to convert.
        :return: The converted W&B Trace Span.
        """
        base_span = self._convert_run_to_wb_span(run)
    ...
    def _convert_tool_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a langchaincoexpert Tool Run into a W&B Trace Span.
        :param run: The langchaincoexpert Tool Run to convert.
        :return: The converted W&B Trace Span.
        """
        base_span = self._convert_run_to_wb_span(run)
        base_span.results = [
    ...
    def _convert_lc_run_to_wb_span(self, run: Run) -> "Span":
        """Utility to convert any generic langchaincoexpert Run into a W&B Trace Span.
        :param run: The langchaincoexpert Run to convert.
        :return: The converted W&B Trace Span.
        """
        if run.run_type == "llm":
    ...
    def build_tree(self, runs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        :return: The nested dictionary representing the langchaincoexpert Run in a tree
...
class WandbTracer(BaseTracer):
    ...
    This will ensure that all langchaincoexpert activity is logged to W&B.
    """
    ...
    def __init__(self, run_args: Optional[WandbRunArgs] = None, **kwargs: Any) -> None:
        ...
        To use W&B to monitor all langchaincoexpert activity, add this tracer like any other
        langchaincoexpert callback:
        ```
        from wandb.integration.langchaincoexpert import WandbTracer
    ...
    def _log_trace_from_run(self, run: Run) -> None:
        """Logs a langchaincoexpert Run to W*B as a W&B Trace."""
        self._ensure_run()
        ...
        if self._wandb.run is not None:
            self._wandb.run.log({"langchaincoexpert_trace": model_trace})
    ...
    def _ensure_run(self, should_print_url: bool = False) -> None:
        ...
        if self._wandb.run is not None:
            if should_print_url:
                ...
                self._wandb.termlog(
                    f"Streaming langchaincoexpert activity to W&B at {run_url}\n"
            ...
            self._wandb.run._label(repo="langchaincoexpert")
