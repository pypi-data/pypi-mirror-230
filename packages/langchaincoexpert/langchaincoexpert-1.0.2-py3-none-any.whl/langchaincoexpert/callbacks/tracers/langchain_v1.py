...
from langchaincoexpert.callbacks.tracers.schemas import (
    ChainRun,
    LLMRun,
    Run,
    ToolRun,
...
from langchaincoexpert.schema.messages import get_buffer_string
from langchaincoexpert.utils import raise_for_status_with_text

logger = logging.getLogger(__name__)


def get_headers() -> Dict[str, Any]:
    """Get the headers for the langchaincoexpert API."""
    headers: Dict[str, Any] = {"Content-Type": "application/json"}
    if os.getenv("langchaincoexpert_API_KEY"):
        headers["x-api-key"] = os.getenv("langchaincoexpert_API_KEY")
    return headers


def _get_endpoint() -> str:
    return os.getenv("langchaincoexpert_ENDPOINT", "http://localhost:8000")


class langchaincoexpertTracerV1(BaseTracer):
    ...
    def _convert_to_v1_run(self, run: Run) -> Union[LLMRun, ChainRun, ToolRun]:
        ...
        if not isinstance(session, TracerSessionV1):
            raise ValueError(
                "langchaincoexpertTracerV1 is not compatible with"
