...
from langchaincoexpert.schema import AgentAction, AgentFinish, LLMResult
from langchaincoexpert.utils import get_from_dict_or_env
...
class MlflowLogger:
    ...
    def langchaincoexpert_artifact(self, chain: Any) -> None:
        with self.mlflow.start_run(
            run_id=self.run.info.run_id, experiment_id=self.mlf_expid
        ):
            self.mlflow.langchaincoexpert.log_model(chain, "langchaincoexpert-model")
...
class MlflowCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        self,
        name: Optional[str] = "langchaincoexpertrun-%",
        experiment: Optional[str] = "langchaincoexpert",
    ...
    def _create_session_analysis_df(self) -> Any:
        ...
        if "name" in on_llm_start_records_df.columns:
            ...
            # ["langchaincoexpert", "llms", "openai", "AzureOpenAI"]
    ...
    def flush_tracker(self, langchaincoexpert_asset: Any = None, finish: bool = False) -> None:
        pd = import_pandas()
        ...
        if langchaincoexpert_asset:
            ...
            if "langchaincoexpert.chains.llm.LLMChain" in str(type(langchaincoexpert_asset)):
                self.mlflg.langchaincoexpert_artifact(langchaincoexpert_asset)
            else:
                langchaincoexpert_asset_path = str(Path(self.temp_dir.name, "model.json"))
                try:
                    langchaincoexpert_asset.save(langchaincoexpert_asset_path)
                    self.mlflg.artifact(langchaincoexpert_asset_path)
                except ValueError:
                    try:
