...
import langchaincoexpert
from langchaincoexpert.callbacks.base import BaseCallbackHandler
from langchaincoexpert.callbacks.utils import (
...
from langchaincoexpert.schema import AgentAction, AgentFinish, Generation, LLMResult

langchaincoexpert_MODEL_NAME = "langchaincoexpert-model"
...
class CometCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        ...
        self.experiment.log_other("Created from", "langchaincoexpert")
        if tags:
        ...
        warning = (
            ...
            "based on updates to `langchaincoexpert`. Please report any issues to "
    ...
    def flush_tracker(
        self,
        langchaincoexpert_asset: Any = None,
        task_type: Optional[str] = "inference",
        workspace: Optional[str] = None,
        project_name: Optional[str] = "comet-langchaincoexpert-demo",
        tags: Optional[Sequence] = None,
        name: Optional[str] = None,
        visualizations: Optional[List[str]] = None,
        complexity_metrics: bool = False,
        ...
        Args:
            ...
            langchaincoexpert_asset: The langchaincoexpert asset to save.
            finish: Whether to finish the run.

            Returns:
                None
        """
        self._log_session(langchaincoexpert_asset)

        if langchaincoexpert_asset:
            try:
                self._log_model(langchaincoexpert_asset)
            except Exception:
    ...
    def _log_model(self, langchaincoexpert_asset: Any) -> None:
        model_parameters = self._get_llm_parameters(langchaincoexpert_asset)
        self.experiment.log_parameters(model_parameters, prefix="model")

        langchaincoexpert_asset_path = Path(self.temp_dir.name, "model.json")
        model_name = self.name if self.name else langchaincoexpert_MODEL_NAME

        try:
            if hasattr(langchaincoexpert_asset, "save"):
                langchaincoexpert_asset.save(langchaincoexpert_asset_path)
                self.experiment.log_model(model_name, str(langchaincoexpert_asset_path))
        except (ValueError, AttributeError, NotImplementedError) as e:
            if hasattr(langchaincoexpert_asset, "save_agent"):
                langchaincoexpert_asset.save_agent(langchaincoexpert_asset_path)
                self.experiment.log_model(model_name, str(langchaincoexpert_asset_path))
            else:
                self.comet_ml.LOGGER.error(
                    f"{e}"
                    " Could not save langchaincoexpert Asset "
                    f"for {langchaincoexpert_asset.__class__.__name__}"
                )

    def _log_session(self, langchaincoexpert_asset: Optional[Any] = None) -> None:
        try:
            llm_session_df = self._create_session_analysis_dataframe(langchaincoexpert_asset)
            # Log the cleaned dataframe as a table
            self.experiment.log_table("langchaincoexpert-llm-session.csv", llm_session_df)
        except Exception:
            self.comet_ml.LOGGER.warning(
        ...
        try:
            metadata = {"langchaincoexpert_version": str(langchaincoexpert.__version__)}
            # Log the langchaincoexpert low-level records as a JSON file directly
            self.experiment.log_asset_data(
                self.action_records, "langchaincoexpert-action_records.json", metadata=metadata
            )
        except Exception:
            self.comet_ml.LOGGER.warning(
                "Failed to log session data to Comet",
        ...
        for idx, (prompt, output) in enumerate(zip(prompts, outputs)):
            ...
            for visualization in self.visualizations:
                try:
                    ...
                    self.experiment.log_asset_data(
                        html,
                        name=f"langchaincoexpert-viz-{visualization}-{idx}.html",
    ...
    def _create_session_analysis_dataframe(self, langchaincoexpert_asset: Any = None) -> dict:
        pd = import_pandas()

        llm_parameters = self._get_llm_parameters(langchaincoexpert_asset)
    ...
    def _get_llm_parameters(self, langchaincoexpert_asset: Any = None) -> dict:
        if not langchaincoexpert_asset:
            return {}
        try:
            if hasattr(langchaincoexpert_asset, "agent"):
                llm_parameters = langchaincoexpert_asset.agent.llm_chain.llm.dict()
            elif hasattr(langchaincoexpert_asset, "llm_chain"):
                llm_parameters = langchaincoexpert_asset.llm_chain.llm.dict()
            elif hasattr(langchaincoexpert_asset, "llm"):
                llm_parameters = langchaincoexpert_asset.llm.dict()
            else:
                llm_parameters = langchaincoexpert_asset.dict()
        except Exception:
            return {}
