# coding: utf-8

"""
    Bacalhau API

    This page is the reference of the Bacalhau REST API. Project docs are available at https://docs.bacalhau.org/. Find more information about Bacalhau at https://github.com/bacalhau-project/bacalhau.  # noqa: E501

    OpenAPI spec version: ${PYPI_VERSION}
    Contact: team@bacalhau.org
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""

import pprint
import re  # noqa: F401

import six

class Spec(object):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    """
    """
    Attributes:
      swagger_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    swagger_types = {
        'annotations': 'list[str]',
        'deal': 'AllOfSpecDeal',
        'do_not_track': 'bool',
        'docker': 'AllOfSpecDocker',
        'engine': 'AllOfSpecEngine',
        'engine_spec': 'EngineSpec',
        'inputs': 'list[StorageSpec]',
        'network': 'AllOfSpecNetwork',
        'node_selectors': 'list[LabelSelectorRequirement]',
        'outputs': 'list[StorageSpec]',
        'publisher': 'AllOfSpecPublisher',
        'publisher_spec': 'PublisherSpec',
        'timeout': 'int',
        'total_allocated_resources': 'AllOfSpecTotalAllocatedResources',
        'wasm': 'AllOfSpecWasm'
    }

    attribute_map = {
        'annotations': 'Annotations',
        'deal': 'Deal',
        'do_not_track': 'DoNotTrack',
        'docker': 'Docker',
        'engine': 'Engine',
        'engine_spec': 'EngineSpec',
        'inputs': 'Inputs',
        'network': 'Network',
        'node_selectors': 'NodeSelectors',
        'outputs': 'Outputs',
        'publisher': 'Publisher',
        'publisher_spec': 'PublisherSpec',
        'timeout': 'Timeout',
        'total_allocated_resources': 'TotalAllocatedResources',
        'wasm': 'Wasm'
    }

    def __init__(self, annotations=None, deal=None, do_not_track=None, docker=None, engine=None, engine_spec=None, inputs=None, network=None, node_selectors=None, outputs=None, publisher=None, publisher_spec=None, timeout=None, total_allocated_resources=None, wasm=None):  # noqa: E501
        """Spec - a model defined in Swagger"""  # noqa: E501
        self._annotations = None
        self._deal = None
        self._do_not_track = None
        self._docker = None
        self._engine = None
        self._engine_spec = None
        self._inputs = None
        self._network = None
        self._node_selectors = None
        self._outputs = None
        self._publisher = None
        self._publisher_spec = None
        self._timeout = None
        self._total_allocated_resources = None
        self._wasm = None
        self.discriminator = None
        if annotations is not None:
            self.annotations = annotations
        if deal is not None:
            self.deal = deal
        if do_not_track is not None:
            self.do_not_track = do_not_track
        if docker is not None:
            self.docker = docker
        if engine is not None:
            self.engine = engine
        if engine_spec is not None:
            self.engine_spec = engine_spec
        if inputs is not None:
            self.inputs = inputs
        if network is not None:
            self.network = network
        if node_selectors is not None:
            self.node_selectors = node_selectors
        if outputs is not None:
            self.outputs = outputs
        if publisher is not None:
            self.publisher = publisher
        if publisher_spec is not None:
            self.publisher_spec = publisher_spec
        if timeout is not None:
            self.timeout = timeout
        if total_allocated_resources is not None:
            self.total_allocated_resources = total_allocated_resources
        if wasm is not None:
            self.wasm = wasm

    @property
    def annotations(self):
        """Gets the annotations of this Spec.  # noqa: E501

        Annotations on the job - could be user or machine assigned  # noqa: E501

        :return: The annotations of this Spec.  # noqa: E501
        :rtype: list[str]
        """
        return self._annotations

    @annotations.setter
    def annotations(self, annotations):
        """Sets the annotations of this Spec.

        Annotations on the job - could be user or machine assigned  # noqa: E501

        :param annotations: The annotations of this Spec.  # noqa: E501
        :type: list[str]
        """

        self._annotations = annotations

    @property
    def deal(self):
        """Gets the deal of this Spec.  # noqa: E501

        The deal the client has made, such as which job bids they have accepted.  # noqa: E501

        :return: The deal of this Spec.  # noqa: E501
        :rtype: AllOfSpecDeal
        """
        return self._deal

    @deal.setter
    def deal(self, deal):
        """Sets the deal of this Spec.

        The deal the client has made, such as which job bids they have accepted.  # noqa: E501

        :param deal: The deal of this Spec.  # noqa: E501
        :type: AllOfSpecDeal
        """

        self._deal = deal

    @property
    def do_not_track(self):
        """Gets the do_not_track of this Spec.  # noqa: E501

        Do not track specified by the client  # noqa: E501

        :return: The do_not_track of this Spec.  # noqa: E501
        :rtype: bool
        """
        return self._do_not_track

    @do_not_track.setter
    def do_not_track(self, do_not_track):
        """Sets the do_not_track of this Spec.

        Do not track specified by the client  # noqa: E501

        :param do_not_track: The do_not_track of this Spec.  # noqa: E501
        :type: bool
        """

        self._do_not_track = do_not_track

    @property
    def docker(self):
        """Gets the docker of this Spec.  # noqa: E501

        Deprecated: use EngineSpec.  # noqa: E501

        :return: The docker of this Spec.  # noqa: E501
        :rtype: AllOfSpecDocker
        """
        return self._docker

    @docker.setter
    def docker(self, docker):
        """Sets the docker of this Spec.

        Deprecated: use EngineSpec.  # noqa: E501

        :param docker: The docker of this Spec.  # noqa: E501
        :type: AllOfSpecDocker
        """

        self._docker = docker

    @property
    def engine(self):
        """Gets the engine of this Spec.  # noqa: E501

        Deprecated: use EngineSpec.  # noqa: E501

        :return: The engine of this Spec.  # noqa: E501
        :rtype: AllOfSpecEngine
        """
        return self._engine

    @engine.setter
    def engine(self, engine):
        """Sets the engine of this Spec.

        Deprecated: use EngineSpec.  # noqa: E501

        :param engine: The engine of this Spec.  # noqa: E501
        :type: AllOfSpecEngine
        """

        self._engine = engine

    @property
    def engine_spec(self):
        """Gets the engine_spec of this Spec.  # noqa: E501


        :return: The engine_spec of this Spec.  # noqa: E501
        :rtype: EngineSpec
        """
        return self._engine_spec

    @engine_spec.setter
    def engine_spec(self, engine_spec):
        """Sets the engine_spec of this Spec.


        :param engine_spec: The engine_spec of this Spec.  # noqa: E501
        :type: EngineSpec
        """

        self._engine_spec = engine_spec

    @property
    def inputs(self):
        """Gets the inputs of this Spec.  # noqa: E501

        the data volumes we will read in the job for example \"read this ipfs cid\"  # noqa: E501

        :return: The inputs of this Spec.  # noqa: E501
        :rtype: list[StorageSpec]
        """
        return self._inputs

    @inputs.setter
    def inputs(self, inputs):
        """Sets the inputs of this Spec.

        the data volumes we will read in the job for example \"read this ipfs cid\"  # noqa: E501

        :param inputs: The inputs of this Spec.  # noqa: E501
        :type: list[StorageSpec]
        """

        self._inputs = inputs

    @property
    def network(self):
        """Gets the network of this Spec.  # noqa: E501

        The type of networking access that the job needs  # noqa: E501

        :return: The network of this Spec.  # noqa: E501
        :rtype: AllOfSpecNetwork
        """
        return self._network

    @network.setter
    def network(self, network):
        """Sets the network of this Spec.

        The type of networking access that the job needs  # noqa: E501

        :param network: The network of this Spec.  # noqa: E501
        :type: AllOfSpecNetwork
        """

        self._network = network

    @property
    def node_selectors(self):
        """Gets the node_selectors of this Spec.  # noqa: E501

        NodeSelectors is a selector which must be true for the compute node to run this job.  # noqa: E501

        :return: The node_selectors of this Spec.  # noqa: E501
        :rtype: list[LabelSelectorRequirement]
        """
        return self._node_selectors

    @node_selectors.setter
    def node_selectors(self, node_selectors):
        """Sets the node_selectors of this Spec.

        NodeSelectors is a selector which must be true for the compute node to run this job.  # noqa: E501

        :param node_selectors: The node_selectors of this Spec.  # noqa: E501
        :type: list[LabelSelectorRequirement]
        """

        self._node_selectors = node_selectors

    @property
    def outputs(self):
        """Gets the outputs of this Spec.  # noqa: E501

        the data volumes we will write in the job for example \"write the results to ipfs\"  # noqa: E501

        :return: The outputs of this Spec.  # noqa: E501
        :rtype: list[StorageSpec]
        """
        return self._outputs

    @outputs.setter
    def outputs(self, outputs):
        """Sets the outputs of this Spec.

        the data volumes we will write in the job for example \"write the results to ipfs\"  # noqa: E501

        :param outputs: The outputs of this Spec.  # noqa: E501
        :type: list[StorageSpec]
        """

        self._outputs = outputs

    @property
    def publisher(self):
        """Gets the publisher of this Spec.  # noqa: E501

        Deprecated: use PublisherSpec instead  # noqa: E501

        :return: The publisher of this Spec.  # noqa: E501
        :rtype: AllOfSpecPublisher
        """
        return self._publisher

    @publisher.setter
    def publisher(self, publisher):
        """Sets the publisher of this Spec.

        Deprecated: use PublisherSpec instead  # noqa: E501

        :param publisher: The publisher of this Spec.  # noqa: E501
        :type: AllOfSpecPublisher
        """

        self._publisher = publisher

    @property
    def publisher_spec(self):
        """Gets the publisher_spec of this Spec.  # noqa: E501


        :return: The publisher_spec of this Spec.  # noqa: E501
        :rtype: PublisherSpec
        """
        return self._publisher_spec

    @publisher_spec.setter
    def publisher_spec(self, publisher_spec):
        """Sets the publisher_spec of this Spec.


        :param publisher_spec: The publisher_spec of this Spec.  # noqa: E501
        :type: PublisherSpec
        """

        self._publisher_spec = publisher_spec

    @property
    def timeout(self):
        """Gets the timeout of this Spec.  # noqa: E501

        How long a job can run in seconds before it is killed. This includes the time required to run, verify and publish results  # noqa: E501

        :return: The timeout of this Spec.  # noqa: E501
        :rtype: int
        """
        return self._timeout

    @timeout.setter
    def timeout(self, timeout):
        """Sets the timeout of this Spec.

        How long a job can run in seconds before it is killed. This includes the time required to run, verify and publish results  # noqa: E501

        :param timeout: The timeout of this Spec.  # noqa: E501
        :type: int
        """

        self._timeout = timeout

    @property
    def total_allocated_resources(self):
        """Gets the total_allocated_resources of this Spec.  # noqa: E501

        the compute (cpu, ram) resources this job requires  # noqa: E501

        :return: The total_allocated_resources of this Spec.  # noqa: E501
        :rtype: AllOfSpecTotalAllocatedResources
        """
        return self._total_allocated_resources

    @total_allocated_resources.setter
    def total_allocated_resources(self, total_allocated_resources):
        """Sets the total_allocated_resources of this Spec.

        the compute (cpu, ram) resources this job requires  # noqa: E501

        :param total_allocated_resources: The total_allocated_resources of this Spec.  # noqa: E501
        :type: AllOfSpecTotalAllocatedResources
        """

        self._total_allocated_resources = total_allocated_resources

    @property
    def wasm(self):
        """Gets the wasm of this Spec.  # noqa: E501

        Deprecated: use EngineSpec.  # noqa: E501

        :return: The wasm of this Spec.  # noqa: E501
        :rtype: AllOfSpecWasm
        """
        return self._wasm

    @wasm.setter
    def wasm(self, wasm):
        """Sets the wasm of this Spec.

        Deprecated: use EngineSpec.  # noqa: E501

        :param wasm: The wasm of this Spec.  # noqa: E501
        :type: AllOfSpecWasm
        """

        self._wasm = wasm

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = {}

        for attr, _ in six.iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value
        if issubclass(Spec, dict):
            for key, value in self.items():
                result[key] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        return pprint.pformat(self.to_dict())

    def __repr__(self):
        """For `print` and `pprint`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, Spec):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
