...
def orthogonal_matrix_chunk_hadamard(d, device=None):
    ...
    h1, h2, h3 = [torch.tensor(hadamard(d), device=device).float() for _ in range(3)]
    
    # Generate 3 diagonal matrices with random signs
    d1, d2, d3 = [torch.diag(torch.sign(torch.rand(d, device=device) - 0.5)).float() for _ in range(3)]
...
def compute_simplex_dir(d, device=None):
    ...
    simp_dir = torch.eye(d, device=device).float() / math.sqrt(2) - torch.ones(d, d, device=device).float() / ((d - 1) * math.sqrt(2)) * (1 + 1 / math.sqrt(d))
    
    simp_dir[d-1, :] = 1 / math.sqrt(2 * d) * torch.ones(d, device=device).float()
    simp_dir[:, d-1] = 0
    ...
    rand_sim = torch.matmul(torch.diag(torch.distributions.chi2.Chi2(torch.ones(d) * d).sample().float().to(device).sqrt()), simp_dir)
