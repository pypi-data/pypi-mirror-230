...
def writeprojecttofile(file: TextIOWrapper, projectname: str, projectpath: str):
    file.write(f"- name: {projectname}\n")
    file.write(f"  path: {projectpath}\n")
...
def colformat(strings, num_sep_spaces=1, terminal_width=None):
    ...
    num_columns = min(
        int((terminal_width + num_sep_spaces) / (max_len + num_sep_spaces)), num_items
    )
