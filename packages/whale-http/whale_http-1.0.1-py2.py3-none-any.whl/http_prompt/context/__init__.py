...
class Context(object):
    ...
    def __init__(self, url=None, spec=None):
        ...
        if spec:
            if not self.url:
                ...
                self.url = (scheme + '://' + spec.get('host', 'http://localhost:8000') +
                            spec.get('basePath', ''))

            base_path_tokens = list(filter(lambda s: s, spec.get('basePath', '').split('/')))
            paths = spec.get('paths')
            if paths:
                for path in paths:
                    path_tokens = (base_path_tokens + list(filter(lambda s: s, path.split('/'))))
                    ...
                    for method, info in endpoint.items():
                        ...
                        if params:
                            ...
                            def parameter_key(i):
                                return (i.get('$ref', None), i.get('name', None), i.get('in', None))
    ...
    def __eq__(self, other):
        return (self.url == other.url and self.headers == other.headers and self.options == other.options
