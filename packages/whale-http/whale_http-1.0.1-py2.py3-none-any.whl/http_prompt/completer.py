...
from .completion import (ROOT_COMMANDS, ACTIONS, OPTION_NAMES, HEADER_NAMES, HEADER_VALUES)
...
class CompletionGenerator(object):
    ...
    def root_commands(self, context, match):
        return chain(self._generic_generate(ROOT_COMMANDS.keys(), {}, ROOT_COMMANDS),
                     self.actions(context, match), self.concat_mutations(context, match))
    ...
    def preview(self, context, match):
        return chain(self.actions(context, match), self.concat_mutations(context, match))
    ...
    def concat_mutations(self, context, match):
        return chain(
            self._generic_generate(context.body_params.keys(), context.body_params, 'Body parameter'),
            self._generic_generate(context.querystring_params.keys(), context.querystring_params,
                                   'Querystring parameter'),
            self._generic_generate(HEADER_NAMES.keys(), context.headers, HEADER_NAMES),
            self._generic_generate(OPTION_NAMES.keys(), context.options, OPTION_NAMES))
    ...
    def existing_querystring_params(self, context, match):
        return self._generic_generate(context.querystring_params.keys(), context.querystring_params,
                                      'Querystring parameter')

    def existing_header_names(self, context, match):
        return self._generic_generate(context.headers.keys(), context.headers, HEADER_NAMES)

    def existing_option_names(self, context, match):
        return self._generic_generate(context.options.keys(), context.options, OPTION_NAMES)
    ...
    def urlpaths(self, context, match):
        ...
        names = [node.name for node in context.root.ls(*path) if node.data.get('type') == 'dir']
