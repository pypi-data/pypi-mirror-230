...
from .completion import ACTIONS, HEADER_NAMES, OPTION_NAMES, ROOT_COMMANDS
...
from .utils import colformat, unescape, unquote
...
class ExecutionVisitor(NodeVisitor):
    ...
    def visit_cp(self, node, children):
        _, _, _, projectname, _ = children
        self.context.should_reload = True
        self.context.project_name = projectname
        return node
    ...
    def _trace_get_response(self, frame, event, arg):
        ...
        if func_name == "collect_messages":
            if event == "call":
                return self._trace_get_response
            elif event == "return" and arg:
    ...
    def _call_httpie_main(self):
        ...
        env = Environment(stdout=self.output, stdin=sys.stdin, is_windows=False)
    ...
    def generic_visit(self, node, children):
        # click.echo(f'generic_visit: {node.expr_name}')
...
def execute(command, context, listener=None, style=None):
    try:
        ...
        try:
            # click.echo(f'root:  {root.expr_name}')
            visitor.visit(root)
            ...
            if exc_class is KeyError:
                ...
                click.secho("Key '%s' not found" % key, err=True, fg="red")
