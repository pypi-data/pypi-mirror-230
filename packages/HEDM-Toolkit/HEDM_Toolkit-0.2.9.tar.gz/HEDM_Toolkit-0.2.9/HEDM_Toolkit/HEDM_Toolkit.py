...
class HEDM_Toolkit:
    def __init__(self, **kwargs):
        ...
        self.proc_h5file = self.params.get('proc_h5file', None)
    ...
    def load_frame(self, filename):
        with h5py.File(filename, 'r') as f:
            if 'images' in f:
                return f['images'][self.frame_number]
            elif 'imageseries/images' in f:
                return f['imageseries/images'][self.frame_number]
            else:
                raise ValueError("Neither 'images' nor 'imageseries/images' were found in the HDF5 file.")

    def visualize_difference(self):
        proc_frame = self.load_frame(self.proc_h5file)
        raw_frame = self.load_frame(self.raw_h5file)

        fig, ax = plt.subplots(figsize=(10, 10))

        # Displaying raw frame with a reversed gray colormap (white background, black spots)
        ax.imshow(raw_frame, cmap='gray_r', origin='lower')

        # Overlaying the proc frame with a reversed red colormap
        # Setting all zero values to transparent in the proc frame
        proc_cmap = plt.cm.Reds_r
        proc_cmap.set_bad(color='none')
        ax.imshow(np.ma.array(proc_frame, mask=proc_frame==0), cmap=proc_cmap, origin='lower')
