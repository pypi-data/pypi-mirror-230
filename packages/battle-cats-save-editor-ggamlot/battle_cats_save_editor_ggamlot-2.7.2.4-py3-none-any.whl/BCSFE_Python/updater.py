...
def update(latest_version: str, command: str = "py") -> bool:
    ...
    try:
        full_cmd = f"{command} -m pip install --upgrade battle-cats-save-editor-ggamlot=={latest_version}"
        subprocess.run(
            full_cmd,
            shell=True,
            capture_output=True,
...
def try_update(latest_version: str):
    ...
    if success:
        return
    helper.colored_text(
        "Update failed\nYou may need to manually update with py -m pip install -U battle-cats-save-editor-ggamlot",
        base=helper.RED,
    )
...
def get_version_info() -> Optional[tuple[str, str]]:
    ...
    package_name = "battle-cats-save-editor-ggamlot"
    try:
