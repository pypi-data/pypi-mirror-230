...
def check_contract(sde, y0, ts, bm, method, adaptive, options, names, logqp):
    ...
    if not paddle.is_tensor(y0):
        raise ValueError("`y0` must be a paddle.Tensor.")
    if y0.dim() != 2:
    ...
    if logqp:
        ...
        y0 = paddle.concat((y0, paddle.zeros(((y0.shape[1], 1)), dtype=y0.dtype)), axis=1)
    # ----------------------------------------
    ...
    if not paddle.is_tensor(ts):
        ...
        ts = paddle.to_tensor(ts, dtype=y0.dtype,)
    ...
    if hasattr(sde, 'g_prod'):
        ...
        v = paddle.randn([batch_sizes[0], noise_sizes[0]], dtype=y0.dtype, )
        g_prod_shape = tuple(sde.g_prod(ts[0], y0, v).shape)
    ...
    if hasattr(sde, 'f_and_g_prod'):
        ...
        f_drift_shape = tuple(_f.shape)
        g_prod_shape = tuple(_g_prod.shape)
        _check_2d('Drift', f_drift_shape)
        _check_2d('Diffusion-vector product', g_prod_shape)
    ...
    if bm is None:
        ...
        bm = BrownianInterval(t0=ts[0], t1=ts[-1], size=(batch_sizes[0], noise_sizes[0]), dtype=y0.dtype,
                              device=y0.place, levy_area_approximation=levy_area_approximation)
...
def parse_return(y0, ys, extra_solver_state, extra, logqp):
    if logqp:
        ys, log_ratio = ys.split((y0.shape[1] - 1, 1), axis=2)
        log_ratio_increments = paddle.stack(
            [log_ratio_t_plus_1 - log_ratio_t
             for log_ratio_t_plus_1, log_ratio_t in zip(log_ratio[1:], log_ratio[:-1])], axis=0
        ).squeeze(axis=2)
