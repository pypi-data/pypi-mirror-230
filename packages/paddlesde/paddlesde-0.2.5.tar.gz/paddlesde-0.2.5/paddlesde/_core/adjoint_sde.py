...
from ..types import TensorOrTensors, Sizes
...
class AdjointSDE(base_sde.BaseSDE):
    ...
    def get_state(self, t, y_aug, v=None, extra_states=False):
        ...
        assert t.is_leaf, "Internal error: please report a bug to paddlesde"
        assert y_aug.is_leaf, "Internal error: please report a bug to paddlesde"
        if v is not None:
            assert v.is_leaf, "Internal error: please report a bug to paddlesde"
    ...
    def _f_corrected_default(self, f, g, y, adj_y, requires_grad):
        g_columns = [g_column.squeeze(axis=-1) for g_column in g.chunk(g.shape[-1], axis=-1)]
        dg_g_jvp = sum([
            misc.jvp(
                outputs=g_column,
                inputs=y,
    ...
    def g(self, t, y):
        ...
        raise RuntimeError("Adjoint `g` not defined. Please report a bug to paddlesde.")
    ...
    def f_and_g(self, t, y):
        ...
        raise RuntimeError("Adjoint `f_and_g` not defined. Please report a bug to paddlesde.")
    ...
    def prod(self, g, v):
        ...
        # never get here.
        raise RuntimeError("Adjoint `prod` not defined. Please report a bug to paddlesde.")
