...
def compute_error(y11: TensorOrTensors, y12: TensorOrTensors, rtol, atol, eps=1e-7):
    ...
    """
    if paddle.is_tensor(y11):
        y11 = (y11,)
    if paddle.is_tensor(y12):
        y12 = (y12,)
    tol = [
        (rtol * paddle.maximum(paddle.abs(y11_), paddle.abs(y12_)) + atol).clip(eps)
...
def _rms(x, eps=1e-7):
    if paddle.is_tensor(x):
        return paddle.sqrt((x ** 2.).sum() / x.numel()).clip(eps)
    else:
        return paddle.sqrt(sum((x_ ** 2.).sum() for x_ in x) / sum(x_.numel() for x_ in x)).clip(eps)
