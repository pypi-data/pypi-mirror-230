...
def _randn(size, dtype, device, seed):
    generator = paddle.Generator().manual_seed(int(seed))
    return paddle.randn(size, dtype=dtype, generator=generator)


def _is_scalar(x):
    return isinstance(x, int) or isinstance(x, float) or (isinstance(x, paddle.Tensor) and x.numel() == 1)
...
def _check_tensor_info(*tensors, size, dtype, device):
    ...
    tensors = list(filter(paddle.is_tensor, tensors))
    ...
    devices += [t.place for t in tensors]
    ...
    # Make sure size is a tuple (not a paddle.Size) for neat repr-printing purposes.
...
def _davie_foster_approximation(W, H, h, levy_area_approximation, get_noise):
    if levy_area_approximation in (LEVY_AREA_APPROXIMATIONS.none, LEVY_AREA_APPROXIMATIONS.space_time):
        ...
        noise = get_noise()
        trs = list(range(noise.shape))
        trs[-1], trs[-2] = trs[-2], trs[-1]
        noise = noise - noise.transpose(trs)  # noise is skew symmetric of variance 2
