...
class BaseSDE(abc.ABC, nn.Layer):
...
class ForwardSDE(BaseSDE):
    ...
    def g_prod_and_gdg_prod_default(self, t, y, v1, v2):
        requires_grad = paddle.is_grad_enabled()
    ...
    def dg_ga_jvp_column_sum_v2(self, t, y, a):
        ...
        with paddle.enable_grad():
            ...
            y_dup = paddle.repeat_interleave(y, repeats=m, axis=0)
            g_dup = self.g(t, y_dup)
            ga_flat = ga.transpose([0, 2, 1]).flatten(0, 1)
            dg_ga_jvp, = misc.jvp(
                outputs=g_dup,
                inputs=y_dup,
                grad_inputs=ga_flat,
            ...
            dg_ga_jvp = dg_ga_jvp.reshape([batch_size, m, d, m]).transpose([0, 2, 1, 3])
            dg_ga_jvp = dg_ga_jvp.diagonal(axis1=-2, axis2=-1).sum(-1)
        return dg_ga_jvp
...
class SDELogqp(BaseSDE):
    ...
    def f_diagonal(self, t, y: Tensor):
        ...
        f_logqp = .5 * (u ** 2).sum(axis=1, keepdim=True)
        return paddle.concat([f, f_logqp], axis=1)

    def g_diagonal(self, t, y: Tensor):
        y = y[:, :-1]
        g = self._base_g(t, y)
        g_logqp = paddle.zeros((y.shape[0], 1), dtype=y.dtype)
        return paddle.concat([g, g_logqp], axis=1)
    ...
    def f_and_g_diagonal(self, t, y: Tensor):
        ...
        f_logqp = .5 * (u ** 2).sum(axis=1, keepdim=True)
        g_logqp = paddle.zeros((y.shape[0], 1), dtype=y.dtype)
        return paddle.concat([f, f_logqp], axis=1), paddle.cat([g, g_logqp], axis=1)
    ...
    def g_general(self, t, y: Tensor):
        ...
        g_logqp = paddle.zeros((g.shape[0], 1, g.shape[-1]), dtype=y.dtype)
        return paddle.concat([g, g_logqp], axis=1)
    ...
    def f_and_g_general(self, t, y: Tensor):
        ...
        f_logqp = .5 * (u ** 2).sum(axis=1, keepdim=True)
        g_logqp = paddle.zeros((g.shape[0], 1, g.shape[-1]), dtype=y.dtype)
        return paddle.concat([f, f_logqp], axis=1), paddle.concat([g, g_logqp], axis=1)
# ----------------------------------------
