...
class _SdeintAdjointMethod(paddle.autograd.PyLayer):
    ...
    def backward(ctx, grad_ys,):  # noqa
        ys, ts, *extras_and_adjoint_params = ctx.saved_tensor()
        ...
        aug_state = [ys[-1], grad_ys[-1]] + list(grad_extra_solver_state) + [paddle.zeros_like(param)
        ...
        if ctx.saved_extras_for_backward:
            ...
            out = [aug_state[1]] + aug_state[2:]
        return out


def sdeint_adjoint(sde: nn.Layer,
                   y0: Tensor,
                   ts: Vector,
    ...
    Args:
        sde (paddle.nn.Layer): Object with methods `f` and `g` representing the
    ...
    adjoint_params = filter(lambda x: not x.stop_gradient, adjoint_params)
    ...
    if method == METHODS.reversible_heun:
        ...
        if adaptive or adjoint_adaptive:
            ...
            if not paddle.allclose(num_steps, num_steps.round()):
