...
class IterPert:
    ...
    def initialize_data(self, path, dataset_name, batch_size = 256, 
                        test_fraction = 0.1, custom_test = None):
        self.dataset = Data(path, dataset_name, batch_size, test_fraction, self.seed, custom_test)
    ...
    def initialize_active_learning_strategy(self, strategy):
        available = ['Random', 'BALD', 'BatchBALD', 'BAIT', 'ACS-FW', 'Core-Set', 'BADGE', 'LCMD', 'IterPert']
        ...
        if strategy == 'IterPert':
            ...
            for i in kernel_list:
                _, k, f = load_kernel(self.dataset_name, i)
                prior_kernel_list.append(k)
                prior_feat_list.append(f)
            pert_list, true_gold, truth_feat = load_kernel(self.dataset_name, 'ground_truth_delta')
