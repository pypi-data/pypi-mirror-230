...
class ConspiracyDetector:
    ...
    def download_model(self):
        ...
        if not (os.path.exists(model_file) and os.path.exists(tfidf_file) and os.path.exists(stopwords_file)):
            # 下载模型文件
            url = 'https://raw.githubusercontent.com/mengxiao2000/conspiratory-cn/main/cons_model.pkl'
            r = requests.get(url)
            open(model_file, 'wb').write(r.content)

            url = 'https://raw.githubusercontent.com/mengxiao2000/conspiratory-cn/main/tfidf.pkl'
            r = requests.get(url)
            open(tfidf_file, 'wb').write(r.content)

            url = 'https://raw.githubusercontent.com/mengxiao2000/conspiratory-cn/main/hit-stopwords.txt'
            r = requests.get(url)
