...
class ChainCombiner(ChainTreeBuilder):
    def __init__(
        ...
        if api_key:
            self.spatial_similarity = SpatialSimilarity(api_key=api_key,verbose=verbose)
        else:
            self.spatial_similarity = (
                spatial_similarity if spatial_similarity else SpatialSimilarity(verbose=verbose)
            )
