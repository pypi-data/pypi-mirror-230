...
class CoordinateRepresentation(Representation):
    ...
    def tetra_dict_to_tree(
        ...
        **kwargs: Dict[str, Any],
        ...
        def build_tree(node_id: str, root: CoordinateTree):
            stack = deque([(node_id, root, 0)])
            
            while stack:
                node_id, node, depth = stack.pop()
                
                # Base case: if maximum depth reached, continue to the next iteration
                if depth > max_depth:
                    continue
                ...
                for child_id in child_ids:
                    ...
                    stack.append((child_id, child, depth + 1))
