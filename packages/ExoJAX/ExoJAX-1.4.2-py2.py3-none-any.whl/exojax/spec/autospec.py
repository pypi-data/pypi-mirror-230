...
from exojax.spec.hitran import line_strength, doppler_sigma, gamma_natural, gamma_hitran, normalized_doppler_sigma
from exojax.spec.exomol import gamma_exomol
from exojax.spec.rtransfer import rtrun_emis_pureabs_fbased2st, dtauM, dtauCIA
...
class AutoXS(object):
    ...
    def linest(self, T):
        ...
        return line_strength(T, self.mdb.logsij0, self.mdb.nu_lines, self.mdb.elower,
                    qt)
...
class AutoRT(object):
    ...
    def rtrun(self):
        ...
        """
        self.F0 = rtrun_emis_pureabs_fbased2st(self.dtau, self.sourcef)
        return self.F0
    ...
    def spectrum(self,
        ...
        Frot = convolve_rigid_rotation(F0, vr_array, self.vsini, self.u1,
                                       self.u2)
        self.F = ipgauss_sampling_slow(self.nuobs, self.nus, Frot, beta, self.RV)
        return self.F
