...
def erfcx_scan(x):
    ...
    q = (-a*(a-2.0)/(a+2.0)-2.0*((a-2.0)/(a+2.0)+1.0)+a) / \
        (a+2.0) + (a-2.0)/(a+2.0)
    _CHEV_COEFS_ = [
        5.92470169e-5, 1.61224554e-4, -3.46481771e-4, -1.39681227e-3,
        1.20588380e-3, 8.69014394e-3, -8.01387429e-3, -5.42122945e-2,
        1.64048523e-1, -1.66031078e-1, -9.27637145e-2, 2.76978403e-1
    ]
...
def rewofzt2(x, y):
    ...
    """
    z = x + y * (1j)
    q = (1j) * z / (jnp.sqrt(jnp.pi)) * (z * z - 2.5) / (z * z *
                                                         (z * z - 3.0) + 0.75)
    return jnp.real(q)
