...
def _start_job(
    ...
    requests_kwargs = []
    for url, input_ in zip(input_urls, inputs):
        request_kwargs = dict(method="put", url=url, headers=BYTES_HEADER, data=dill.dumps(input_))
        requests_kwargs.append(request_kwargs)
    make_async_requests(requests_kwargs)
...
def remote_parallel_map(
    ...
    for input_batch in input_batches:
        try:
            ...
            kwargs = [dict(method="get", url=url, headers=BYTES_HEADER) for url in output_urls]
            pickled_results = make_async_requests(kwargs)
            batch_return_values = [dill.loads(pickled_result) for pickled_result in pickled_results]
            return_values.extend(batch_return_values)
