...
from .query_tree import QueryTreeNode
...
def debug_vis(root: QueryTreeNode, output_folder: Path, browser: bool = True):
    """
...
def _visit_nodes(
    root: QueryTreeNode,
) -> Tuple[DiGraph, Dict[QueryTreeNode, str]]:
    """
...
def _recursive_visit(
    ...
    *,
    visited: Dict[QueryTreeNode, str],
    node_type_count: Dict[Type[QueryTreeNode], int],
    graph: DiGraph,
    context: QueryTreeNode | None = None,
):
    """
    Parameters
    ----------
    ...
    context: QueryTreeNode | None
        The child of the current subtree. None for the starting node.
    """
    assert isinstance(root, QueryTreeNode), type(root)

    if root in visited:
        return

    # Mark visited
    node_type_count[type(root)] += 1
    assert root not in visited
    root_name = f"{type(root).__name__}_{node_type_count[type(root)]}"
...
def _split_by_generated_sql(node: QueryTreeNode) -> List[str | QueryTreeNode]:
    """
...
def _recursive_gen_sql_indent(
    root: QueryTreeNode,
    *,
    sql_splits: Dict[QueryTreeNode, List[str | QueryTreeNode]],
    indent_level: int = 0,
    background: Callable[[QueryTreeNode], str] = lambda _: "#FFFFFF",
...
def _sql_highlighter(
    *, visited: Dict[QueryTreeNode, str], graph: DiGraph
) -> Dict[str, str]:
    """
...
def _gen_sql_splits(
    visited: Dict[QueryTreeNode, str]
) -> Dict[QueryTreeNode, List[str | QueryTreeNode]]:
    """
    ...
    generated_splits: Dict[QueryTreeNode, List[str | QueryTreeNode]] = {}
...
def PONDER_SHOW_FORMATTED_SQL(node: QueryTreeNode) -> str:
    """
...
def _color_palatte(
    ...
    type_id: Dict[Type[QueryTreeNode], int] = {}
    for node, name in visited.items():
        if type(node) in type_id:
            continue

        if len(type_id) == 0:
            type_id[type(node)] = 1
        else:
            type_id[type(node)] = max(type_id.values()) + 1

    for node, name in visited.items():
        id = type_id[type(node)]
