...
def launch_custom_rippled_networks_menu():
    ...
    if selection_text == "Build rippled from local code":
        ...
        relaunch_wizard = False
        clear_screen()
        print("\nINFO: In order to '{}', run: 'claudia rippled install'".format(selection_text))
        print("You have opted to execute '{}'. Starting now...\n".format(selection_text))
...
def launch_command_rippled_install():
    command = get_launch_command_rippled_install()
    return subprocess.call(command, shell=True)


def get_launch_command_rippled_install():
    set_to_project_root_wd()
    return "sh network_setup/setup.sh --rippledInstall"
