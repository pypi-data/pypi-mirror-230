...
push_start_opt = typer.Option(
    help="Push the dates so that it " "starts at this date.", formats=["%Y-%m-%d"]
)
...
def amanuens_cmd(
    ...
    if start:
        ...
        amanuensis = hr.compute_amanuensis_data(booked, begin_date=start)
    else:
        amanuensis = hr.compute_amanuensis_data(booked)
            ...
            json.dump(
                {
                    ...
                    "push_start": push_start.isoformat(),
                    "end": end.isoformat(),
                    "events": events,
                },
                outfile,
