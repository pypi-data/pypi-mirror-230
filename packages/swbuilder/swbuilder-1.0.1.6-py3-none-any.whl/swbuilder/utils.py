...
def generate_name(image, tag, repo):
    repo_base = repo
...
def build_targets(args, defs, targets):
    ...
    defs_name = defs.env.get("name", None)
    ...
    for t in targets:
        ...
        if (defs_name):
            targetname = generate_name(defs_name, t, repository)
        else:
            targetname = generate_name(t, tag, repository)
