...
def evaluate_model(experiment, arguments_to_check, baseline=False):
    ...
    for i in range(0, arguments_to_check):
        ...
        sentence_transformer_examples = sample_similar(i, sentence_transformer_similarities, df_training, df_training.shape[0])
        ...
        lda_transformer_examples = sample_similar(i, lda_similarities, df_training, df_training.shape[0])
        lda_transformer_examples.rename(columns={"text":"lda-text", "topic":"lda-topic"},inplace=True)
        ...
        all_similar_examples.append(pd.concat([examples_sorted.reset_index(), sentence_transformer_examples.reset_index(), lda_transformer_examples.reset_index()], axis=1))
    ...
    df_sorted_examples.to_csv("~/contexutal_topic_model_evaluation.csv", sep="\t", columns=["query-text", "query-topic",
                                                                    "ctm-text", "ctm-topic", "sentence-transformer-text", "sentence-transformer-topic", "lda-text", "lda-topic"])
