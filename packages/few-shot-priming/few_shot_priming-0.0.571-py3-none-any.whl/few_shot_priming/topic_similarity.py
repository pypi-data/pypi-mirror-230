...
def evaluate_model(experiment, arguments_to_check, baseline=False):
    ...
    if baseline:
        baseline_similarities = calc_similarity_sentence_transformer(df_validation, df_training)
    ...
    for i in range(0, arguments_to_check):
        ...
        baseline_examples_sorted = sample_similar(i, baseline_similarities, df_training, df_training.shape[0])
        baseline_examples_sorted.rename(columns={"text":"sentence-transformer-text", "topic":"sentence-transformer-topic"}, inplace=True)
        ...
        all_similar_examples.append(pd.concat([examples_sorted.reset_index(inplace=True), baseline_examples_sorted.reset_index(inplace=True)], axis=1))
    ...
    df_sorted_examples.to_csv("~/contexutal_topic_model_evaluation.csv", sep="\t", columns=["query-text", "query-topic",
                                                                    "ctm-text", "ctm-topic", "sentence-transformer-text", "sentence-transformer-topic"])
