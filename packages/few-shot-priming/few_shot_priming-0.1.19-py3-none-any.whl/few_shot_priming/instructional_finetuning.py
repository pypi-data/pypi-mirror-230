...
def generate_and_tokenize_datasets(data, prompter, cutoff_len, train_on_inputs=True, add_eos_token=False):
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        if not train_on_inputs:
            ...
            tokenized_user_prompt = tokenize(
                user_prompt, add_eos_token=add_eos_token, cutoff_len=cutoff_len
            )
    ...
    train_data = data["training"].shuffle().map(generate_and_tokenize_prompt)
...
def prepare_dataset(experiment, prompter, cut_off_len):
    ...
    train_data, val_data, test_data = generate_and_tokenize_datasets(data, prompter, cut_off_len=cut_off_len)
    return train_data, val_data, test_data
...
def run_instructional_fine_tuning(config, params, experiment, validate, logger):
    ...
    train_data, val_data, test_data = prepare_dataset(experiment, prompter, cutoff_len)
    if validate:
        test_data = val_data
    model = train(train_data, test_data, tokenizer, prompter, model_name, logger, batch_size=params["batch-size"], num_epochs=params["epochs"], output_dir=config["model-path-fine-tuned"])
