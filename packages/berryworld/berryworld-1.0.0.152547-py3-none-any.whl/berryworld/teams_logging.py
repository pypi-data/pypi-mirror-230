...
import datetime
import re
from bwgds.berryworld.microsoft_teams import MicrosoftTeams
from bwgds.berryworld.vivantio import Vivantio
...
class TeamsLogging:
    ...
    def __init__(self, organisation_id, teams_credentials, vivantio_token=None):
        if teams_credentials is None:
            raise Exception('Credentials are required to connect to Teams Logging')
        else:
            self.ms_teams_con = MicrosoftTeams(organisation_id=organisation_id, credentials=teams_credentials)

        self.vivantio = False
        if vivantio_token:
            self.vivantio = True
            self.vivantio_con = Vivantio(token=vivantio_token)
