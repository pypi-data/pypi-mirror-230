...
class StreamApiData(BaseModel):
    ...
    # 【输入】任务明细清单
    task_items: List[StreamTaskItem] = Field(description="任务明细清单")
    # 【输入】任务队列后缀（用于拆分某些应用专用任务队列）
    task_queue_suffix: str = Field(description="任务队列后缀", default=None)
    # 【输入】文件名称配置键
    file_name_config: str = Field(description="任务明细清单", default='default')
