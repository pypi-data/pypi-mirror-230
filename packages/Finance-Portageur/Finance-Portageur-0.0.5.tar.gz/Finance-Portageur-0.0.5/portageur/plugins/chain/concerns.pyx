...
class Concerns(object):
    ...
    def _summary_match_sources(self, name, results_map, citationmap_dict,
        ...
        for search_term in controversy_dict.keys():
            ...
            answer_new, citation_map_new = ESGControversy.match_sources_and_format(answer, citation_map)
            # if len(md_str) > 0:
            #     answer += f"\nSources: \n{md_str}"
            cache_data = {
                'task': 'controversy',
    ...
    def _overall_match_sources(self, name, overall, controversy_dict,
        ...
        answer_new, citation_map_new = ESGControversy.match_sources_and_format(overall, flat_citemap)
        # if len(md_str) > 0:
        #     overall += f"\n\nSources: \n{md_str}"
        cache_data = {
            'task': 'controversy',
