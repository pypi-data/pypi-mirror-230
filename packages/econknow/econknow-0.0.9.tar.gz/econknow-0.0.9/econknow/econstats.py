...
def prean():
    ...
    lower_bound = mean - t_crit * std / np.sqrt(n)
    upper_bound = mean + t_crit * std / np.sqrt(n)

    # Проверяем каждое наблюдение на аномалию
    outliers = []
    for d in data:
...
def krr():
    ...
    df['T'] = pd.period_range(start='2008-01-01', end='2021-11-3', freq='Q')
    df.index = df.index.to_timestamp(freq='Q')
    
    from statsmodels.tsa.holtwinters import ExponentialSmoothing
    1. model = ExponentialSmoothing(df["y"], seasonal="add", seasonal_periods=4)
    fit_model = model.fit()
...
lower = forecast - t.ppf(confidence_level, 4) * se
    2. браун: model = SimpleExpSmoothing(df.y)

fit_model = model.fit()
ху 
model = ExponentialSmoothing(df.y, seasonal_periods=4, trend='add', seasonal='add', dates=df.index)
fit_model = model.fit()
...
pre_trend = df.y.rolling(window_size).mean()
trend1 = pre_trend.rolling(window_size).mean()
trend2 = trend1.rolling(window_size).mean()
result = seasonal_decompose(df.y, model='multiplecative')
...
result.seasonal[::1].plot(ax = ax1)

ax1.set_title('Компонента 1')

result.seasonal[::2].plot(ax =ax2)
ax2.set_title('Компонента 2')

result.resid.plot(ax =ax3)
ax3.set_title('Остаточная')
    """)


def dox():
    print("""
    df['доходность'] = (df['<CLOSE>'] - df['<OPEN>'] ) / df['<OPEN>']
...
if test[0]> test[4]['5%']:
    ...
    def autocov(data):
    autocov = np.correlate(data, data, mode='full')[-len(data):]

    plt.plot(autocov)
    plt.xlabel('Лаг')
    plt.ylabel('Автоковариация')
    plt.title('График автоковариации')
    plt.grid(True)
    plt.show()
 ...
 def build_model(data, p1, p2=0, p3=0):
    model = sm.tsa.ARIMA(data, order=(p1, p2, p3))
    model_fit = model.fit()
...
sum_odds_ratios = np.sum(odds_ratios)
contributions = odds_ratios / sum_odds_ratios

for i, factor in enumerate(X.columns):
    print(f"Фактор {factor}: Доля вклада = {contributions[i]:.2%}")

    """)

def panel():
    print("""
    data['Год'] = pd.to_datetime(data['Год'], format='%Y')
    df = pd.read_csv('exam_1.csv', index_col=['НазваниеРегиона', 'Год'], parse_dates=['Год'])
df

model = sm.MixedLM.from_formula('Y ~ X1 + X2 + X3 + X4',data ,groups=data['НазваниеРегиона'])
result = model.fit()

print(result.summary())

model = PanelOLS.from_formula('Y ~ X1 + X2 + X3 + X4 + EntityEffects', data=df)
res = model.fit()
...
fixed_model = PanelOLS.from_formula('Y ~ 1 + X1 + X2 + X3 + X4 + EntityEffects', data=df)
fixed_effects_results = fixed_model.fit(cov_type='clustered', cluster_entity=True)


random_model = RandomEffects.from_formula('Y ~ 1 + X1 + X2 + X3 + X4', data=df)
random_effects_results = random_model.fit(cov_type='clustered', cluster_entity=True)

# Получение коэффициентов и их стандартных ошибок
fixed_effects_coefs = fixed_effects_results.params
fixed_effects_std_errors = fixed_effects_results.std_errors

random_effects_coefs = random_effects_results.params
random_effects_std_errors = random_effects_results.std_errors

# Расчет теста Хаусмана
hausman_test_stat = np.square(fixed_effects_coefs - random_effects_coefs) / (fixed_effects_std_errors**2 - random_effects_std_errors**2)
p_value = 1 - chi2.cdf(hausman_test_stat, df=5)


print(hausman_test_stat)
print(p_value)
...
def theor():
    print("""
    предварительный анализ рядов: econknow.prean()
    кривай роста econknow.krr()
    доходность любой акции econknow.dox()
    логит пробит econknow.lp()
    панельные данные econknow.panel()
    """)
