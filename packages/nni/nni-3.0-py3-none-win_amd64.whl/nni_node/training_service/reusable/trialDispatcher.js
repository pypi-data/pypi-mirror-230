...
class TrialDispatcher {
    ...
    getStorageService(environmentService) {
        ...
        if (this.useSharedStorage) {
            ...
            storageService = ioc_shim_1.IocShim.get(sharedStorage_1.SharedStorageService).storageService;
        }
        else if (environmentService.hasStorageService) {
            this.log.debug(`TrialDispatcher: use existing storage service.`);
            storageService = ioc_shim_1.IocShim.get(storageService_1.StorageService);
        }
        else {
    ...
    async cleanUp() {
        ...
        if (this.useSharedStorage) {
            ...
            await ioc_shim_1.IocShim.get(sharedStorage_1.SharedStorageService).cleanUp();
    ...
    async allocateEnvironment(trial, environment) {
        ...
        if (this.useSharedStorage) {
            const storageService = ioc_shim_1.IocShim.get(sharedStorage_1.SharedStorageService).storageService;
    ...
    async initializeSharedStorage(config) {
        switch (config.storageType) {
            case 'NFS':
                ioc_shim_1.IocShim.bind(sharedStorage_1.SharedStorageService, nfsStorageService_1.NFSSharedStorageService);
                break;
            case 'AzureBlob':
                ioc_shim_1.IocShim.bind(sharedStorage_1.SharedStorageService, azureblobStorageService_1.AzureBlobSharedStorageService);
                break;
            default: {
        ...
        await ioc_shim_1.IocShim.get(sharedStorage_1.SharedStorageService).config(config);
        this.useSharedStorage = true;
        return Promise.resolve();
    }
    async getTrialOutputLocalPath(trialJobId) {
        if (this.useSharedStorage) {
            const localWorkingRoot = ioc_shim_1.IocShim.get(sharedStorage_1.SharedStorageService).localWorkingRoot;
