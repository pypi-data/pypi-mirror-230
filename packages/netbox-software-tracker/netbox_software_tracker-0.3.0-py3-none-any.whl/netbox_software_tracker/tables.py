...
class GoldenImageListTable(NetBoxTable):
    ...
    def order_progress(self, queryset, is_descending):
        upgraded_count = Count(
            "instances", 
            filter=Q(**{f"instances__custom_field_data__{CF_NAME_SW_VERSION}__iregex": F("golden_image__software__version")}),
        )
        
        queryset = queryset.annotate(total=Count("instances"), upgraded=upgraded_count)
        queryset = queryset.annotate(progress=Round(Cast(F("upgraded"), FloatField()) / Cast(F("total"), FloatField()) * 100, 2))
