...
class ConnectSRHFT_API :
    ...
    def _getTokenFeed(self,tokenid2=str()):
        url_details=self.__urls.get('getTokenFeed')
        print(url_details)
        response = self.http.post(url_details.get('endPoint'),json={"event":url_details.get("event"),"access_token":self.accesstoken, "tokenid2":tokenid2, "data":{}})
        return response.json()
    
    def _getHistoricalData(self,data=dict()):
        url_details=self.__urls.get('getHistoricalData')
        print(url_details)
        response = self.http.post(url_details.get('endPoint'),json={"event":url_details.get("event"),"access_token":self.accesstoken, "data":data})
        return response.json()
