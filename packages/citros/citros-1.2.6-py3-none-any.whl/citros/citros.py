...
class Citros:
    ...
    def checkout_user_branch_if_different(self):
        repo = git.Repo(self.CITROS_REPO_DIR)
        citros_branch = repo.active_branch.name
        ...
        if citros_branch != user_branch:
            self.checkout_branch(user_branch, 
                                    create_if_not_exist=True, 
                                    uncommitted_changes_ok=True)
    ...
    def checkout_branch(self, branch_name, create_if_not_exist=False, uncommitted_changes_ok=False):
        if self.check_uncommitted_changes(self.CITROS_REPO_DIR) and not uncommitted_changes_ok:
        ...
        if self.branch_exists(branch_name):
            ...
            def create_and_checkout():
                msg = f"Creating and checking out new citros branch `{branch_name}`."
    ...
    def merge_branch(self, branch_name):
        ...
        try:
            ...
            if repo.is_dirty(untracked_files=True):
                ...
                msg = f"Error occurred during merge: {ex}"
                self.print(msg, color='red')
                self.log.error(msg)
