...
class StatusCode(Enum):
    ...
    """
    COULD_NOT_RETRIEVE_PRO_FIRMWARE = -10021
    """
    Device firmware could not be retrieved. Check that the device is running Pro
    firmware, that the device ID is correct, that the specified CAN bus is correct,
    and that the device is powered.
    """
    COULD_NOT_DECODE_DEVICE_FIRMWARE = -10022
    """
    Device firmware could not be decoded. Check that the device is running Pro
    firmware, that the device ID is correct, that the specified CAN bus is correct,
    and that the device is powered.
    """
    ...
    def get_description(self) -> str:
        if(False):
            ...
            return "COULD_NOT_RETRIEVE_PRO_FIRMWARE"
