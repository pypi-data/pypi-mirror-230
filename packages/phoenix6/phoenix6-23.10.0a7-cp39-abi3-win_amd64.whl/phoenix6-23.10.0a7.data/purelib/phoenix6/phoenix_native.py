...
class Native:
    ...
    __c_args = [
        ...
        (
            ...
            c.c_int,
            [c.c_int, c.c_int, c.c_char_p, c.POINTER(c.c_uint32)],
        ),
        (
    ...
    def instance(clazz):
        ...
        if clazz.__instance is None:
            os.add_dll_directory(pkg_resources.resource_filename("phoenix6", "lib")) # Add local lib to path so we can find our libs
            ...
            if "CTR_TARGET" in os.environ and os.environ["CTR_TARGET"] == "Hardware":
                first_load = "CTRE_PhoenixTools"
                second_load = "CTRE_PhoenixTools_Sim"
            else:
                first_load = "CTRE_PhoenixTools_Sim"
                second_load = "CTRE_PhoenixTools"

            try:
                # First try our desired target
                clazz.__instance = c.cdll.LoadLibrary(first_load + extension)
            except:
                try:
                    # If we fail, then load the second target
                    clazz.__instance = c.cdll.LoadLibrary(second_load + extension)
                except:
