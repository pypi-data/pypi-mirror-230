...
from tecton_core.query.executor_params import URI_SCHEME_TO_STAGING_LOCATION
...
def build_materialization_querytree(
    ...
    staging_destination_uri: Optional[ParseResult] = None,
) -> NodeRef:
    ...
    Args:
        ...
        staging_destination_uri: If set, the results will be staged to a location with the given uri.
    """
    ...
    if fdw.is_temporal:
        ...
        tree = StagingNode(
            ...
            staging_location=QueryTreeStagingLocation.MEMORY,
        ...
        raise Exception(msg)
    if staging_destination_uri:
        staging_location = URI_SCHEME_TO_STAGING_LOCATION[staging_destination_uri.scheme]
        tree = StagingNode(
            input_node=tree,
            staging_table_name=f"{fdw.name}",
            staging_location=staging_location,
            query_tree_step=QueryTreeStep.AGGREGATION,
            staging_destination_uri=staging_destination_uri,
        ).as_ref()
    return tree
