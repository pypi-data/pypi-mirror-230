...
from tecton_core.query.pandas.nodes import PandasStagedTableScanNode
...
def _convert_to_pandas_nodes(tree: NodeRef, odfv_input_df: pd.DataFrame) -> PandasExecNode:
    ...
    if isinstance(logical_tree_node, StagedTableScanNode):
        return PandasStagedTableScanNode(
            input_df=odfv_input_df, input_node=None, columns=logical_tree_node.columns, column_name_updater=lambda x: x
        )
