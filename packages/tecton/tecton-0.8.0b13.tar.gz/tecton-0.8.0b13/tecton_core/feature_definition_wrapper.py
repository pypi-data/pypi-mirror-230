...
class FeatureDefinitionWrapper:
    ...
    @property
    def max_aggregation_window(self) -> duration_pb2.Duration:
        if not self.is_temporal_aggregate:
            msg = "Invalid `max_aggregation_window` invocation on unsupported FeatureView type"
            raise TypeError(msg)
    ...
    @property
    def earliest_anchor_time_from_max_aggregation_interval(self) -> int:
        return self.earliest_anchor_time_from_window(self.max_aggregation_window.ToTimedelta())

    def earliest_anchor_time_from_window(self, window: datetime.timedelta) -> int:
