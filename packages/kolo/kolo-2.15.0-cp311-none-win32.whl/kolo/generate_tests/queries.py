...
class AssertUpdate:
    def __init__(self, variable_name, model, fields, query=None):
        self.variable_name = variable_name
        self.model = model
        self.fields = fields
        self.query = query
...
def collect_gfk(fields, name, schema, names):
    ...
    for field in fields:
        if field.name == schema["gfk_content_type_field"]:
            content_type_field = field
        elif field.name == schema["gfk_object_id_field"]:
...
def build_update_assert_fields(columns, schema_fields, names):
    ...
    for name, schema in schema_fields.items():
        if schema["gfk_object_id_field"]:
            fields = collect_gfk(fields, name, schema, names)
    return fields
            ...
            if isinstance(parsed_query, sqlglot.expressions.Update):
                ...
                for lookup_name, lookup_value in lookups:
                    ...
                    assert_update = AssertUpdate(
                        variable_name, table_schema["model_name"], fields, query
                    )
