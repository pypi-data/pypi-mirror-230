...
@contextmanager
def db_cursor(db_path, wal_mode=True, timeout=60):
    """
    ...
    if wal_mode:
        connection = sqlite3.connect(
            str(db_path), isolation_level=None, timeout=timeout
        )
        connection.execute("pragma journal_mode=wal")
    else:
        connection = sqlite3.connect(str(db_path), timeout=timeout)  # pragma: no cover
...
def save_invocation_in_sqlite(
    ...
    *,
    ...
    with db_cursor(db_path, wal_mode, timeout) as cursor:
