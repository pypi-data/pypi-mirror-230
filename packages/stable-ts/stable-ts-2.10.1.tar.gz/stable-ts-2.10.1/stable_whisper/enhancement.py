...
@torch.no_grad()
def enhance_one_track(model, audio_path, cut_len, n_fft=400, hop=100, output=None):
    ...
    if output:
        sf.write(output, est_audio, sr)

    return est_audio, length


def evaluation(model_path, audio, output):
    n_fft = 400
    model = TSCNet(num_channel=64, num_features=n_fft//2+1).cuda()
    model.load_state_dict((torch.load(model_path)))
    model.eval()
    enhance_one_track(model, audio, 16000*16, n_fft, n_fft//4, output)


if __name__ == '__main__':
    inp = r'E:\Projects\stable-ts\demo/pv.wav'
    o = r'E:\Projects\stable-ts\demo/pvx.wav'
    evaluation(r'E:\Projects\stable-ts\weights/cmgan.pt', inp, o)