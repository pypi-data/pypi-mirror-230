...
class Probe:
    ...
    def run(self, target_path) -> int:
        ...
        def validate_result(res: int):
            _bound = max(res, len(Probe.__return_values__))
            return _bound, Probe.__return_values__.get(str(_bound))
        ...
        logging.info(f"Probe {_validated_res[1]} ({ret})")
        return _validated_res[0]
