...
class Probe:
    ...
    def run(self, target_path) -> int:
        ...
        def validate_result(res: int) -> tuple[str, int]:
            def _filter(key_val):
                return key_val[0] == 'undefined' if res not in Probe.__return_codes__.values() else key_val[1] == res
            return list(filter(_filter, Probe.__return_codes__.items()))[0]
...
class AnalysisMatrix:
    ...
    def analyze(self, include=None, exclude=None):
        ...
        """
        _break_line = '=' * 20
        logging.info("Starting matrix analysis")
        for tg in self.matrix.columns[1:]:
            logging.info(f"Starting subject analysis ({tg.filename})")
            for y_coord, probe in enumerate(self.matrix["Probe"]):
                if (include is not None and probe.id in include) or (exclude is not None and probe.id not in exclude):
                    ...
                    self.matrix.at[y_coord, tg] = probe.run(tg)
            logging.info(f"Subject analysis completed ({tg.filename})\n{_break_line}")
        logging.info(f"Matrix analysis completed. All ({self.count_targets}) subjects analyzed")
