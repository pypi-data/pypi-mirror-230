...
class BucketHelper:
    ...
    def upload_file(self, local_file_path, cloud_blob_path):
        ...
        :return: A tuple (success_count, total_count) indicating the number of successful
        uploads and total files processed.
        """
        try:
            ...
            if os.path.isfile(local_file_path):
                ...
                # Handle directories and their contents recursively
                for item in glob.glob(os.path.join(local_file_path, '*')):
                    if item == ".keep":
                        continue
                    success, total = self.upload_file(item, os.path.join(cloud_blob_path, os.path.basename(item)))
