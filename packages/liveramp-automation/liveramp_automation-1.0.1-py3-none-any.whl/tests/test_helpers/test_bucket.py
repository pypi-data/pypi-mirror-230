...
def test_download_files():
    result = bucket_helper.download_files(destination_blob_name, download_folder)
    assert result != []


def test_download_files_with_structure():
    result = bucket_helper.download_files_with_structure(destination_blob_name, "reports/002")
    assert result != []
