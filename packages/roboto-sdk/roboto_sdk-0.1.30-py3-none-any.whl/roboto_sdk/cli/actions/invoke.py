...
def invoke(
    ...
    action = Action.from_name(
        name=args.action.action_name,
        action_delegate=context.actions,
        invocation_delegate=context.invocations,
        org_id=args.org,
        digest=args.action.action_digest,
        action_owner_id=args.action.action_owner,
    )
    ...
    print(
        f"Queued invocation of '{args.action.reference}'. Invocation ID: '{invocation.id}'"
    )
