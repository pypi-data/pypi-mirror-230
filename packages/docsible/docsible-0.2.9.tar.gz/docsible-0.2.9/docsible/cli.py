...
def document_role(role_path, playbook_content, generate_graph):
    ...
    if os.path.exists(meta_path):
        try:
            ...
            # If 'docsible' key doesn't exist, add it
            if 'docsible' not in meta_data:
                print("I didn't find docsible key into role metadata. Adding those keys to meta/main.yml.")
                docsible = []
                docsible['docsible'].append({
                    'description': '',
                    'description_long': '',
                    'requester': '',
                    'categories': [],
                ...
                print(yaml.safe_load(docsible))
                    
                # # Write the updated data back to 'meta/main.yml'
                # with open(meta_path, 'w') as f:
                #     yaml.dump(meta_data, f, default_flow_style=False)

                print('Once keys are added, relaunch docsible.')
                            
        except Exception as e:
            print(f"An error occurred while processing {meta_path}: {e}")
