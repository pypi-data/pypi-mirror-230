...
class TracerWrapper(object):
    def __new__(cls) -> "TracerWrapper":
        if not hasattr(cls, "instance"):
            ...
            # Force flushes for debug environments (e.g. local development, colab, etc.)
            atexit.register(obj.exit_handler)
            try:
                get_ipython().events.register("post_run_cell", obj.exit_handler)
            except Exception:
                pass  # Ignore if not in a notebook
