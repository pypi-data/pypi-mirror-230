...
class KokocCRMService:
    ...
    def _make_request(cls, method: str, path: str, params: dict = None):
        if params:
            ...
            data = cls._prepare_params(params=params).encode("utf-8")
        else:
            data = None
    ...
    @classmethod
    def get_deals(cls, category: constants.CRM_CATEGORIES = None) -> dict:
        results = cls._make_request(method="get", path="crm.deal.list.json")
        results["data"] = results["data"]["result"]

        if category:
            results["data"] = [item for item in results["data"] if item["CATEGORY_ID"] == category]

        return results

    @classmethod
    def get_statuses(cls, category: constants.CRM_CATEGORIES = None) -> dict:
        results = cls._make_request(method="get", path="crm.status.list.json")
        results["data"] = results["data"]["result"]
    ...
    @classmethod
    def update_deal_stage(cls, deal_id: str, stage: str) -> dict:
        params = {
            "id": deal_id,
            "fields": {
                "STAGE_ID": stage,
    ...
    @classmethod
    def delete_deal(cls, deal_id: str) -> dict:
