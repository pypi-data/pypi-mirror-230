...
def _validate_parse_function(
    ...
    num_annotations = len(stream_input_annotations)
    if num_annotations == 1:
        stream_input_annotation = stream_input_annotations[0]
    elif num_annotations == 2:
        if isinstance(stream_input_annotations[0], StateWrapper):
            stream_input_annotation = stream_input_annotations[1]
        elif isinstance(stream_input_annotations[1], StateWrapper):
            stream_input_annotation = stream_input_annotations[0]
        else:
            raise TypeError(
                f"Streaming resolver '{stream_fqn}' of length '{num_annotations}' must have "
