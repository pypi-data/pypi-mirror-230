...
def init(sweep = False, sweep_name = "", run_name="", save_files = False):
    ...
    if sweep == True:
      ...
      if len(str(sweep_name)) == 0:
        ...
        sweep_name = sweep_name+'_'

      r_id = create_run(int(project_id_global), sweep = True, sweep_name = sweep_name)
      run_id_global = r_id
      run_dir_global = project_dir_global+'/SWP'+str(r_id)+'_'+sweep_name+str(datetime.now())[:10].replace("-","")
      ...
      run_dir_global = project_dir_global+'/R'+str(r_id)+'_'+run_name+str(datetime.now())[:10].replace("-","")
      os.mkdir(run_dir_global)
...
def torch_dataloader(train=None, val=None, test=None, dataset_name="dataset", label_index=1,
  ...
  if newest_data_folder == None:
      ...
      if (train != None) and (len(train) > 0) and (os.path.exists(newest_data_folder + '/train.pkl')): #Both exist
          print("c")
