...
class AcquireLockSensor(BaseSensorOperator):
    """
    ...
    def try_acquire_lock(self, db_table_name, system_name, lakehouse_table_name, partition_col, db_connection_id,
                         hook_type):
        ...
        current_record = hook.get_first(
            ...
            where lower(partition_col)='{partition_col}' and lower(lakehouse_table_name)='{lakehouse_table_name}'""")
        ...
        if current_record is None:
            ...
            hook.run(
                ...
                values('{system_name}','{lakehouse_table_name}','{partition_col}','running',null)""", autocommit=True)
            return True
        elif current_record[1] == 'running':
            # f"system - {current_record[3]} has currently acquired the lock on partition - {partition_col}"
            return False
        elif current_record[1] == 'success':
            current_partition_col = current_record[0]
            current_state = current_record[1]
            current_lakehouse_table_name = current_record[2]
            current_system = current_record[3]
            ...
            end_time=null
            where lower(lakehouse_table_name)='{current_lakehouse_table_name}' 
            and lower(state)='{current_state}'
            and lower(partition_col)='{current_partition_col}'
            and lower(system)='{current_system}'
            """
            hook.run(sql=query, autocommit=True)

            status_check_record = hook.get_first(
                sql=f"""select lower(state) from {db_table_name} 
                where lower(lakehouse_table_name)='{lakehouse_table_name}' and lower(partition_col)='{partition_col}'""")
    ...
    def poke(self, context):
        try:
            ...
            partition_col_seq = list(
                map(lambda element: element.strip(), self.op_kwargs['partition_col_seq'].split(',')))

            partition_cols = []
            for p_col in partition_col_seq:
                p_col_value = self.op_kwargs[p_col]
                partition_cols.append(p_col_value.lower())
            ...
            lock_acquired = self.try_acquire_lock(db_table_name, system_name, lakehouse_table_name, partition_col,
                                                  db_connection_id,
                                                  hook_type)
            ...
            raise Exception("error occurred in AcquireLockSensor.poke", ex)
