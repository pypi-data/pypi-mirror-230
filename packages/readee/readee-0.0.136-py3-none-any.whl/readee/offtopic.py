...
OFFTOPIC_ATT = [
	...
	'elementor-post-navigation', 'elementor-location-popup', 'article/support/request', 'speechify-ignore'
]
...
def _decompose(item):
	...
	if 'sHhtYhaCe2Uc3IU0IgKwIQ' in str(item): # testing code, but no need to comment out
		if item.name in OFFTOPIC_TAG:
			print(item.name)
		if _isOffTopic(item.attrs):
			print(_isOffTopic(item.attrs))
