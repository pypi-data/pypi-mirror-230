...
class DeviceDetails(
    ...
    Attributes:
        ...
        device_box(Optional[constr(max_length=16000)], optional): Device related information retrieved from TrustWidget.
...
class Code2(
    ...
    Attributes:
        ...
        FORBIDDEN(Any): --
        NOT_FOUND(Any): --
        ACCOUNT_UPDATE_NOT_FOUND(Any): --
        TOO_MANY_REQUESTS(Any): --
        INTERNAL_SERVER_ERROR(Any): --
        BAD_GATEWAY(Any): --
        RETRYABLE_ACCOUNT_SCREEN_FAILURE(Any): --
        RETRYABLE_ACCOUNT_UPDATE_FAILURE(Any): --
        GATEWAY_TIMEOUT(Any): --
        BAD_REQUEST(Any): --
...
class AccountTakeoverError(
    ...
    Attributes:
        code(Code2): Snake cased all caps error code interpreted from the HTTP status code that can programmatically be acted upon.
        message(str): A human-readable explanation of the error, specific to this error occurrence.
...
class AccountTakeoverServiceUnavailableError(
    ...
    r"""pydantic model AccountTakeoverServiceUnavailableError: Indicates that the API is either down for maintenance or overloaded and cannot fulfill the request at the current time. This is a temporary error and retrying the same request after a certain delay could eventually result in success.
    There will be a Retry-After HTTP header in API response specifying how long to wait to retry the request. If there is no Retry-After HTTP header then retry can happen immediately. If the error persists after retrying with delay, please reach out to <support team>."
...
class AccountTakeoverBadRequestError(
    ...
    Attributes:
        causes(Optional[list[Cause1]], optional): --
...
class Type1(
    ...
    Attributes:
        MULTI_FACTOR_AUTHENTICATION_UPDATE(Any): --
        REMEDIATION_UPDATE(Any): --
...
class AccountUpdateRequestGeneric(
    ...
    * `REMEDIATION_UPDATE`: `RemediationUpdate`
    ...
    Attributes:
        type(Type1): The categorized type of account update event from the Partner's system.
        risk_id(constr(max_length=200)): The `risk_id` provided by Expedia's Fraud Prevention Service in the `AccountScreenResponse`.
...
class RemediationUpdateAction(
    ...
    Attributes:
        action_name(ActionName): The categorized remediation action initiated by the Partner''s system to a user. Possible values are: - `PASSWORD_RESET` - Applicable if this event is the result of a password reset by the Partner''s system. - `DISABLE_ACCOUNT` - Applicable if this event is the result of disabling an account by the Partner''s system. - `TERMINATE_ALL_SESSIONS` - Applicable if this event is the result of terminating all active user sessions of an account by the Partner''s system.
        status(Status2): The status of the remediation action.   - `SUCCESS` - Applicable if the Partner''s system was successfully able to perform the remediation action.   - `FAILED` - Applicable if the Partner''s system failed to perform the remediation action.
        update_end_date_time(Optional[datetime], optional): The local date and time the remediation action to a user ended in the Partner's system, in ISO-8061 date and time format `yyyy-MM-ddTHH:mm:ss.SSSZ`.
...
class AccountTakeoverSiteInfo(
    ...
    Attributes:
        locale(Optional[constr(regex=r'^([a-z]{2}-[A-Z]{2})$')], optional): The locale of the website a user is accessing, which is separate from the user configured browser locale, in ISO 639-2 language code format and in ISO 3166-1 country code format.
        name(Optional[constr(max_length=200)], optional): Name of the website from which the event is generated.
        brand_name(constr(max_length=200)): The trademark brand name that is displayed to a user on the website.
        placement_name(Optional[PlacementName], optional): The categorized name of the page where a user initiated event is being evaluated. - `LOGIN` - Applicable if the user initiated this account event from a login web page. - `PASSWORD_RESET` - Applicable if the user initiated this account event from a password reset web page.
...
class AccountTakeoverName(
    ...
    Attributes:
        ...
        middle_name(Optional[constr(max_length=200)], optional): Middle name of the person.
        title(Optional[constr(max_length=200)], optional): Title of the person for name (e.g. Mr., Ms. etc).
        suffix(Optional[constr(max_length=50)], optional): Generational designations (e.g. Sr, Jr, III) or values indicate that the individual holds a position, educational degree, accreditation, office, or honor (e.g. PhD, CCNA, OBE).
...
class ChallengeDetail(
    ...
    Attributes:
        displayed_flag(bool): Indicates that there was a challenge initiated by the Partner's system to a user before calling Expedia's Fraud Prevention Service.
        type(Type4): The kind of challenge served by the Partner''s system to a user prior to calling Expedia''s Fraud Prevention Service. - `CAPTCHA` - Applicable if the challenge served by the Partner''s system was a Captcha challenge. - `TWO_FACTOR` - Applicable if the challenge served by the Partner''s system was a two-factor challenge including (Email verification, One Time Password, Okta, etc).
        status(Status3): The status of the challenge served by the Partner''s system to a user before calling Expedia''s Fraud Prevention Service. - `SUCCESS` - Applicable if the user successfully passed the challenge. - `FAILED` - Applicable if the user failed the challenge.
...
class MultiFactorAuthenticationAttempt(
    ...
    Attributes:
        delivery_method(DeliveryMethod): The delivery method of the Multi-Factor Authentication to a user.
        status(Status1): The status of a user''s response to the Multi-Factor Authentication initiated by the Partner''s system to the user.' - `SUCCESS` - Applicable if the user successfully passed the challenge. - `ABANDON` - Applicable if the user did not complete the challenge. - `FAILED` - Applicable if the user failed the challenge.
        reference_id(constr(max_length=200)): The identifier related to a Multi-Factor Authentication attempt by the Partner's system to the Multi-Factor Authentication provider.
        provider_name(constr(max_length=200)): The vendor providing the Multi-Factor Authentication verification.
        attempt_count(float): The number of attempts a user tried for this Multi-Factor Authentication.
        update_start_date_time(Optional[datetime], optional): The local date and time the Multi-Factor Authentication was initiated to a user from the Partner's system, in ISO-8061 date and time format `yyyy-MM-ddTHH:mm:ss.SSSZ`.
        update_end_date_time(Optional[datetime], optional): The local date and time the Multi-Factor Authentication to a user ended in the Partner's system, in ISO-8061 date and time format `yyyy-MM-ddTHH:mm:ss.SSSZ`.
        telephone(Optional[Telephone], optional): --
        email_address(Optional[EmailStr], optional): Email address used for the Multi-Factor Authentication by a user.
...
class RemediationUpdate(
    ...
    Attributes:
        ...
        type(Literal['REMEDIATION_UPDATE']): The categorized type of account update event from the Partner's system.
...
class AccountScreenResponse(
    ...
    Attributes:
        risk_id(Optional[constr(max_length=200)], optional): Unique identifier assigned to the transaction by Expedia's Fraud Prevention Service.
        decision(Optional[AccountTakeoverFraudDecision], optional): --
...
class AccountTakeoverCustomerAccount(
    ...
    Attributes:
        user_id(constr(max_length=200)): Unique account identifier provided by the Partner's Identity Provider/System assigned to the account owner by the partner. `user_id` is specific to the Partner's namespace. Used to track repeat account activity by the same user.
        account_type(AccountType1): Identifies the account type of a user''s account. Possible values are: - `INDIVIDUAL` - Applicable if this account is for an individual traveler. - `BUSINESS` - Applicable if this account is for a business or organization account used by suppliers or Partners.
        account_role(Optional[AccountRole], optional): Identifies the account role and associated permissions of a user''s account. Possible values are: - `USER`: Basic account with no special privileges. - `MANAGER`: Account with additional privileges, such as the ability to make bookings for others. - `ADMIN`: Account with higher privileges than a manager, including the ability to grant manager access to other users.
        name(Optional[AccountTakeoverName], optional): --
        username(constr(max_length=200)): Username of the account.
        email_address(EmailStr): Email address for the account owner.
        telephones(Optional[list[Telephone]], optional): --
        address(Optional[Address], optional): --
        registered_time(datetime): The local date and time that the customer first registered on the Partner's site, in ISO-8061 date and time format `yyyy-MM-ddTHH:mm:ss.SSSZ`.
        active_flag(bool): Indicator for if this account is an active account or not.
        loyalty_member_id(Optional[constr(max_length=200)], optional): Unique loyalty identifier for a user.
...
class CurrentUserSession(
    ...
    Attributes:
        session_id(Optional[constr(max_length=200)], optional): Unique identifier for a user's session on their device
        start_date_time(Optional[datetime], optional): The local date and time a user's session started, in ISO-8061 date and time format `yyyy-MM-ddTHH:mm:ss.SSSZ`.
        challenge_detail(Optional[ChallengeDetail], optional): --
...
class MultiFactorAuthenticationUpdate(
    ...
    Attributes:
        ...
        type(Literal['MULTI_FACTOR_AUTHENTICATION_UPDATE']): The categorized type of account update event from the Partner's system.
...
class AccountTakeoverTransactionDetailsGeneric(
    ...
    * `LOGIN`: `LoginTransactionDetails`
    ...
    Attributes:
        type(Type3): The categorized type of account event related to a user's action.
        transaction_date_time(datetime): The local date and time the transaction occured in the Partner's system, in ISO-8061 date and time format `yyyy-MM-ddTHH:mm:ss.SSSZ`.
        transaction_id(constr(max_length=200)): Unique identifier to identify a transaction attempt in the Partner's system.
        current_user_session(Optional[CurrentUserSession], optional): --
...
AccountTakeoverServiceUnavailableError.update_forward_refs()
...
class ExpediaGroupAccountTakeoverServiceUnavailableErrorException(ExpediaGroupApiException):
    r"""Exception wrapping a AccountTakeoverServiceUnavailableError object."""
    pass
...
@dataclass
class AccountTakeoverServiceUnavailableErrorDeserializationContract:
    exception: type = ExpediaGroupAccountTakeoverServiceUnavailableErrorException
    model: type = AccountTakeoverServiceUnavailableError
