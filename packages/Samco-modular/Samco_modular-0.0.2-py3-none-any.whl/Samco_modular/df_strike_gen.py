...
def df_strikeprice_func(token, iDict, Scripmaster):
    ...
    ######### Codes for fetching Banknifty live value ##########
    ...
    banknifty_ref=float(r['spotPrice'])
    banknifty_spot=banknifty_ref
    banknifty_ref= round(banknifty_ref/strike_step,0)*strike_step

    ######### Codes for strikes generation for Banknifty strikes into df_strikeprice ##########
    ...
    for i in range(num_strikes):   ## finding all the strikes referring ATM calculated as above
        df_strikeprice["Strike_Selection"][i]=banknifty_ref - strike_step*((num_strikes-1)/2-i)
...
#     print("Time after loop=",code_completion-mid2)
    ...
    print("\n"+underlying_print+" LTP= ",banknifty_spot)
