        ...
        if cz_url is not None:
            ...
            if service is None:
                ...
                if protocol is None or protocol == 'https':
                    ...
                    self.service = self.service + ':443' if not self.service.endswith(':443') else self.service
                elif protocol == 'http':
                    self.service = 'http://' + service if not service.startswith('http://') else service
                    self.service = self.service + ':80' if not self.service.endswith(':80') else self.service
                else:
        ...
        if 'protocol' in query:
            ...
            if self.protocol == 'http':
                if not url.host[length:].endswith(':80'):
                    self.service = 'http://' + url.host[length:] + ':80'
                else:
                    self.service = 'http://' + url.host[length:]
            else:
                raise ValueError('protocol must be http or https. Other protocols are not supported yet.')
        self.protocol = 'https'
        self.service = 'https://' + url.host[length:] + ':443' if not url.host[length:].endswith(
            ':443') else 'https://' + url.host[length:]
