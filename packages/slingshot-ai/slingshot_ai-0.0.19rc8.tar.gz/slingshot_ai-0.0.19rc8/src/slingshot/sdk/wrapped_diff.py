...
def _key_path_to_path_nodes(
    ...
    for key_idx, current_key in enumerate(keys):
        ...
        if isinstance(current_key, str):
            ...
            # If the diff type is not one of these, the index order for list items may have changed, so we get the
            # index of the item with the item, and use that to get the item from the current object
            if diff_type not in {'values_changed', 'type_changes'}:
