...
def _plot_convergence_1(
    ...
    means_U = numpy.zeros(
            (num_ensembles, missing_indices_in_ocean_inside_hull.shape[0]),
            dtype=float)
    means_V = numpy.zeros(
            (num_ensembles, missing_indices_in_ocean_inside_hull.shape[0]),
            dtype=float)
    ...
    x = numpy.arange(1, num_ensembles)
    ...
    ax.set_xlabel(r'Number of Ensembles $s$')
    ax.set_ylabel('Max Mean Difference')
    ax.set_title('Convergence of Mean of Ensembles')

    fig.set_tight_layout(True)

    # Save plot
    if save:
        filename = 'ensembles_convergence_1'
...
def _plot_convergence_wrt_mean(
    ...
    This function plots convergence of the difference of the ensembles and the
    mean of ensembles ONLY on the unknown domain where the missing data were
    inpainted. Hence, the mean and central ensemble are not the same.

    The cumulative mean of the ensembles convergences to the mean of the total
    number of the ensembles, however, they are not necessarily converge to the
    central ensemble.
    """
    ...
    means_U = numpy.ma.masked_all((num_ensembles, indices.shape[0]),
                                  dtype=float)
    means_V = numpy.ma.masked_all((num_ensembles, indices.shape[0]),
                                  dtype=float)
    ...
    # Save plot
    if save:
        filename = 'ensembles_convergence_wrt_mean'
        save_plot(filename, transparent_background=True, pdf=True,
                  bbox_extra_artists=None, verbose=verbose)
...
def _plot_convergence_wrt_central(
        ...
        verbose=True):
    """
    Plots convergence of ensembles w.r.t the central ensemble (not the mean).
    ...
    U = numpy.zeros((num_ensembles, indices.shape[0]), dtype=float)
    V = numpy.zeros((num_ensembles, indices.shape[0]), dtype=float)
    ...
    # Z score of data
    U_z_score = numpy.zeros((num_ensembles, indices.shape[0]), dtype=float)
    V_z_score = numpy.zeros((num_ensembles, indices.shape[0]), dtype=float)
    for ensemble in range(num_ensembles):
        U_z_score[ensemble, :] = (U[ensemble, :] - U_central) / U_std
        V_z_score[ensemble, :] = (V[ensemble, :] - V_central) / V_std

    # Cumulative std of the z-scores
    U_cum_mean = numpy.zeros((num_ensembles, indices.shape[0]), dtype=float)
    V_cum_mean = numpy.zeros((num_ensembles, indices.shape[0]), dtype=float)
    for ensemble in range(num_ensembles):
        U_cum_mean[ensemble, :] = numpy.mean(U_z_score[:ensemble+1, :], axis=0)
        V_cum_mean[ensemble, :] = numpy.mean(V_z_score[:ensemble+1, :], axis=0)

    # U_cum_mean_std = numpy.zeros((num_ensembles-1, indices.shape[0]),
    #                              dtype=float)
    # V_cum_mean_std = numpy.zeros((num_ensembles-1, indices.shape[0]),
    #                              dtype=float)
    # for ensemble in range(1, num_ensembles):
    ...
    # Save plot
    if save:
        filename = 'ensembles_convergence_wrt_central'
        save_plot(filename, transparent_background=True, pdf=True,
                  bbox_extra_artists=None, verbose=verbose)
