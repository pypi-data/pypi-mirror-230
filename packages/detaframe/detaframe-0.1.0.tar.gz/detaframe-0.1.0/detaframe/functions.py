...
def paginated(data: list, size: int = 20) -> list[list]:
    
    def slice_list(content: list) -> tuple[list, list]:
        if len(content) <= size:
            return content, []
        else:
    ...
    while len(data_copy) > 0:
        g, data_copy = slice_list(data_copy)
        result.append(g)
...
def filter_not_none(data: Sequence) -> list:
    return [i for i in data if i not in [None, '']]
