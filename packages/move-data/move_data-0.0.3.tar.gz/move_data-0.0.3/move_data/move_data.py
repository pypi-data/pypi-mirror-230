...
class sharepoint:
  ...
  def upload_file_put(self,upload_url,modified_data,content_type='application/octet-stream'):
    headers = {
        'Authorization': f'Bearer {self.access_token}',
  ...
  def upload_file_patch(self,upload_url,modified_data,content_type='application/octet-stream'):
    headers = {
        'Authorization': f'Bearer {self.access_token}',
        'Content-Type': content_type,
        'Content-Length': str(len(modified_data)),
    }
    upload_response = requests.patch(upload_url, data=modified_data, headers=headers)
    if upload_response.status_code == 200:
      print("File uploaded successfully.")
    else:
      print(f"Failed to upload file. Status code: {upload_response.status_code}")
