...
class TestHolaClient(unittest.TestCase):
    ...
    def sys_info(self):
        client = pyholaclient.HolaClient('http://localhost', '123')
        sysinfo = client.sys_info()
        self.assertEqual(sysinfo.total_ram, 3072)
        print('TestHolaClient.test_sys_info Total RAM: OK')
        self.assertEqual(sysinfo.total_disk, 10240)
        print('TestHolaClient.test_sys_info Total Disk: OK')
        self.assertEqual(sysinfo.used_ram, 0)
        print('TestHolaClient.test_sys_info Used RAM: OK')
        self.assertEqual(sysinfo.used_disk, 0)
        print('TestHolaClient.test_sys_info Used Disk: OK')

    def user_resource_set(self):
        client = pyholaclient.HolaClient('http://localhost', '123')
        status = client.user_resource_set(1111, 1024, 1024, 1, 1, 1, 1, 1, 1)
        self.assertEqual(status, True)
        print('TestHolaClient.test_user_resource_set: OK')
