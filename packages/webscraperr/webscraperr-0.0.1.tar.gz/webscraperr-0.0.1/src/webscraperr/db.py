...
class WebScraperDBSqlite:
    ...
    def save_url_and_info(self, url: str, info: str):
        self.cursor.execute(f"INSERT OR IGNORE INTO {self.table}(URL, INFO) VALUES(?, ?)", [url, info])

    def save_url_and_info_many(self, datas: list):
        self.cursor.executemany(f"INSERT OR IGNORE INTO {self.table}(URL, INFO) VALUES(?, ?)", datas)
    ...
    def set_info_by_url(self, url: str, info: str):
        self.cursor.execute(f"UPDATE {self.table} SET INFO=? WHERE URL=?", [info, url])
...
class WebScraperDBMySQL:
    ...
    def save_url(self, url: str):
        self.cursor.execute(f"INSERT INTO {self.config['TABLE']}(URL) VALUES(%s)", (url,))
    
    def save_urls(self, urls: str):
        self.cursor.executemany(f"INSERT IGNORE INTO {self.config['TABLE']}(URL) VALUES(%s)", urls)
    
    def save_url_and_info(self, url: str, info: str):
        self.cursor.execute(f"INSERT INTO {self.config['TABLE']}(URL, INFO) VALUES(%s, %s)", (url, info))
