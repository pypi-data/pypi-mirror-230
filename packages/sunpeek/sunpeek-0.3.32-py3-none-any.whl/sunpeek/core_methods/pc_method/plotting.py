...
class PCPlots:
    ...
    def plot_collector_overview(self) -> Optional[List[matplotlib.figure.Figure]]:
        ...
        for c in unique_collectors:
            tbl_rows.append({
                ...
                'kd': f'{c.kd.to("").m:.2f}',
                'f_prime': self.na_str if c.f_prime is None else f'{c.f_prime.to("").m:.2f}',
            })
    ...
    def plot_data_overview(self) -> Optional[List[matplotlib.figure.Figure]]:
        ...
        fig_list = []
        for ao in self.pc_output.array_output:
            has_tp = not ao.array.tp.data.isna().all()
            tbl_rows = []
            for i in range(self.pc_output.plant_output.n_intervals):
                tbl_rows.append({
                    ...
                    'rd_gti': f'{ao.data.rd_gti[i].to("W m**-2").m:0.0f}' if ao.data.rd_gti is not None else self.na_str,
                    'rd_bti': f'{ao.data.rd_bti[i].to("W m**-2").m:0.0f}' if ao.data.rd_bti is not None else self.na_str,
                    'rd_dti': f'{ao.data.rd_dti[i].to("W m**-2").m:0.0f}' if ao.data.rd_dti is not None else self.na_str,
                    've_wind': f'{ao.data.ve_wind[i].to("m s**-1").m:0.1f}' if ao.data.ve_wind is not None else self.na_str,
                    'tp_measured': f'{ao.tp_sp_measured[i].to("W m**-2").m:0.0f}' if has_tp else self.na_str,
                    'tp_estimated_safety': f'{ao.tp_sp_estimated_safety[i].to("W m**-2").m:0.0f}',
                    'tp_ratio': f'{(ao.tp_sp_measured[i] / ao.tp_sp_estimated_safety[i]).to("percent").m:0.1f}' if has_tp else self.na_str,
                })
    ...
    def _plot_single_interval(self,
        ...
        p = a.plant
        has_tp = not a.tp.data.isna().all()
        has_vf = not a.vf.data.isna().all()
