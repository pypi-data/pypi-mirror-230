...
class ArtifactoryStructDownloader(object):
    ...
    def download_artifactory_directory(self, url, save_path):
        ...
        # 如果是超大文件会卡住，因此需要判断下head是不是html，如果不是html 直接跳过,避免302
        rep_header = requests.head(url, headers=self.header)
        if rep_header.status_code == 302:
            logger.debug(f"find 302, visit it and for again")
            url = rep_header.url+"/"
            rep_header = requests.head(url, headers=self.header)
        content_type = rep_header.headers.get('Content-Type', '')
        if 'html' not in content_type:
            logger.debug(f"find file {url}, directly download it.")
            self.download_file(url, os.path.join(save_path, os.path.basename(url)))
            return
        ...
        if rsp_code == 200:
            ...
            logger.warning(f"[HINT] if you find the token is valid, use `efjdown -c` to clean token and reset.")
        else:
    ...
    try:
        ...
        if args.clean and not (args.url or args.save_path or args.token):
            ...
            if _token:
                ...
                efjdown.header = {'Authorization': _token}
            else:
                if not os.path.exists(os.path.join(str(Path.home()), '.efgdown_config.ini')):
                    ...
                    token_ = input("Please enter your public account token:")
                    if 'Bearer' not in token_:
                        token = f"Bearer {token_}"
                    ArtifactoryStructDownloader.save_token(token)
                    try:
                        efjdown.header = {'Authorization': f'{efjdown.load_token()}'}
                    except Exception as e:
                        logger.error(f"Maybe save config or read config is fail, please check error:{e}")
                else:
                    token = ArtifactoryStructDownloader.load_token()
                    efjdown.header = {'Authorization': f'{token}'}
