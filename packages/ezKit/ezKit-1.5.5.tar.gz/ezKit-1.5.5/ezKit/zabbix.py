    ...
    def create_item(
        ...
        创建监控项目
            ...
            item: 
                ...
                key
                type (default: 7)
                value_type (default: 3)

            trigger:

                name
                priority
                expression (必须要有 {}, 用于定义HOST)
        """
        """
        参考文档:
        ...
        try:
            ...
            match True:
                case True if not utils.v_true(ips, list):
                    ...
                    return False
                case True if not utils.v_true(item, dict):
                    logger.error('ERROR!! item is not dict')
                    return False
                case _:
                    next
            ...
            for ip in ips:
                ...
                if utils.v_true(trigger, dict):
                    ...
                    #   'last(/{}/system.uptime)<10m'.format('DIYCL-110-30')
                    # 所以, 传递参数的时候, expression 中就必须要有 {} 符号, 用于定义 HOST
                    #   'last(/{}/system.uptime)<10m'
                    # 如果传递参数的时候使用了 f-strings, 如果要保留 {}, 再套一层 {} 即可
                    #   f'last(/{{}}/system.uptime)<10m'
                    params = [
                        {
                            'description': trigger.get('name'),
                            'priority': trigger.get('priority', '2'),
                            'expression': f"{trigger.get('expression')}".format(host),
                            'manual_close': '1'
                        }
                    ]
