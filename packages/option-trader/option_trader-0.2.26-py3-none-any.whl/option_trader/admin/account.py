...
class account():
    ...
    def expire_position_summary(self, srow):
        ...
        sql = """update position_summary set exp_stock_price=?, exp_price=?, last_price=?, pl=?, gain=?, status=?, stop_date=?, stop_reason=?, stop_price=?,last_quote_date=?, close_cash=?, close_margin=?, cash_position=?, margin_postion=?, where uuid==?"""        
    ...
    def get_stock_open_shares(self, symbol):
        ...
        pos = pos[(pos[position.SYMBOL]==symbol) & (pos[position.STATUS]==asset.OPENED)]
        if pos.shape[0] > 0:
    ...
    def create_daily_account_summary(self): 
        ...
        opt['amount'] = opt.apply(lambda x: x[position_summary.LAST_PRICE] * x[position_summary.QUANTITY], axis = 1)  
        opt['amount'] = opt.apply(lambda x: x['amount'] * 100 if x[position_summary.STRATEGY] != st.UNPAIRED  else x['amount'], axis = 1)  
        opt['amount'] = opt.apply(lambda x: -1 * x['amount'] if x[position_summary.CREDIT] == 'True' else x['amount'], axis = 1) 
        asset_value = opt[opt[position_summary.STATUS]==asset.OPENED]['amount'].sum()
