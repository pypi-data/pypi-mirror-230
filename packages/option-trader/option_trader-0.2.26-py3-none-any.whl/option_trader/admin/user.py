...
class user():  
    ...
    def send_account_status_report(self):
        ...
        history_summary = history_df[['account name', 'account value', 'asset value', 'cash position', 'margin position', 'Profit/Loss' ]]
        ...
        if stock_position.shape[0] > 0:            
            stock_summary = stock_position[['account name', position_summary.SYMBOL, position_summary.QUANTITY, position_summary.GAIN]]
            mail_body += '<h3>Stock Positions</h3>'+stock_summary.to_html(index=False) 

        option_position = position_df[position_df[position_summary.STRATEGY] != st.UNPAIRED] 
        if option_position.shape[0] > 0:
            option_summary = option_position[['account name',position_summary.SYMBOL, position_summary.STRATEGY, position_summary.EXP_DATE, position_summary.GAIN, position_summary.LAST_WIN_PROB]]
            option_summary.sort_values(position_summary.EXP_DATE, inplace=True)
            mail_body += '<h3>Option Positions</h3>'+ option_summary.to_html(index=False)
