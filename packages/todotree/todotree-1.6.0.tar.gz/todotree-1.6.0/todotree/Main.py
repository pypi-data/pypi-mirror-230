...
def add(task: Tuple):
    ...
    try:
        ...
        config.git.commit_and_push("add")
...
def append(task_nr: int, append_string: Tuple[str]):
    ...
    config.git.commit_and_push("append")
...
def cd():
    ...
    config.console.verbose("The location to the data folder is: ")
...
def edit():
    ...
    config.git.commit_and_push("edit")
...
def filter_list(search_term):
    ...
    config.console.info(f"Todos for term '{search_term}'")
...
def revive(done_number):
    ...
    try:
        ...
        MainUtils.handle_done_file_not_found(e, config)
        exit(1)
    config.git.commit_and_push("revive")
...
def schedule(task_number: int, new_date: Tuple[str]):
    ...
    if not date_pattern.match(date):
        config.console.verbose(f"Attempt to parse {date} with the `date` program.")
    ...
    config.git.commit_and_push("schedule")
