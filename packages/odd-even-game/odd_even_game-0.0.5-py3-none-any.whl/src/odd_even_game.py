...
def toss():
    ...
    if (toss_comp_imp + toss_inp) % 2 == 0:
        if user_choice == "even" or user_choice == "eve":
            print("Well, ", toss_inp, " + ", toss_comp_imp, " is ", (toss_comp_imp + toss_inp),
                  " Which is even, So You win :)")
            player_toss_win()
        else:
            print("Well, ", toss_inp, " + ", toss_comp_imp, " is ", (toss_comp_imp + toss_inp),
                  " Which is even, So I win :)")
            computer_toss_win()
    else:
        if user_choice == "odd":
            print("Well, ", toss_inp, " + ", toss_comp_imp, " is ", (toss_comp_imp + toss_inp),
                  " Which is odd, So You win :)")
            player_toss_win()
        else:
            print("Well, ", toss_inp, " + ", toss_comp_imp, " is ", (toss_comp_imp + toss_inp),
                  " Which is odd, So I win :)")
            computer_toss_win()
        ...
        while not out:
            try:
                ...
                if inp <= 0 or inp >= 11:
                    ...
                    if inp - cmp_imp == 1 or cmp_imp - inp == 1 or (cmp_imp == 1 and inp == 10) or (
                            inp == 1 and cmp_imp == 10):
                        print("OUT!")
                        out = True
                    elif inp == cmp_imp:
                        print("Squared!!!")
