            ...
            if mode == 'r':
                try:
                    ...
                    if "Bad magic number for central directory" in str(error):
                        print('Handling Bad magic number for central directory bug.')
                        self.fp.seek(0)
                        data = self.fp.read()[:-22]
                        self.fp.close()
                        self.fp = io.BytesIO(data[:data.rfind(b'PK\005\006') + 22])
                        self._RealGetContents()
                    else:
                        raise
        ...
        while total < size_cd:
            ...
            if flags & 0x800:
                ...
                coding_name = api.get_cn_charset(
                    ...
                    source_type="data",
                    mode=1,
                    special_encodings=["gb18030", "utf-16", "utf-32"]
                )
                if not coding_name:
                    coding_name = api.from_data(data=filename, mode=2)

                if coding_name:
                    filename = api.convert_encoding(
                        source_data=filename,
    ...
    def open(self, name, mode="r", pwd=None, *, force_zip64=False):
        ...
        try:
            ...
            if fheader[_FH_GENERAL_PURPOSE_FLAG_BITS] & 0x800:
                ...
                if not coding_name:
                    coding_name = api.from_data(data=fname, mode=2)

                if coding_name:
                    fname_str = api.convert_encoding(
                        source_data=fname,
                        source_encoding=coding_name,
                        target_encoding="utf-8",
                    )
