...
class BaleCloud:
    ...
    def sendPhoto(self, photoPathOrName : str = None, chatID : str = None, caption : str = '', messageID : str = None):
        ...
        if (photoPathOrName == None or chatID == None):
            ...
            while 1:
                try:
                    req = requests.post(f'https://tapi.bale.ai/bot{self.token}/sendPhoto?photo={photoPathOrName}&chat_id={chatID}&caption={caption}&reply_to_message_id={messageID}', headers=headers, proxies=self.proxy)
                    return dict(req.json())
                    break
                except:continue
