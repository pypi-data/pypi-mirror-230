...
def set_freq_hz(freq: float) -> SetFreq:
    ...
    q1asm_freq = int(
        NCO_MIN_INT_FREQ
        + (NCO_MAX_INT_FREQ - NCO_MIN_INT_FREQ) / (NCO_MAX_HZ_FREQ - NCO_MIN_HZ_FREQ) * (freq - NCO_MIN_HZ_FREQ)
    )
...
def set_awg_gain_relative(gain_0: float, gain_1: float) -> SetAwgGain:
    ...
    Args:
        gain_0 (float): Gain from 0.0 to 1.0 for path 0.
        gain_1 (float): Gain from 0.0 to 1.0 for path 1.

    Returns:
        SetAwgGain: SetAwgGain Instruction.
    """
    if not (0.0 <= gain_0 <= 1.0) or not (0.0 <= gain_1 <= 1.0):
