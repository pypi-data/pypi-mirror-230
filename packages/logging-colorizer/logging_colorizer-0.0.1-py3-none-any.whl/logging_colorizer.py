...
# "from alagha import log" work
class Log:
    def __init__(self, lvl=logging.DEBUG, format=None):
        self._lvl = lvl
        if not format:#"black", "red", "green", "yellow", "blue", "purple", "cyan", "white"
            self.format = "%(black)s TIME: %(asctime)s %(blue)s|%(black)s IP: %(clientip)s %(blue)s|%(black)s %(purple)s CODE: %(Code)s %(blue)s|%(log_color)s %(message)s %(reset)s"
    ...
    def critical(self, message, extra_plus_ali, *args, **kwargs):
        for line in str(message).splitlines():
            self.logger.critical(line,
                                 extra={"styledname": self.theme[logging.CRITICAL]} | extra_plus_ali,
                                 *args, **kwargs)
    crit = c = fatal = critical
    def error(self, message, extra_plus_ali, *args, **kwargs):
        for line in str(message).splitlines():
            self.logger.error(line,
                              extra={"styledname": self.theme[logging.ERROR]} | extra_plus_ali,
                              *args, **kwargs)
    err = e = error
    def warn(self, message, extra_plus_ali, *args, **kwargs):
        for line in str(message).splitlines():
            self.logger.warn(line,
                             extra={"styledname": self.theme[logging.WARNING]} | extra_plus_ali,
                             *args, **kwargs)
    warning = w = warn
    def info(self, message, extra_plus_ali, *args, **kwargs):
        for line in str(message).splitlines():
            self.logger.info(line,
                             extra={"styledname": self.theme[logging.INFO]} | extra_plus_ali,
                             *args, **kwargs)
    inf = nfo = i = info
    def debug(self, message, extra_plus_ali, *args, **kwargs):
        for line in str(message).splitlines():
            self.logger.debug(line,
                              extra={"styledname": self.theme[logging.DEBUG]} | extra_plus_ali,
                              *args, **kwargs)
    dbg = d = debug
