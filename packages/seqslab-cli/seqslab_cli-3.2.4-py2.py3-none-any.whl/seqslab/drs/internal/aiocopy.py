...
def total_blocks(files: [URL], chunk_size: int) -> int:
    blocks = 0
    for file in files:
        block = os.stat(str(file)).st_size / chunk_size
        if os.stat(str(file)).st_size % chunk_size == 0:
            blocks += int(block)
        else:
            blocks += (int(block)+1)
    return blocks
...
async def file_to_blob(
    ...
    async with get_factory().load_storage(kwargs.get("workspace")) as store:
        ...
        checksum_bar, optargs['progress_bar'] = ProgressBarObject(total_tasks=len(files), log=True), ProgressBarObject(
            total_tasks=blocks, log=True)
            ...
            for r in resp:
                try:
                    ...
                    if isinstance(r, dict):
                        ...
                        # if optargs.get('progress_bar'):
                        #     optargs['progress_bar'].print(f"file_to_blob: Upload process done.")
