...
@api.command()
@click.argument('scan', required=True, type=int)
@pass_rebotics_context
def get_voting_data(ctx, scan: int):
    """
    Usage:
        retailer -r epsilon get-voting-data 112233
    """
    ctx.verbose_log('Exporting scan/realogram products data...')
    try:
        response = ctx.provider.get_voting_data(scan)
    except ProviderHTTPClientException as exc:
