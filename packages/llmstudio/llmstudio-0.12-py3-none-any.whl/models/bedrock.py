...
class BedrockClient(LLMVendorClient):
    ...
    class BedrockModel(LLMModel):
        ...
        def __init__(
            ...
            self.api_key = (
                api_key
                or os.environ.get("LS_BEDROCK_KEY")
                or self._raise_api_error("api_key", "LS_BEDROCK_KEY")
            )
            self.api_secret = (
                api_secret
                or os.environ.get("LS_BEDROCK_SECRET")
                or self._raise_api_error("api_secret", "LS_BEDROCK_SECRET")
            )
            self.api_region = (
                api_secret
                or os.environ.get("LS_BEDROCK_REGION")
                or self._raise_api_error("api_region", "LS_BEDROCK_REGION")
            )
