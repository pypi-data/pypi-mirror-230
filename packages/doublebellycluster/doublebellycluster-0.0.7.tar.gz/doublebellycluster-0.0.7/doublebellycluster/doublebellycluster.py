...
class Doubleclustering:
    ...
    def do_color_cluster(self):
        ...
        dd = agglom_clustering(pair_mat_0, self.cut_edge_disjoint)
    ...
    def do_continis(self, xy,
        ...
        self.d3plot(xy)
        self.fs['col'] = 0
        d3_color_map(self.fs,self.dir+'/3d begin')
    
        self.do_fs_beer()
        self.fs_regress_allneed()

        print(self.fs_to_plot)
...
if __name__ == '__main__':
    ...
    '''
    xy = pd.read_excel('C:/Users/User/Desktop/emitters500e -msk.xlsx')

    xy = np.array(xy['point'].map(lambda x: x[:-1].split(' ')).to_list())
    xy = [list(i) for i in xy if i[0]!='' and i[1]!='']
    xy=pd.DataFrame(xy).astype(float)
    xy.columns = ['yy','xx']
    xy['xy'] = np.sin(xy['xx'])
    '''

    xy = pd.read_csv('C:/Users/User/Desktop/accidents.csv',sep=',').dropna()
    xy = xy.loc[xy['Local_Authority_(District)']==8]
    xy= xy[['lon','lat']]
    xy.columns = ['yy','xx']
