...
class MySpeakerDiarization(SpeakerDiarizationMixin, Pipeline):
    ...
    def __init__(
        ...
        segmentation_type='pyannote'
    ):
        super().__init__()
        
        self.whisper_model = whisper.load_model("base.en") ### added whisper model
    ...
    def get_whisper_segments(self,file): ### get whisper segmentations
        
        asr  = self.whisper_model.transcribe(file)
    ...
    def get_segments_list(self,file):  ### get list of segments (from whisper)
        segments_list= self.get_whisper_segments(file)
        #segments_list= self.get_pipechunks(file)
        return segments_list
        
    
    def get_segmentations_mod(self,file,hook=None) -> SlidingWindowFeature: ### in this function, i apply the whisper segments to get the ndarray
        link=str(file['audio'])
        segments_list=self.get_segments_list(link)
        #segments: SlidingWindowFeature=self._segmentation.crop(link,segments_list,duration=2.0)
        #segmentations: SlidingWindowFeature = self._segmentation(file, hook=hook)
        if self.segmentation_type=='whisper':
            segmentations= self._segmentation.crop(link,segments_list,duration=1.0) ### inference crop function
        else:
            segmentations: SlidingWindowFeature = self._segmentation(file, hook=hook) ###
        return segmentations
     
    def get_segmentations_arr(self,path,segments):
        return self._segmentation.crop(path,segments,duration=1.0)
    ...
    def apply(
        ...
        segmentations = self.get_segmentations_mod(file, hook=hook) ### used the modified get_segmentation function here
