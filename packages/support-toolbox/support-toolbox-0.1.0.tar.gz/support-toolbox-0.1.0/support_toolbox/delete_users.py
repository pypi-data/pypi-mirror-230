...
# Get the absolute path to the 'tokens.ini' file within the package directory
package_dir = os.path.dirname(os.path.abspath(__file__))
tokens_file_path = os.path.join(package_dir, 'tokens.ini')

# Initialize the configparser
config = configparser.ConfigParser()

# Read the tokens configuration file
...
admin_token = config['delete_users']['ADMIN_TOKEN']
...
def delete_agent_id(agent_id, email):
    header = {
        'Content-Type': 'application/json',
...
def run():
    ...
    for email in emails_to_delete:
        ...
        agent_id = get_agent_id(email)

        while True:
            selection = input("Would you like to delete this user? y/n: ")

            if selection == 'y':
                delete_agent_id(agent_id, email)
                break
