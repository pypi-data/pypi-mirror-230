...
class RateLimitException(Exception):
    def __init__(self, rate_id: str, rate_limit: int, rate: int):
        super().__init__(
            f"Usage rate :: {rate} exceeds rate_limit :: {rate_limit} with rate_id :: {rate_id}."  # noqa
        )
...
class ValidateRateLimit:
    ...
    def validate_user_rate(self, user_id: str) -> None:
        ...
        if (
            approx_user_rate is not None
            and approx_user_rate >= self.rate_limit
        ):
            return
        raise RateLimitException(
            self.rate_id, self.rate_limit, approx_user_rate or -1
        )
