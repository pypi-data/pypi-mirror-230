...
class RedundancySolverTest(unittest.TestCase):
    ...
    def test_merge_redundant_candidates_by_name(self):
        candidate_list=[Candidate(Name="joe biden", Skills=["skill_{}".format(x)]) for x in range(3)]
        result_candidate_list=merge_redundant_candidates(candidate_list, LSA)
        self.assertEqual(len(result_candidate_list),1)
        final_candidate=result_candidate_list[0]
        self.assertEqual(len(final_candidate.Skills),3)
    ...
    def test_merge_some_redundant_candidates_by_name(self):
        candidate_list=[Candidate(Name="joe biden") for _x in range(3)]+[Candidate("donald trump") for _y in range(2)]
        result_candidate_list=merge_redundant_candidates(candidate_list, LSA)
