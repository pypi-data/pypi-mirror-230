...
class VllmBase(LLM):
    ...
    async def _acall(self, prompt: str, stop: List[str] = None, run_manager: Optional[langchain.callbacks.manager.AsyncCallbackManagerForLLMRun] = None,):
    ...
    def _call(self, prompt: str, stop: List[str] = None, run_manager: Optional[langchain.callbacks.manager.CallbackManagerForLLMRun] = None,):
        if stop is not None:
            self.stop = stop
        query = self.make_context(prompt, self.hist, self.system)
