"""Methodology API function definitions."""
__all__ = ("get_app",)
...
from openrpc import OpenRPC, ParamStructure, RPCServer, Schema, SchemaType
...
from tabella._cache import MethodologyCache
...
cache = MethodologyCache.get_instance()
...
def get_app(  # noqa: PLR0913
    ...
    middleware: Callable[[dict[str, str]], dict[str, Any]] | None = None,
    *,
    ...
    :param api_url: URL API is hosted on.
    :param rpc: RPC server to host.
    :param ws_path: Path to host server over websocket. Will not be
        hosted over websockets if None.
    :param http_path: Path to host server over HTTP. Will not be hosted
        over HTTP if None.
    :param middleware: Function to get RPServer `depends` values from
        request headers.
    :param enable_auth: Enable API key authentication from the docs
        viewer.
    """
        ...
        async def ws_process_rpc(websocket: WebSocket) -> None:
            ...
            try:
                ...
                async def _process_rpc(request: str) -> None:
                    depends = middleware({**websocket.headers}) if middleware else {}
    ...
    if http_path:
        ...
        @app.post(http_path, response_model=ErrorResponse | ResultResponse | None)
        async def http_process_rpc(
            request: Request, rpc_request: RequestType
        ) -> Response:
            """Process RPC request through HTTP server."""
            depends = middleware({**request.headers}) if middleware else {}
...
async def api_pass_through(request: Request) -> FileResponse:
    ...
    client_name = caseswitcher.to_kebab(rpc.info.title) + "-client"
