...
MAIN_STREAM = "_SEAPLANE_ENDPOINT"
...
def create_first_subject(app_id: str, task_id: str) -> str:
    return f"{MAIN_STREAM}.in.{app_id}.{task_id}.*"

def create_output_persisted_subject(app_id: str, task_id: str) -> str:
    meta = "${! meta(\"request_id\")}"
    return f"{MAIN_STREAM}.out.{app_id}.{task_id}.{meta}"
...
def create_carrier_workload_file(
    ...
    input: str = create_subject(app_id, task.id)
    output: Optional[Dict[str, Any]] = None

    if is_first_task:
        input = create_first_subject(app_id, task.id)
    ...
    if len(next_tasks) > 1:
        ...
        if has_to_save_output:
            output = {
                "carrier": {"subject": create_output_persisted_subject(app_id, task.id)},
            }
...
def deploy_task(    
    ...
    workload = create_carrier_workload_file(app.id, task, next_tasks, project_url, is_first_task, has_to_save_output)    
...
def deploy(task_id: Optional[str] = None) -> None:
    ...
    create_stream("_SEAPLANE_ENDPOINT")
    ...
    if task_id is not None and task_id != "entry_point":
        for sm in context.apps:
            for c in sm.tasks:
                if c.id == task_id:
                    deploy_task(sm, c, schema, secrets[:], project_url)
