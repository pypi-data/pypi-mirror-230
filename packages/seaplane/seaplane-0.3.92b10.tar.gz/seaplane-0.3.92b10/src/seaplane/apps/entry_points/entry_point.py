...
def start_task(task_id: str, save_result: bool) -> None:
    ...
    while True:
        try:
            ...
            metadata = message_from_processor.meta

            message = json.loads(body_bytes)

            log.debug(f" Message received: {message}")
            log.debug(f" Metadata received: {metadata}")

            id = metadata["nats_subject"].split(".")[4]  # Getting the request_id from NATS subject            
            metadata["request_id"] = id  # I have to set request_id on metadata, because Flow expect to have request_id
            order  = 0
            ...
            try:
                ...
                task_result = task.process(task_data)

                log.debug(f" Task Result: {message}")
                log.debug(f" Metadata Result: {metadata}")                
                
                output = str(json.dumps(task_result)).encode()
                output_msg = processor._Msg(output, metadata)

                processor.write(output_msg)
                processor.flush()

                if requests_datasource:
                    requests_datasource.save_result(id, order, task_result)
                ...
                output = str(json.dumps(next_message)).encode()
                output_msg = processor._Msg(output, metadata)
            ...
            next_message = { "input": {"error": error_str} }
