...
def verify_aws_memorydb_cluster(  # noqa: PLR0911, PLR0912
    ...
    try:
        ...
        if response["Clusters"][0]["TLSEnabled"]:
            logger.error(
                f"MemoryDB cluster {cloud_resource.memorydb_cluster_config.id} has TLS enabled which is not supported by ray. Please disable TLS for this cluster."
            )
            return False
