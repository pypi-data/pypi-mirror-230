...
from anyscale.utils.cloud_update_utils import (
    detect_drift,
    is_cross_account_iam_role_drifted,
...
class CloudController(BaseController):
    ...
    def update_managed_cloud(  # noqa: PLR0912
        self,
        ...
        cfn_stacks = cfn_client.describe_stacks(StackName=aws_cloudformation_stack_id)[
            "Stacks"
        ]
        ...
        try:
            drifts = detect_drift(
                stack_name=stack_name, region=cloud.region, logger=self.log,
            )
        except ClientError as e:
            raise ClickException(
                f"Failed to detect drifts for cloudformation stack {stack_name}. Error: {e}"
            )
        if is_cross_account_iam_role_drifted(drifts):
            inline_policies = [policy.policy_name for policy in ANYSCALE_IAM_POLICIES]
            customer_role_drift_details_url = f"https://{cloud.region}.console.aws.amazon.com/cloudformation/home?region={cloud.region}#/stacks/drifts/info?stackId={aws_cloudformation_stack_id}&logicalResourceId=customerRole"
            self.log.info(
                f"Drfits detected on the cross account IAM role: {customer_role_drift_details_url}"
            )
            self.log.warning(
                f"Cloud update will override the changes you made in inline policies {inline_policies}. "
                "To avoid this, you can create a new inline policy to include your changes."
            )
            confirm(
                "Proceed to update the cloud?", yes,
            )
        try:
            update_cloudformation_stack(
