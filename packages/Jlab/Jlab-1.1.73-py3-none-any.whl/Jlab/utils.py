...
def update_file(repo, filename, folder_path="", branch=None):
    ...
    try:
        if branch:
            contents = repo.get_contents(os.path.join(folder_path, os.path.basename(filename)), ref=branch)
        else:
            contents = repo.get_contents(os.path.join(folder_path, os.path.basename(filename)))
        sha = contents.sha
    ...
    if sha:
        try:
            if branch:
                repo.update_file(
                    os.path.join(folder_path, os.path.basename(filename)), "committing files", content, sha, branch=branch
                )
            else:
                repo.update_file(os.path.join(folder_path, os.path.basename(filename)), "committing files", content, sha)
        ...
        try:
            if branch:
                repo.create_file(os.path.join(folder_path, os.path.basename(filename)), "committing files", content, branch=branch)
            else:
                repo.create_file(os.path.join(folder_path, os.path.basename(filename)), "committing files", content)
