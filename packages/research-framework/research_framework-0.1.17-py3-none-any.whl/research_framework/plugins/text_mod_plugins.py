...
class FilterRowsByNwords(BaseFilterPlugin):
    def __init__(self, df_headers=["id", "text", "label"], text_col='text', upper_cut=100, lower_cut=10):
        self.evr = None
    ...
    def predict(self, x):
        if type(x) == pd.DataFrame:
            aux = {}
            x.reset_index()
            pbar = tqdm(x.itertuples(index=False))
            pbar.set_description(f"FilterRowsByNwords - {self.get_params(deep=False)}")
            for sentence in pbar:
                try:
                    if len(str(sentence[self.text_col])) > self.lower_cut\
                    and (len(str(sentence[self.text_col])) < self.upper_cut or self.upper_cut < 0):
                        for k,v in sentence._asdict().items():
                            inner = aux.get(k, [])
                            inner.append(v)
                            aux[k] = inner
                except Exception as ex:
                    print(sentence[self.text_col])
                    raise ex
