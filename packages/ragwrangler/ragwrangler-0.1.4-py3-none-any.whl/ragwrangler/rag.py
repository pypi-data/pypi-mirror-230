...
class RAGTask:
    ...
    Attributes:
        ...
        client (weaviate.Client): The Weaviate client to connect to the database.
    """
    def __init__(self, task_prompt_builder: Callable[[str], str], client: weaviate.Client):
        ...
        self.client = client
        db.configure_database(self.client)
    ...
    def get_output(
        ...
        :param overwrite_existing: Whether to overwrite the output if it already exists in Weaviate.
...
def call_chatgpt(prompt: str, model_name: str = "gpt-3.5-turbo") -> str:
    ...
    if openai.api_key is None:
        raise ValueError("OpenAI API key has not been set. Please set it using `set_openai_api_key` function.")
