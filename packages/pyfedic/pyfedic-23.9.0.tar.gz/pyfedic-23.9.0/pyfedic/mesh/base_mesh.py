...
class BaseMesh:
    ...
    def new(cls, nodes, cells_by_type):
        ...
        if len(cells_by_type) == 1:
            ...
            zoi = np.unique(np.hstack(p))
            if zoi.size == 1 and zoi[0] % 1 == 0:
                zoi = int(zoi[0])
                nelems = tuple(len(x) for x in p[:mesh.ndim][::-1])
                x0, y0, z0 = mesh.nodes[0]
                xlims = int(x0+0.5), int(x0+0.5)+nelems[-1]*zoi
                ylims = int(y0+0.5), int(y0+0.5)+nelems[-2]*zoi
                if mesh.ndim == 2:
                    mesh = RegularMesh(xlims, ylims, zoi=zoi)
                else:
                    zlims = int(z0+0.5), int(z0+0.5)+nelems[-3]*zoi
                    mesh = RegularMesh(xlims, ylims, zlims, zoi=zoi)
            return mesh
    ...
    def save(self, filename, U=None, compute_eps=False, point_values=None, cell_values=None, dN=None, **kwargs):
        ...
        sel = ~np.isnan(self.nodes).any(axis=1)
