...
class ProgressTrackingBytesIO(BytesIO):
    def __init__(self, buffer, callback: Optional[Callable[[int], Any]]):
        super().__init__(buffer)
        self.callback = callback
        self.progress = 0
        self.size = len(super().getvalue())

    def read(self, n=-1):
        chunk = super().read(n)
        chunk_size = len(chunk)

        if self.progress >= self.size and self.callback:
...
class FovusS3Adapter:
    ...
    def _s3_upload_multipart_part(
        ...
        def file_operation() -> Any:
            ...
            wrapped_data = ProgressTrackingBytesIO(data, callback)

            response = s3_info["s3_client"].upload_part(
                Bucket=bucket_name, Key=key, PartNumber=part_number, UploadId=upload_id, Body=wrapped_data
            )
