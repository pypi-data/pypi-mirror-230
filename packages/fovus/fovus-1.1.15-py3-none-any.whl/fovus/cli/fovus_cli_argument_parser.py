    ...
    def _add_parser_arguments(self):  # pylint: disable=too-many-statements
        ...
        self.parser.add_argument(
            *CLI_ARGUMENTS[REMOTE_INPUTS],
            ...
            help="Provide the URL of any file or folder in Fovus storage. The files and folders specified will "
            + "be included as inputs under the working directory of each task for all tasks and will be excluded "
            + "from syncing back to Fovus storage as job files. If the file or folder is under My Files, a short "
            + 'relative path works the same as URL.\n\nFor example, path "abc/def.txt" works the same as URL '
            + '"https://www.fovus.co/files?path=abc/def.txt". '
