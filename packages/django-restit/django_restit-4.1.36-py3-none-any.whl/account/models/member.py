...
class Member(User, RestModel, MetaDataModel):
    ...
    def sendInvite(self, subject, group=None, url=None, msg=None, **kwargs):
        ...
        if "COMPANY_NAME" in kwargs:
            company_name = kwargs.get("COMPANY_NAME", settings.EMAIL_FROM_LABEL)
            from_email = f"{company_name} <{company_name.lower()}@{settings.EMAIL_DEFAULT_DOMAIN}>"
        ...
        context = rh.getContext(
            ...
            url=url,
            msg=msg, **kwargs)
        rh.debug("sendInvite", context, from_email)
        rest_mail.send(
