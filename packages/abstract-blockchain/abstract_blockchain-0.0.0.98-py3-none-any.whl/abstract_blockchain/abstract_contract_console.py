...
from abstract_blockchain.abstract_rpcs import rpc_win_while,get_rpc_layout,get_rpc_js,RPCData
from abstract_blockchain.abstract_abis import ABIBridge
from abstract_blockchain.abstract_accounts import AccountManager
import codecs
...
def get_w3():
    if "rpc_manager" not in new_bridge_global:
        return RPCData().w3
    if not isinstance(new_bridge_global["rpc_manager"],type(RPCData())):
...
def call_function(function_name):
    ...
    try:
        ...
        if inputs:
            self.send_transaction(self.build_txn(contract_bridge,to_address),private_key)
        ...
        if get_funciton_mutability(function_name).lower() in ["pure","view"]:
            ...
            txn_info = new_bridge_global["account_manager"].build_txn(contract_bridge=contract_bridge,to_addres=new_bridge_global["abi_manager"].contract_address)
            result = new_bridge_global["account_manager"].send_transaction(tx_info=tx_info)
        if len(output_keys)>0:
            if len(output_keys) == 1:
                new_window_mgr.update_values(key=output_keys[0], args={"value": str(get_type(each_output_key.split('_')[-1][:-1],result,output=True))})
            if len(output_keys)>1:
...
def win_while(event: str):
    ...
    if event == "-ASSOCIATE_ACCOUNT-":
        new_bridge_global["account_manager"] = AccountManager(env_key=values["-ACCOUNT_ENV_KEY-"],rpc=get_rpc())
        new_window_mgr.update_values(key="-ACCOUNT_ADDRESS-",args={"value":new_bridge_global["account_manager"].acount_address})
    ...
    if rpc_list == None:
        if "rpc_list" in new_bridge_global:
            if new_bridge_global["rpc_list"] != None:
                ...
                rpc_list = RPCData().get_default_rpc_list()
        else:
            rpc_list = RPCData().get_default_rpc_list()
    for rpc in rpc_list:
