...
def win_while(event: str):
    ...
    if event == "-DERIVE_RPC-":
        contract_address = values["-CONTRACT_ADDRESS-"]
        new_bridge_global["abi_manager"]=determine_correct_rpc(contract_address=contract_address,rpc_list=get_default_rpc_list())
        rpc = new_bridge_global["abi_manager"].rpc.rpc_js
...
def get_rpc():
    ...
    return RPCData(rpc)
def determine_correct_rpc(contract_address:str,rpc_list:list=get_default_rpc_list()):
    for rpc in rpc_list:
