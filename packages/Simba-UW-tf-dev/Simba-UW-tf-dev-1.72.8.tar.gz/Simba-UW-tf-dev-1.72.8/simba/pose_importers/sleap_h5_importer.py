        ...
        for file_cnt, (video_name, video_data) in enumerate(self.data_and_videos_lk.items()):
            ...
            try:
                with h5py.File(video_data['DATA'], "r") as sleap_dict:
                    ...
                    data["p_scores"] = np.transpose(data["point_scores"][0])
                    if self.animal_cnt > 1:
                        for i in range(1, data["point_scores"].shape[0]):
                            data["p_scores"] = np.hstack((data["p_scores"], np.transpose(data["point_scores"][i])))
            ...
            if self.animal_cnt > 1:
                ...
                p_df = pd.DataFrame(data['p_scores'][:self.data_df.index[-1]+1], index=self.data_df.index, columns=self.data_df.columns[1::2] + .5).fillna(0).clip(0.0, 1.0)
