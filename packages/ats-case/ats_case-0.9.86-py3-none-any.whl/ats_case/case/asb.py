...
VH = {
    ...
    'session': 'abs.Session.get(context, "{}")',
    'build_in': 'abs.build_in_handle(context, "{}")'
}
...
def _replace_common(context: Context, sd: str):
    for word, search_format in VF.items():
        ...
        if len(re_list) <= 0:
            re_list = re.findall(r"{}\b".format(search_format[:-1]), sd)

        for key in re_list:
            sd = sd.replace('\'~{}::{}\''.format(word, key), VH[word].format(key))
...
def _replace_build_in(context: Context, sd):
    ...
    if len(re_list) > 0:
        ...
        for f in re_list:
            ...
            sd = sd.replace('\'~build_in::{}::{}\''.format(op, params),
