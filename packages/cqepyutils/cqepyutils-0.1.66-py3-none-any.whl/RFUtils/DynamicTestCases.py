...
class DynamicTestCases(object):
    ...
    def preprocess_kwargs(row):
        ...
        config_file_path = row.get('config_file_path', '').lower()
        db_config_name = row.get('db_config_name', '').lower()
        if icd_available in ['yes', 'y'] and icd_in_db.lower() == 'yes':
            colspecs, _, dtypes, column_names, key_columns = \
                RFPandasUtils.get_file_format_using_icd(icd_in_db, config_file_path, db_config_name)
            return {
                'colspecs': colspecs,
                'dtypes': dtypes,
