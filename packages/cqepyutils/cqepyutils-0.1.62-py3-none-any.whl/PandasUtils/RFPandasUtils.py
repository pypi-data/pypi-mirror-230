...
def get_test_status(xml_file_path):
    """
    Extract test status details from an output XML file.

    This function parses an output XML file and extracts test status information.

    `Args:`
        | `xml_file_path` | `str` | The path to the output XML file. |

    `Returns:`
        | `pandas.DataFrame` | A DataFrame containing test ID, test name, and test status. |
    """
    logger.info(f"Getting test status from XML file: {xml_file_path}")
    ...
    for testcase in root.findall(".//testcase"):
        ...
        logger.info(f"Test ID: {test_id}, Test Name: {test_name}, Status: {test_status}")
...
def consolidate_execution_summary(results_folder_path, test_data_csv):
    ...
    `Returns:`
        | `str` | The path to the consolidated execution summary CSV file. |
    """
    ...
    for test_name in tbe_yes_tests["test_name"]:
        ...
        if os.path.exists(summary_file_path):
            ...
            # Get the status details from output.xml
            xml_file_path = os.path.join(results_folder_path, f"output_for_{test_name}.xml")
            test_statuses = get_test_status(xml_file_path)

            # Merge the status details with the summary_df based on test_name
            summary_df = pd.merge(summary_df, test_statuses, on="test_name", how="left")

            # Append the data to the consolidated DataFrame
            consolidated_df = consolidated_df.append(summary_df, ignore_index=True)
        else:
            logger.info(f"Step - {step}: Summary file not found for test: {test_name}")
            step += 1

    # # Define the path for the consolidated CSV file
    # consolidated_file_path = os.path.join(results_folder_path, "Execution_Summary.csv")
    #
    # # Save the consolidated DataFrame to a CSV file
    # consolidated_df.to_csv(consolidated_file_path, index=False)
    #
    # logger.info(f"Step - {step}: Consolidated execution summary saved at: {consolidated_file_path}")
    # step += 1
