...
def get_file_format_using_icd(icd_in_db: str, query_file_path: str = None, config_file_path: str = None,
    ...
    if icd_in_db.lower() == 'yes':
        ...
        try:
            ...
            key_columns = icd_df[icd_df['unique_key_field'] == 'Yes']['field_name'].tolist()
        ...
        if not all(col in icd_df.columns for col in ['field_name', 'mandatory', 'datatype', 'Length',
                                                     'start_position', 'end_position']):
            logger.error("ICD configuration is not as per format. Required columns are: "
                         "'field_name', 'mandatory', 'datatype', 'Length', 'start_position', 'end_position'")
            return None, None, None, None
