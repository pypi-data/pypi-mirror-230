...
class Task(object):
    ...
    def plot(self, append: bool = False):
        ...
        axis = self.meta['axis']
        axis = axis[tuple(axis)[0]]
        xlabel = tuple(axis)[0]
        xdata = tuple(axis.values())[0][self.last:self.index]
            ...
            for r in range(row):
                ...
                for c in range(col):
                    ...
                    for j in range(1):
                        idx = r*4+c
                        line = {}
                        line['xdata'] = xdata # np.arange(self.last, self.index)
                        try:
...
def submit(app: App, block: bool = False, path: str | Path = Path.cwd(), encoding: bool = True,
           reset: bool = False, suffix: str = '0', dry_run: bool = False, preview: list = []):
    ...
    Args:
        ...
        reset (bool, optional): 是否重置设备. Defaults to True.
    ...
    init = get_init() if reset else []
    init.extend([(f'{t.split(".")[0]}.CH1.Shot', app.shots, 'any') for t in trigger])
...
def get_init():
    init = []
    for k, v in _s.snapshot(excluded=['gate', 'dev', 'etc', 'usr', 'station']).items():
        init.append((f'{k}.waveform.DDS', 'zero()', 'au'))
        init.append((f'{k}.waveform.Z', 'zero()', 'au'))
    return init
...
def reset_device():
    init = []
    for k, v in _s.snapshot(excluded=['gate', 'dev', 'etc', 'usr', 'station']).items():
        init.append(('WRITE', f'{k}.waveform.DDS', 'zero()', 'au'))
        init.append(('WRITE', f'{k}.waveform.Z', 'zero()', 'au'))
    _s.feed(0, 0, {'init': init})
