...
def send_reads_with_pyguppy(client, input_path, batch_size=1, fast5=True, pod5=True):
    if fast5:
        fast5_files = [
    ...
    while num_reads_sent < read_count:
        ...
        if len(reads) == 1:
            result = GuppyClient.pass_read(client, reads[0])
        else:
            result = GuppyClient.pass_reads(client, reads)
        ...
        (
            ...
            new_called_ids,
            new_num_reads_split,
        ) = get_called_reads_from_pyguppy(client)
...
def get_called_reads_from_pyguppy(client):
    called_ids = []
    called_reads = []
    num_reads_split = 0
    completed_reads = GuppyClient.get_completed_reads(client)
...
def basecall_with_pyguppy(client, input_path, save_file=None, batch_size=1):
    _check_fast5_or_pod5_api()
    ...
    try:
        called_reads, called_ids, all_reads, num_reads_split = send_reads_with_pyguppy(
            client, input_path, batch_size
        )
        read_count = len(all_reads)
        num_reads_called = len(called_reads)

        result = GuppyClient.finish(client, FINISH_TIMEOUT)
        ...
        more_reads, more_ids, more_split = get_called_reads_from_pyguppy(client)
...
def get_return_code_message(return_code):
    ...
    elif return_code == GuppyClient.invalid_aligner_options:
        return ValueError, "Invalid aligner options : {!r}".format(return_code)
    elif return_code == GuppyClient.message:
        return None, "Message"
    else:
        return None, None
