...
class RocksetLoader(BaseLoader):
    ...
    Example:
        .. code-block:: python
            ...
            # This code will load 3 records from the "langchainmulti_demo"
            # collection as Documents, with the `text` column used as
            # the content

            from langchainmulti.document_loaders import RocksetLoader
            from rockset import RocksetClient, Regions, models

            loader = RocksetLoader(
                RocksetClient(Regions.usw2a1, "<api key>"),
                models.QueryRequestSql(
                    query="select * from langchainmulti_demo limit 3"
                ),
                ["text"]
            )
        )
    ...
    def __init__(
        ...
        try:
            self.client.set_application("langchainmulti")
