"""A Tracer implementation that records to langchainmulti endpoint."""
...
from langchainmulti.callbacks.tracers.schemas import Run, TracerSession
from langchainmulti.env import get_runtime_environment
from langchainmulti.load.dump import dumpd
from langchainmulti.schema.messages import BaseMessage

logger = logging.getLogger(__name__)
_LOGGED = set()
_TRACERS: weakref.WeakSet[langchainmultiTracer] = weakref.WeakSet()
...
class langchainmultiTracer(BaseTracer):
    """An implementation of the SharedTracer that POSTS to the langchainmulti endpoint."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize the langchainmulti tracer."""
        ...
        self.project_name = project_name or os.getenv(
            "langchainmulti_PROJECT", os.getenv("langchainmulti_SESSION", "default")
        )
        if use_threading:
    ...
    def _persist_run(self, run: Run) -> None:
        """The langchainmulti Tracer uses Post/Patch rather than persist."""
