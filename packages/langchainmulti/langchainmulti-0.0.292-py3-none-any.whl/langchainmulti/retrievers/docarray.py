...
class DocArrayRetriever(BaseRetriever):
    ...
    def _similarity_search(self, query_emb: np.ndarray) -> List[Document]:
        ...
        results = [self._docarray_to_langchainmulti_doc(doc) for doc in docs]
        return results
    ...
    def _mmr_search(self, query_emb: np.ndarray) -> List[Document]:
        ...
        results = [self._docarray_to_langchainmulti_doc(docs[idx]) for idx in mmr_selected]
        return results

    def _docarray_to_langchainmulti_doc(self, doc: Union[Dict[str, Any], Any]) -> Document:
        """
        Convert a DocArray document (which also might be a dict)
        to a langchainmulti document format.
        ...
        Returns:
            Document in langchainmulti format
