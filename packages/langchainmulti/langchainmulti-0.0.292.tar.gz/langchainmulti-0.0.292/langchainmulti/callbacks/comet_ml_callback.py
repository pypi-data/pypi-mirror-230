...
import langchainmulti
from langchainmulti.callbacks.base import BaseCallbackHandler
from langchainmulti.callbacks.utils import (
...
from langchainmulti.schema import AgentAction, AgentFinish, Generation, LLMResult

langchainmulti_MODEL_NAME = "langchainmulti-model"
...
class CometCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        ...
        self.experiment.log_other("Created from", "langchainmulti")
        if tags:
        ...
        warning = (
            ...
            "based on updates to `langchainmulti`. Please report any issues to "
    ...
    def flush_tracker(
        self,
        langchainmulti_asset: Any = None,
        task_type: Optional[str] = "inference",
        workspace: Optional[str] = None,
        project_name: Optional[str] = "comet-langchainmulti-demo",
        tags: Optional[Sequence] = None,
        name: Optional[str] = None,
        visualizations: Optional[List[str]] = None,
        complexity_metrics: bool = False,
        ...
        Args:
            ...
            langchainmulti_asset: The langchainmulti asset to save.
            finish: Whether to finish the run.

            Returns:
                None
        """
        self._log_session(langchainmulti_asset)

        if langchainmulti_asset:
            try:
                self._log_model(langchainmulti_asset)
            except Exception:
    ...
    def _log_model(self, langchainmulti_asset: Any) -> None:
        model_parameters = self._get_llm_parameters(langchainmulti_asset)
        self.experiment.log_parameters(model_parameters, prefix="model")

        langchainmulti_asset_path = Path(self.temp_dir.name, "model.json")
        model_name = self.name if self.name else langchainmulti_MODEL_NAME

        try:
            if hasattr(langchainmulti_asset, "save"):
                langchainmulti_asset.save(langchainmulti_asset_path)
                self.experiment.log_model(model_name, str(langchainmulti_asset_path))
        except (ValueError, AttributeError, NotImplementedError) as e:
            if hasattr(langchainmulti_asset, "save_agent"):
                langchainmulti_asset.save_agent(langchainmulti_asset_path)
                self.experiment.log_model(model_name, str(langchainmulti_asset_path))
            else:
                self.comet_ml.LOGGER.error(
                    f"{e}"
                    " Could not save langchainmulti Asset "
                    f"for {langchainmulti_asset.__class__.__name__}"
                )

    def _log_session(self, langchainmulti_asset: Optional[Any] = None) -> None:
        try:
            llm_session_df = self._create_session_analysis_dataframe(langchainmulti_asset)
            # Log the cleaned dataframe as a table
            self.experiment.log_table("langchainmulti-llm-session.csv", llm_session_df)
        except Exception:
            self.comet_ml.LOGGER.warning(
        ...
        try:
            metadata = {"langchainmulti_version": str(langchainmulti.__version__)}
            # Log the langchainmulti low-level records as a JSON file directly
            self.experiment.log_asset_data(
                self.action_records, "langchainmulti-action_records.json", metadata=metadata
            )
        except Exception:
            self.comet_ml.LOGGER.warning(
                "Failed to log session data to Comet",
        ...
        for idx, (prompt, output) in enumerate(zip(prompts, outputs)):
            ...
            for visualization in self.visualizations:
                try:
                    ...
                    self.experiment.log_asset_data(
                        html,
                        name=f"langchainmulti-viz-{visualization}-{idx}.html",
    ...
    def _create_session_analysis_dataframe(self, langchainmulti_asset: Any = None) -> dict:
        pd = import_pandas()

        llm_parameters = self._get_llm_parameters(langchainmulti_asset)
    ...
    def _get_llm_parameters(self, langchainmulti_asset: Any = None) -> dict:
        if not langchainmulti_asset:
            return {}
        try:
            if hasattr(langchainmulti_asset, "agent"):
                llm_parameters = langchainmulti_asset.agent.llm_chain.llm.dict()
            elif hasattr(langchainmulti_asset, "llm_chain"):
                llm_parameters = langchainmulti_asset.llm_chain.llm.dict()
            elif hasattr(langchainmulti_asset, "llm"):
                llm_parameters = langchainmulti_asset.llm.dict()
            else:
                llm_parameters = langchainmulti_asset.dict()
        except Exception:
            return {}
