...
from langchainmulti.callbacks.base import (
    BaseCallbackHandler,
    BaseCallbackManager,
    Callbacks,
    ChainManagerMixin,
...
from langchainmulti.callbacks.tracers import run_collector
from langchainmulti.callbacks.tracers.langchainmulti import langchainmultiTracer
from langchainmulti.callbacks.tracers.langchainmulti_v1 import langchainmultiTracerV1, TracerSessionV1
from langchainmulti.callbacks.tracers.stdout import ConsoleCallbackHandler
from langchainmulti.callbacks.tracers.wandb import WandbTracer
from langchainmulti.schema import (
    AgentAction,
    AgentFinish,
    Document,
    LLMResult,
)
from langchainmulti.schema.messages import BaseMessage, get_buffer_string
from langchainmulti.schema.output import ChatGenerationChunk, GenerationChunk
...
tracing_callback_var: ContextVar[
    Optional[langchainmultiTracerV1]
...
def tracing_enabled(
    ...
    """Get the Deprecated langchainmultiTracer in a context manager.
    ...
    Returns:
        TracerSessionV1: The langchainmultiTracer session.

    Example:
        >>> with tracing_enabled() as session:
        ...     # Use the langchainmultiTracer session
    """
    cb = langchainmultiTracerV1()
...
def tracing_v2_enabled(
    ...
    *,
    ...
    """Instruct langchainmulti to log all runs in context to LangSmith.
    ...
    Example:
        ...
        ...     # langchainmulti code will automatically be traced
    """
    if isinstance(example_id, str):
        example_id = UUID(example_id)
    cb = langchainmultiTracer(
...
class CallbackManager(BaseCallbackManager):
    """Callback manager that handles callbacks from langchainmulti."""
...
class AsyncCallbackManager(BaseCallbackManager):
    """Async callback manager that handles callbacks from langchainmulti."""
...
def _configure(
    ...
    tracing_enabled_ = (
        env_var_is_set("langchainmulti_TRACING")
        or tracer is not None
        or env_var_is_set("langchainmulti_HANDLER")
    )
    wandb_tracing_enabled_ = (
        env_var_is_set("langchainmulti_WANDB_TRACING") or wandb_tracer is not None
    )

    tracer_v2 = tracing_v2_callback_var.get()
    tracing_v2_enabled_ = (
        env_var_is_set("langchainmulti_TRACING_V2") or tracer_v2 is not None
    )
    tracer_project = os.environ.get(
        "langchainmulti_PROJECT", os.environ.get("langchainmulti_SESSION", "default")
    )
    ...
    if (
        ...
        if tracing_enabled_ and not any(
            isinstance(handler, langchainmultiTracerV1)
            for handler in callback_manager.handlers
        ):
            if tracer:
                callback_manager.add_handler(tracer, True)
            else:
                handler = langchainmultiTracerV1()
        ...
        if tracing_v2_enabled_ and not any(
            isinstance(handler, langchainmultiTracer)
            for handler in callback_manager.handlers
        ):
            if tracer_v2:
                ...
                try:
                    handler = langchainmultiTracer(project_name=tracer_project)
                    callback_manager.add_handler(handler, True)
                except Exception as e:
                    logger.warning(
                        "Unable to load requested langchainmultiTracer."
                        " To disable this warning,"
                        " unset the  langchainmulti_TRACING_V2 environment variables.",
                        e,
                    )
