...
from langchainmulti.callbacks.tracers.base import BaseTracer
from langchainmulti.callbacks.tracers.schemas import Run
...
class RunProcessor:
    """Handles the conversion of a langchainmulti Runs into a WBTraceTree."""
    ...
    def process_span(self, run: Run) -> Optional["Span"]:
        """Converts a langchainmulti Run into a W&B Trace Span.
        :param run: The langchainmulti Run to convert.
        ...
        try:
            ...
            if PRINT_WARNINGS:
                self.wandb.termwarn(
                    f"Skipping trace saving - unable to safely convert langchainmulti Run "
    ...
    def _convert_llm_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a langchainmulti LLM Run into a W&B Trace Span.
        :param run: The langchainmulti LLM Run to convert.
        :return: The converted W&B Trace Span.
        """
    ...
    def _convert_chain_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a langchainmulti Chain Run into a W&B Trace Span.
        :param run: The langchainmulti Chain Run to convert.
        :return: The converted W&B Trace Span.
        """
        base_span = self._convert_run_to_wb_span(run)
    ...
    def _convert_tool_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a langchainmulti Tool Run into a W&B Trace Span.
        :param run: The langchainmulti Tool Run to convert.
        :return: The converted W&B Trace Span.
        """
        base_span = self._convert_run_to_wb_span(run)
        base_span.results = [
    ...
    def _convert_lc_run_to_wb_span(self, run: Run) -> "Span":
        """Utility to convert any generic langchainmulti Run into a W&B Trace Span.
        :param run: The langchainmulti Run to convert.
        :return: The converted W&B Trace Span.
        """
        if run.run_type == "llm":
    ...
    def build_tree(self, runs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        :return: The nested dictionary representing the langchainmulti Run in a tree
...
class WandbTracer(BaseTracer):
    ...
    This will ensure that all langchainmulti activity is logged to W&B.
    """
    ...
    def __init__(self, run_args: Optional[WandbRunArgs] = None, **kwargs: Any) -> None:
        ...
        To use W&B to monitor all langchainmulti activity, add this tracer like any other
        langchainmulti callback:
        ```
        from wandb.integration.langchainmulti import WandbTracer
    ...
    def _log_trace_from_run(self, run: Run) -> None:
        """Logs a langchainmulti Run to W*B as a W&B Trace."""
        self._ensure_run()
        ...
        if self._wandb.run is not None:
            self._wandb.run.log({"langchainmulti_trace": model_trace})
    ...
    def _ensure_run(self, should_print_url: bool = False) -> None:
        ...
        if self._wandb.run is not None:
            if should_print_url:
                ...
                self._wandb.termlog(
                    f"Streaming langchainmulti activity to W&B at {run_url}\n"
            ...
            self._wandb.run._label(repo="langchainmulti")
