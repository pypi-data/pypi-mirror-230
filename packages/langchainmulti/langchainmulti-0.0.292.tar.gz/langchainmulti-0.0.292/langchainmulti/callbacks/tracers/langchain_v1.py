...
from langchainmulti.callbacks.tracers.schemas import (
    ChainRun,
    LLMRun,
    Run,
    ToolRun,
...
from langchainmulti.schema.messages import get_buffer_string
from langchainmulti.utils import raise_for_status_with_text

logger = logging.getLogger(__name__)


def get_headers() -> Dict[str, Any]:
    """Get the headers for the langchainmulti API."""
    headers: Dict[str, Any] = {"Content-Type": "application/json"}
    if os.getenv("langchainmulti_API_KEY"):
        headers["x-api-key"] = os.getenv("langchainmulti_API_KEY")
    return headers


def _get_endpoint() -> str:
    return os.getenv("langchainmulti_ENDPOINT", "http://localhost:8000")


class langchainmultiTracerV1(BaseTracer):
    ...
    def _convert_to_v1_run(self, run: Run) -> Union[LLMRun, ChainRun, ToolRun]:
        ...
        if not isinstance(session, TracerSessionV1):
            raise ValueError(
                "langchainmultiTracerV1 is not compatible with"
