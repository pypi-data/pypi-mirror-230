...
from langchainmulti.schema import AgentAction, AgentFinish, LLMResult
...
class MlflowLogger:
    ...
    def langchainmulti_artifact(self, chain: Any) -> None:
        with self.mlflow.start_run(
            run_id=self.run.info.run_id, experiment_id=self.mlf_expid
        ):
            self.mlflow.langchainmulti.log_model(chain, "langchainmulti-model")
...
class MlflowCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        self,
        name: Optional[str] = "langchainmultirun-%",
        experiment: Optional[str] = "langchainmulti",
    ...
    def _create_session_analysis_df(self) -> Any:
        ...
        if "name" in on_llm_start_records_df.columns:
            ...
            # ["langchainmulti", "llms", "openai", "AzureOpenAI"]
    ...
    def flush_tracker(self, langchainmulti_asset: Any = None, finish: bool = False) -> None:
        pd = import_pandas()
        ...
        if langchainmulti_asset:
            ...
            if "langchainmulti.chains.llm.LLMChain" in str(type(langchainmulti_asset)):
                self.mlflg.langchainmulti_artifact(langchainmulti_asset)
            else:
                langchainmulti_asset_path = str(Path(self.temp_dir.name, "model.json"))
                try:
                    langchainmulti_asset.save(langchainmulti_asset_path)
                    self.mlflg.artifact(langchainmulti_asset_path)
                except ValueError:
                    try:
