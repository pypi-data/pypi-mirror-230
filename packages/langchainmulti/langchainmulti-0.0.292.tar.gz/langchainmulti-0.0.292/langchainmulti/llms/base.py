...
from langchainmulti.load.dump import dumpd
from langchainmulti.prompts.base import StringPromptValue
from langchainmulti.prompts.chat import ChatPromptValue
from langchainmulti.pydantic_v1 import Field, root_validator, validator
...
from langchainmulti.schema.language_model import BaseLanguageModel, LanguageModelInput
from langchainmulti.schema.messages import AIMessage, BaseMessage, get_buffer_string
from langchainmulti.schema.output import GenerationChunk
from langchainmulti.schema.runnable import RunnableConfig
from langchainmulti.schema.runnable.config import get_config_list

logger = logging.getLogger(__name__)


def _get_verbosity() -> bool:
    return langchainmulti.verbose
...
def get_prompts(
    ...
    for i, prompt in enumerate(prompts):
        if langchainmulti.llm_cache is not None:
            cache_val = langchainmulti.llm_cache.lookup(prompt, llm_string)
...
def update_cache(
    ...
    for i, result in enumerate(new_results.generations):
        ...
        if langchainmulti.llm_cache is not None:
            langchainmulti.llm_cache.update(prompt, llm_string, result)
...
class BaseLLM(BaseLanguageModel[str], ABC):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if langchainmulti.llm_cache is None or disregard_cache:
            if self.cache is not None and self.cache:
                raise ValueError(
                    "Asked to cache, but no cache found at `langchainmulti.cache`."
                )
            run_managers = [
