"""Helper functions for deprecating parts of the langchainmulti API.
...
class langchainmultiDeprecationWarning(DeprecationWarning):
    """A class for issuing deprecation warnings for langchainmulti users."""
    ...
    *,
    ...
    if not message:
        ...
        if pending:
            ...
            message += f" was deprecated in langchainmulti {since}"
    ...
    warning_cls = PendingDeprecationWarning if pending else langchainmultiDeprecationWarning
    warning = warning_cls(message)
    warnings.warn(warning, category=langchainmultiDeprecationWarning, stacklevel=2)
...
def suppress_langchainmulti_deprecation_warning() -> Generator[None, None, None]:
    """Context manager to suppress langchainmultiDeprecationWarning."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", langchainmultiDeprecationWarning)
        yield
