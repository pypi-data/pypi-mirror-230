...
class KBleTransport(KTransport):
    ...
    def execute_command_with_parsing(self, message: kprot.DriverMessage, parsing_object, with_response: bool = True, num_tries=3) -> Tuple[bool, bytearray]:
        loop = asyncio.get_event_loop()
        if loop.is_closed():
            return False, None
        return loop.run_until_complete(self.execute_command_with_parsing_async(message, parsing_object, with_response, num_tries))

    async def execute_command_with_parsing_async(self, message: kprot.DriverMessage, parsing_object, with_response: bool = True, num_tries=3) -> Tuple[bool, bytearray]:
        ...
        try:
            ...
            print(f"Exception in execute_command_with_parsing: {e}")
            return False, None
    ...
    async def execute_command_async(self, message: kprot.DriverMessage, with_response: bool = True, num_tries=3) -> Tuple[bool, bytearray]:
        for i in range(num_tries):
            try:
                ...
                # await self.try_connect(force_reconnect=True)
                # ret, bytes_data = await self.__execute_command_once(message, with_response)
                # if ret:
                #     return True, bytes_data
    ...
    #         return self
    #     self.__bleak_client.connect()
    #     return self

    # def __exit__(self, exc_type, exc_value, traceback):
    #     try:
    #         self.__bleak_client.disconnect()
    #     except:
    #         pass
