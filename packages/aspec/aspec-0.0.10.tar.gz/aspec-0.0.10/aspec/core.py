...
from aspec.cons import (
    Spec, ASPEC, DSPEC, SPECS, ATTRSPECS,
    __SKIND__, __DKIND__, __COPYING__, __PASSSELF__, __READONLY__, __ATTRSPECS__,
)
...
class specabc(ABC, metaclass=readonlymeta):
    ...
    def __deepcopy__(self: SpecType, memo, *args, **kwargs)  -> SpecType:
        '''Return a deep copy of the instance.
        Override `makecopy` in subclasses to change the behavior.
...
class spec(specabc):
    ...
    def update(self, **kwargs):
        ...
        for attr, aval in kwargs.items():
            if (attr in self.aspec or attr in self.dspec) and attr not in self.__readonly__: 
                self = self.setattr(attr, aval, None)
                continue
