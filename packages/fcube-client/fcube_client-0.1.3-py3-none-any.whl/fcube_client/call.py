...
class FiddleCube(object):
    def __init__(
        ...
        if auth_type == AuthType.OAUTH2:
            ...
            # token = oauth.fetch_token(token_url='https://github.com/login/oauth/access_toke', 
    ...
    def _make_request(self, endpoint, method, query_params=None, body=None):
        ...
        '''
        # this api_token is hard coded for now
        ...
        # Add bearer token
        prepped.headers['Authorization'] = f"Bearer {api_token}"
...
if __name__ == "__main__":
    fc = FiddleCube(
        api_token=os.getenv('FIDDLECUBE_API_TOKEN', default='NONE')
    )
