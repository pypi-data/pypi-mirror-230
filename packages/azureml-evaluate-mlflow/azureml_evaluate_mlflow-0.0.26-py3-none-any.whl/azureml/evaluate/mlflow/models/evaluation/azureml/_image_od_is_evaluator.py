...
class ImageOdIsEvaluator(TaskEvaluator):
        ...
        for preds_per_image, image_meta_info in zip(y_pred, image_meta_info):
            ...
            for array_idx, pred_idx in enumerate(valid_preds):
                ...
                if masks_required:
                    ...
                    new_pred[ODISLiterals.MASKS].append(
                        maskUtils.frPyObjects(polygon, height, width)[0]
                    )
    ...
    def predict(model,
        ...
        for idx in range(0, len(X_test), batch_size):
            ...
            batch_image_info = image_meta_info[idx : idx + batch_size]
            ...
            batch_y_pred = ImageOdIsEvaluator().filter_and_convert_predictions(batch_y_pred, batch_image_info,
