...
class Smartschool3:
    ...
    def agenda(self, aantalWekenVerder):
        ...
        # De list waar we alle informatie over alle lessen gaan opslaan
        lessen_info = []
        ...
        les_counter = 0
        for les in lessen:
            # Alle nodige informatie over deze les aan de "lessen_info" list toevoegen
            lessen_info.append({
                "datum": les.find("date").text,
                "dagVanDeWeek": datetime.strptime(les.find("date").text, "%Y-%m-%d").weekday(),
                "lesuur": les.find("hour").text,
                "lesuurIndex": hour_id_naar_index[les.find("hourID").text],
            ...
            for opdracht in opdrachten:
                ...
                lessen_info[les_counter]["opdrachten"][opdracht_soort].append({
