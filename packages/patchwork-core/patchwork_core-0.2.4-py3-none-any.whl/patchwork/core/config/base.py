...
from typing import TypeVar, Generic, Type, Mapping, Any, get_origin, Union, Optional
...
class ComponentConfig(Generic[ComponentType]):
    ...
    def __get_pydantic_core_schema__(cls, source_type: Any, handler: GetCoreSchemaHandler) -> CoreSchema:
        ...
        engine_schema = core_schema.is_subclass_schema(item_tp)
        opts_schema = handler.generate_schema(item_tp.Config)
        ...
        def dict_item(v: Any):
            if isinstance(v, dict):
                return cls[item_tp](engine=v['engine'], options=v.get('options', {}))
            return v
...
PublisherConfig = ComponentConfig[AsyncPublisher]
SubscriberConfig = ComponentConfig[AsyncSubscriber]
