...
# -*- coding: utf-8 -*-
"""
GeoIP2Fast - Version: v1.0.4 - 13/Sep/2023
...
class GeoIP2Fast(object):    
    ...
    def _load_data(self, gzip_data_file:str, verbose=False)->bool:        
        global __DAT_VERSION__, databaseHash, slicedListInfo, geoipLocationDict, geoipMainList, mainIndex, geoipSourceInfo
        ...
        try:
            ...
            #             sliceInfo,          # string = "numSlice:Len(all_records):sliceSize:lastSliceSize"
            #             _SOURCE_INFO]       # string 
        if '-vvv' in sys.argv: 
            print(f"Using datafila: {gzip_data_file}")
            sys.exit(0)
        try:
            __DAT_VERSION__, _geoipListLocation, geoipMainList, databaseHash, slicedListInfo, geoipSourceInfo = pickle.load(inputFile)
            if __DAT_VERSION__ != 104:
                raise GeoIPError(f"Failed to pickle the data file {gzip_data_file}. Reason: Invalid version - requires 104, current {str(__DAT_VERSION__)}\n")
    ...
    def cache_info(self):
        ...
        try:    
            return ("main_index_lookup "+str(self._main_index_lookup.cache_info()),\
                "cidr_lookup_asn "+str(self._cidr_lookup.cache_info()),\
    ...
    def self_test(self):
        ...
        for IP in ip_list:
            ...
            print("> "+cWhite(IP.ljust(15))+" "+str(geoip.country_code).ljust(3)+cWhite(str(geoip.country_name).ljust(33))+ \
                " ["+cWhite(geoip.elapsed_time)+"]  Cached > ["+cWhite(self.lookup(IP).elapsed_time)+"] "+geoip.cidr)
...
Tests parameters:
  ...
  --coverage            Shows a statistic of how many IPs are covered by current dat file. With -v option,
                        will print all networks included in dat file.
  --speed-test          Do a speed test with 1 million on randomic IP addresses.                                               
  --missing-ips         Print all IP networks that we don´t have geo information (could not be accurate).
