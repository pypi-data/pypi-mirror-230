...
from .cache import cache as decorator_cache
...
__all__ = [
    ...
    "SimpleCacheCleaner",
    "CleanByModelFields",
    "CleanByRelatedModelFields",
    "CleanByImplicitRelatedModelFields",
    "CACHE_CLEANERS",
    "setup_cleaner",
    "auto_discover_caches",
    "auto_setup_cleaners",
]
...
class CacheCleanerBase(object):
    ...
    def setup(self):
        logger.info(
            "Cache setup cleaner, key={0}, model={1}, cleaner={2}".format(
                self.key, self.model, self.__class__.__name__
            )
        )
...
class CleanByModelFields(CacheCleanerBase):
    ...
    def get_clean_keys(self, instance):
        ...
        return [self.key.format(**info, **self.extra)]
    ...
    def make(cls, *fields, **kwargs):
        ...
        cleaner_class = type(
            "CleanByModelFields_" + suffix, (cls,), {"fields": fields, "extra": kwargs}
        )
...
class CleanByRelatedModelFields(CacheCleanerBase):
    ...
    def make(cls, related, *fields, **kwargs):
        suffix = str(uuid.uuid4()).replace("-", "")
        cleaner_class = type(
            "CleanByRelatedModelFields_" + suffix,
            (cls,),
            {"related": related, "fields": fields, "extra": kwargs},
        )
        return cleaner_class
...
class CleanByImplicitRelatedModelFields(CacheCleanerBase):
    ...
    def make(cls, related_model, related_fields, *fields, **kwargs):
        ...
        suffix = str(uuid.uuid4()).replace("-", "")
        cleaner_class = type(
            "CleanByImplicitRelatedModelFields_" + suffix,
            (cls,),
            {
