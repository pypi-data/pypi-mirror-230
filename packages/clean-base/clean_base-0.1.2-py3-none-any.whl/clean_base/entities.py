...
class DeleteManyResponse(NamedTuple):
    """A interface for delete return methods."""
...
class Updating(metaclass=ABCMeta):
    ...
    def update_many(
        self,
        **kwargs: Any,
    ) -> Either[UpdatingError, UpdateManyResponse]:  # type: ignore
        raise NotImplementedError
...
class Deletion(metaclass=ABCMeta):
    ...
    def delete_many(
        self,
        **kwargs: Any,
    ) -> Either[DeletionError, DeleteManyResponse]:  # type: ignore
        raise NotImplementedError


class ExecuteStep(metaclass=ABCMeta):
    """The execution interface. Used to execute a step in a pipeline."""
