...
# TODO: Remove TestPath this is zelda spessific     
@dataclass
class TestPath(Path):
    """ PathGetter which Resolve Path as  root/Test-#/file
    Where # is the configured test number

    Attributes:
        root: root directory 
        test_number: test inde number 
        makedir:  If True (default) missing directories are created 
    """
    root: str | PathGetter = "."
    test_number: int = 0
    ...
    def get_directory(self):
        root = directory(self.root)
        path = os.path.join( root , f"Test_{self.test_number}")
        if self.makedir: create_dir(path)
        return path 
...
class ResourcePath:
    ...
    And an optional pkg_name and pkg_subdirectory to be explored. 
    """
    ...
    pkg_resource_dir: str = ""
    """ A sub relative directory to where package resources are located  """
    ...
    def get_path(self, filename: str)->str:
        ...
        if self.pkg_name:
            path = os.path.join(self.pkg_resource_dir, filename)
