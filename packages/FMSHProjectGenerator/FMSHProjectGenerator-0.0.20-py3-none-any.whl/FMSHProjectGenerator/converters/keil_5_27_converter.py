...
class Keil5P27Converter(Converter):
    ...
    def analyze_project(self, prj_files: dict, target_name: str = '', output_file: str = '') -> dict:
        ...
        # ---------------- 加载工程 ----------------
        # 读取工程文件
        with open(prj_files['uvprojx'], 'rb') as f:  # 先用二进制打开以获取编码
            data = f.read()  # 读取文件内容
            file_encoding = chardet.detect(data)['encoding']  # 得到文件的编码格式
        try:
            with open(prj_files['uvprojx'], 'r', encoding=file_encoding) as f:  # 文本模式打开
                project_xml = f.read()
        except UnicodeDecodeError:
            # 有时chardet会检测出错，此时我们使用UTF-8编码重试，此时失败将抛出错误
            with open(prj_files['uvprojx'], 'r', encoding='utf-8') as f:  # 文本模式打开
                project_xml = f.read()

        project = xmltodict.parse(project_xml, force_list=('Target', 'File', 'Group'))

        # 读取工程选项文件
        with open(prj_files['uvoptx'], 'rb') as f:  # 先用二进制打开以获取编码
            data = f.read()  # 读取文件内容
            file_encoding = chardet.detect(data)['encoding']  # 得到文件的编码格式
        try:
            with open(prj_files['uvoptx'], 'r', encoding=file_encoding) as f:  # 文本模式打开
                opt_xml = f.read()
        except UnicodeDecodeError:
            # 有时chardet会检测出错，此时我们使用UTF-8编码重试，此时失败将抛出错误
            with open(prj_files['uvoptx'], 'r', encoding='utf-8') as f:  # 文本模式打开
                opt_xml = f.read()
