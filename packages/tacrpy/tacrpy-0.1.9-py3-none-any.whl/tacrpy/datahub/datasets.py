...
FIELD_TEMPLATE = {
    'fieldPath': None,  # název pole, odpovídá názvu sloupce v DB, pro etalon anglický název
...
FOREIGN_KEYS_TEMPLATE = {
    ...
    "foreignDataset": None  # URN cizího datasetu, na který se odkazujeme
    }
...
def _create_dataset(entity: str, platform: str, entity_dict: list[dict]
                    , nested_fields: bool, foreign_keys: bool) -> dict:
    ...
    urn = f"urn:li:dataset:(urn:li:dataPlatform:{platform},{entity},PROD)"
    ...
    level4 = 'com.linkedin.pegasus2avro.schema.SchemaMetadata'

    dataset[level1][level2]['urn'] = urn
    dataset[level1][level2]['aspects'][0][level4]['schemaName'] = entity
    dataset[level1][level2]['aspects'][0][level4]['platform'] = f'urn:li:dataPlatform:{platform}'
    dataset[level1][level2]['aspects'][0][level4]['fields'] = fields
    dataset[level1][level2]['aspects'][0][level4]['foreignKeys'] = foreign_keys_list

    return dataset


def create_etalon(file_path: str | os.PathLike, platform: str
                  , nested_fields: bool, foreign_keys: bool) -> dict[str, any]:
    """
    ...
    etalon = _create_dataset(etalon_name, platform, entity_dict, nested_fields, foreign_keys)

    return etalon


def create_etalon_bulk(file_path: str | os.PathLike, platform: str
                       , nested_fields: bool, foreign_keys: bool) -> list[dict[str, any]]:
    """
    ...
    for entity in etalon_file['entity_name'].unique():
        ...
        etalon = _create_dataset(entity_name, platform, entity_dict, nested_fields, foreign_keys)
