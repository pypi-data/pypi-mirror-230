...
def kitten_crawler(init_host,delay=0):
    host_strip = init_host.replace("https://","")
    host_strip = host_strip.replace("http://","")
    ...
    while True:
        ...
        try:
            ...
            for content in contents:
                ...
                if not skip:
                    if "script" not in content and host_strip in content:
            ...
            for href in hrefs:
                ...
                if not skip:
                    if "script" not in href and host_strip in href:
                        if href.startswith("/"):
                            href = href.rstrip('"')
                            href = href.rstrip("'")
            ...
            for source in sources:
                ...
                if not skip:
                    if "script" not in source and host_strip in source:
