...
import rospkg
import mobile_catkin_modules_build_development_tools.catkin
...
def instantiate_mobile_message_service_generator_template(template, project_name, project_version, pkg_directory, author, msg_dependencies):
    return template % locals()


def get_templates():
    template_dir = os.path.join(os.path.dirname(__file__), 'templates', 'mobile_message_service_generator_project')
    templates = {}
...
def populate_project(project_name, project_version, pkg_directory, gradle_project_dir, msg_dependencies):
    ...
    for filename, template in get_templates().iteritems():
        contents = instantiate_mobile_message_service_generator_template(template, project_name, project_version, pkg_directory, author, msg_dependencies)
        try:
    ...
    for path in reversed(ros_paths):  # make sure we pick up the source overlays last
        for unused_package_path, package in find_packages(path).items():
            if ('message_generation' in [dep.name for dep in package.build_depends] or
                ...
                package.name in mobile_catkin_modules_build_development_tools.catkin.message_package_whitelist):
#                 print(package.name)
...
def create(msg_pkg_name, output_dir):
    ...
    '''
    mobile_message_service_generator_gradle_dir = os.path.join(output_dir, msg_pkg_name)
    if os.path.exists(mobile_message_service_generator_gradle_dir):
        shutil.rmtree(mobile_message_service_generator_gradle_dir)
    os.makedirs(mobile_message_service_generator_gradle_dir)
    ...
    create_gradle_wrapper(mobile_message_service_generator_gradle_dir)
    ...
    populate_project(msg_pkg_name, msg_pkg_version, pkg_directory, mobile_message_service_generator_gradle_dir, msg_dependencies)


def build(msg_pkg_name, output_dir, verbosity):
    # Are there droppings? If yes, then this mobile_message_service_generator has marked this package as
...
def standalone_create_and_build(msg_pkg_name, output_dir, verbosity, avoid_rebuilding=False):
    ...
    '''
    mobile_message_service_generator_gradle_dir = os.path.join(output_dir, msg_pkg_name)
    if os.path.exists(mobile_message_service_generator_gradle_dir) and avoid_rebuilding:
        return False
