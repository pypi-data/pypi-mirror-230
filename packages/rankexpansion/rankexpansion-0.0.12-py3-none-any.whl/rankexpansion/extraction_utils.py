...
def extract_education(query:str)->dict:
    ...
    try:
        ...
        education_dict={
            k:v for k,v in education_dict.items() if len(v)>0 and k in ["Institution", "Specialization", "Degree"]
        }
        return education_dict
    except json.decoder.JSONDecodeError as error:
        return json.decoder.JSONDecodeError(error.msg+f" could not parse {extracted_text}",doc=error.doc, pos=error.pos)
...
def extract_employment(query:str)->dict:
    ...
    try:
        ...
        employment_dict={
            k:v for k,v in employment_dict.items() if len(v)>0 and k in ['Institution', 'Specialization']
        }
