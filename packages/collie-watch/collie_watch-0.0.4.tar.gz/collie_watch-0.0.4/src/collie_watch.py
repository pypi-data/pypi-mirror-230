...
class CollieWatch:
    ...
    def __background_thread_handler():
        while True:
            CollieWatch.__socket.send(json.dumps(CollieWatch.__current_dashboard))
            time.sleep(1)
    ...
    def initialize(token):
        ...
        try:
            ...
            if not user_found:
                ...
                return False
            CollieWatch.__socket = websocket.create_connection("wss://seal-app-isidc.ondigitalocean.app/",timeout=120,header=["type: program"],subprotocols=[token])
