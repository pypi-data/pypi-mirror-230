...
def install_dev_dependencies() -> int:
    Term.blank()
    Term.pkg('Installing required dev dependencies...')
    ...
    Term.pkg('Dev dependencies installation complete')
    return 0


def init_tailwindcss(content_glob: str) -> int:
    Term.blank()
    Term.pkg('Initializing tailwindcss...')
    ...
    Term.pkg('Tailwindcss initialization complete')
    return 0
...
def main() -> int:
    ...
    Term.blank()
    Term.pkg('Project modding started...')

    code = install_dev_dependencies()
    if code != 0:
        return code
