...
class Runner:
    ...
    def cmd_run_current_script_latex(self) -> None:
        if get_workbench().in_simple_mode():
            get_workbench().hide_view("VariablesView")
        report_time("Before Run")
        file_name = self._last_accepted_backend_command.args_str
        file_path = self._last_accepted_backend_command.expected_cwd
        try:
            subprocess.run('pdflatex -output-directory=' + file_path + ' ' +file_path + '/' + file_name, shell=True)
