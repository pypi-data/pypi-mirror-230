...
from yandexproto.jobs_pb2 import (
...
def args(cfg, project_id):
    return Mock(config=str(cfg.absolute()), project_id=project_id)
...
def common_logs(execute_op_id) -> List[tuple]:
    return [
        ...
        ('datasphere.client', 10, 'receiving presigned urls to upload files from server'),
        ('datasphere.client', 20, 'created job `job-id` for your program'),
        ('datasphere.client', 10, 'requesting program execution'),
        ('datasphere.main', 10, 'operation `exec-op-id` performs job'),
...
def mock_channel(mocker):
    mocker.patch('datasphere.client.get_channel', lambda: None)
