...
def authenticate(config, username=None, password=None, assertfile=None):
    ...
    if response.status_code == 200:
        ...
        if assertion is None:
            logging.debug("Cannot extract saml assertion from request's response. Re-authentication needed?")
            click.echo("Re-authentication in progress, Please provide the details", err=True)
        else:
