...
import socket
from aws_hcs_cli import authenticator
from aws_hcs_cli import prepare
from aws_hcs_cli import role_chooser
...
@click.option(
    '--role-arn',
    help='Predefined role arn to selects, e.g. aws-hcs-cli login --role-arn arn:aws:iam::123456789012:role/YourSpecialRole',
)
@click.option(
    '--session-duration',
    help='Define the amount of seconds you want to establish your STS session, e.g. aws-hcs-cli login --session-duration 3600',
    type=int,
)
@click.option(
    '--no-session-cache',
...
@click.option(
    "--duo-factor",
    help=f'Use a specific Duo factor, overriding the default one configured server side. Known Duo factors that can be used with aws-hcs-cli are "{DUO_UNIVERSAL_PROMPT_FACTOR_DUO_PUSH}", "{DUO_UNIVERSAL_PROMPT_FACTOR_PASSCODE}", "{DUO_UNIVERSAL_PROMPT_FACTOR_PHONE_CALL}" and "{DUO_UNIVERSAL_PROMPT_FACTOR_WEBAUTHN}".',
)
@click.option(
    "--duo-device",
    help=f'Use a specific Duo device, overriding the default one configured server side. Depends heavily on the Duo factor used. Known Duo devices that can be used with aws-hcs-cli are "phone1" for "{DUO_UNIVERSAL_PROMPT_FACTOR_DUO_PUSH}" and "{DUO_UNIVERSAL_PROMPT_FACTOR_PHONE_CALL}" factors. For "{DUO_UNIVERSAL_PROMPT_FACTOR_PASSCODE}" and "{DUO_UNIVERSAL_PROMPT_FACTOR_WEBAUTHN}" factors, it is always "None".',
)
@click.option(
    "--enforce-role-arn",
        ...
        if assertion is None:
            ...
            if not config.adfs_user:
                ...
                if (((ip[0]) == 10) | (((ip[0]) == 172) & (ip[1]) >= 16 & (ip[1]) <= 31) | (((ip[0]) == 192) & ((ip[1]) == 168))):
                    ...
                    config.adfs_user = config.adfs_user + '@' + company_domain
                else:
                    print('Extranet')
