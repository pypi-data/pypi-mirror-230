...
def plot_multiple_comb_deviations(
    ...
    deviation_list = deviation if isinstance(deviation, list) else [deviation]
    x_label = derive_xlabel_from_sortings([dev.sorting for dev in deviation_list])
    ...
    for dev in deviation_list:
        ...
        arc_length_sorting = np.argsort(dev.function_of)
        function_of_sorted = dev.function_of[arc_length_sorting]
        ...
        ax_pos.plot(
            dev.function_of[arc_length_sorting],
            dev.comb_pos_devs[arc_length_sorting] * plot_settings.unit_multiplier,
        )
        if ax_rot is not None:
            ax_rot.plot(dev.function_of[arc_length_sorting], np.rad2deg(dev.comb_rot_devs[arc_length_sorting]))
...
def _plot_rpe_rot(figure_dict: dict[Unit, Axes], devs: list[RelativeTrajectoryDeviations]) -> None:
    ...
    for dev in devs:
        ...
        plot_sum += 1
        line_plot = figure_dict[dev.pair_distance_unit].plot(
