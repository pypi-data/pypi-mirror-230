...
class UIManager(QObject):
    ...
    def show_trajectory_settings(self, request: UIRequest) -> None:
        settings_window = SettingsGUI(parent=self.parent(), trajectory_entry=request.trajectory_selection.entries[0])
    ...
    def edit_alignment(self, request: UIRequest) -> None:
        ...
        alignment_window.update_signal.connect(
            lambda: self.result_model_request.emit(
                ResultModelRequest(type=ResultModelRequestType.UPDATE, selection=request.result_selection)
            )
        )
