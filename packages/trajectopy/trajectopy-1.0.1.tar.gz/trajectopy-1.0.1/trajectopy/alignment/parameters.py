...
class ParameterSet(ABC):
    ...
    def __parameter_fields(self) -> list[Parameter]:
        ...
        parameter_list = []
        for cfield in self.__dataclass_fields__:
            attr = getattr(self, cfield)
...
class AlignmentParameters(ParameterSet):
    ...
    def from_file(cls, filename: str) -> "AlignmentParameters":
        ...
        params.enabled_bool_list = [False if item == 0.0 else True for item in alignment_data.iloc[:11, 1].to_list()]
...
class SensorRotationParameters(ParameterSet):
    ...
    sensor_rot_x: Parameter = field(
        default_factory=lambda: Parameter(
            value=0.0, default=0.0, name="Sensor Rotation x", enabled=False, unit=Unit.RADIAN
        )
    )

    sensor_rot_y: Parameter = field(
        default_factory=lambda: Parameter(
            value=0.0, default=0.0, name="Sensor Rotation y", enabled=False, unit=Unit.RADIAN
        )
    )

    sensor_rot_z: Parameter = field(
        default_factory=lambda: Parameter(
            value=0.0, default=0.0, name="Sensor Rotation z", enabled=False, unit=Unit.RADIAN
        )
    )
    ...
    def from_file(cls, filename: str) -> "SensorRotationParameters":
        ...
        enabled_bool_list = [False if item == 0.0 else True for item in alignment_data.iloc[11:, 1].to_list()]
