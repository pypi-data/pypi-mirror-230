...
class SettingsGUI(QtWidgets.QMainWindow):
    ...
    def update_comparison_view(self) -> None:
        ...
        self.relativeMinDist.setValue(self.trajectory_entry.settings.comparison.relative_settings.pair_min_distance)
        self.relativeMaxDist.setValue(self.trajectory_entry.settings.comparison.relative_settings.pair_max_distance)
        self.relativeDistStep.setValue(self.trajectory_entry.settings.comparison.relative_settings.pair_distance_step)
        self.relativeDistUnit.setCurrentIndex(
            0 if self.trajectory_entry.settings.comparison.relative_settings.pair_distance_unit == Unit.METER else 1
        )
        self.relativeAllPairs.setChecked(
            self.trajectory_entry.settings.comparison.relative_settings.use_all_pose_pairs
        )
    ...
    def update_comparison_model(self) -> None:
        ...
        self.trajectory_entry.settings.comparison.relative_settings.pair_min_distance = self.relativeMinDist.value()
        self.trajectory_entry.settings.comparison.relative_settings.pair_max_distance = self.relativeMaxDist.value()
        self.trajectory_entry.settings.comparison.relative_settings.pair_distance_step = self.relativeDistStep.value()
        self.trajectory_entry.settings.comparison.relative_settings.pair_distance_unit = UNIT_DICT.get(
            self.relativeDistUnit.currentIndex(), Unit.METER
        )
        self.trajectory_entry.settings.comparison.relative_settings.use_all_pose_pairs = (
            self.relativeAllPairs.isChecked()
        )
