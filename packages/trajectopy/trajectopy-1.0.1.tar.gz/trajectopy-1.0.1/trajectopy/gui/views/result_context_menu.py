...
from trajectopy.gui.models.entries import AlignmentEntry, AbsoluteDeviationEntry, RelativeDeviationEntry
...
class ResultContextMenu(QtWidgets.QMenu):
    ...
    def property_context(self) -> None:
        ...
        property_action.triggered.connect(
            lambda: self.ui_request.emit(
                UIRequest(type=UIRequestType.RES_PROPERTIES, result_selection=self.get_selection())
            )
        )
    ...
    def plot_context(self):
        ...
        if all(isinstance(entry, AbsoluteDeviationEntry) for entry in self.get_selection().entries):
            ...
            plot_action.triggered.connect(
                lambda: self.plot_request.emit(
                    PlotRequest(type=PlotRequestType.MULTI_ABS_DEVIATIONS, result_selection=self.get_selection())
                )
            )
        ...
        if all(isinstance(entry, RelativeDeviationEntry) for entry in self.get_selection().entries):
            ...
            plot_action.triggered.connect(
                lambda: self.plot_request.emit(
                    PlotRequest(type=PlotRequestType.REL_DEVIATIONS, result_selection=self.get_selection())
                )
            )
            self.addAction(plot_action)
    ...
    def plot_single(self):
        ...
        if isinstance(self.get_selection().entries[0], AbsoluteDeviationEntry):
            plot_action.triggered.connect(
                lambda: self.plot_request.emit(
                    PlotRequest(type=PlotRequestType.SINGLE_ABS_DEVIATIONS, result_selection=self.get_selection())
                )
            )
            self.addAction(plot_action)
            ...
            plot_laps_action.triggered.connect(
                lambda: self.plot_request.emit(
                    PlotRequest(type=PlotRequestType.DEVIATION_LAPS, result_selection=self.get_selection())
                )
            )
            if self.get_selection().entries[0].deviations.state.sorting_known:
