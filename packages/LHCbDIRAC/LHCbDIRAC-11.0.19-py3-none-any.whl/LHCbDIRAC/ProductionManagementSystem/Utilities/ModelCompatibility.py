...
def step_to_legacy_dict(i: int, step: ProductionStep):
    ...
    result["Usable"] = "Yes" if step.ready else "Not ready"
...
def step_to_step_manager_dict(i: int, step: ProductionStep):
    ...
    if step.input:
        if isinstance(step.input, list):
            ...
            result["InputFileTypes"] = [
                {"FileType": x.strip(), "Visible": "Y" if step.input.visible else "N"}
                for x in step.input.types.split(",")
            ]
    if isinstance(step.output, list):
        result["OutputFileTypes"] = [{"FileType": x.type, "Visible": "Y" if x.visible else "N"} for x in step.output]
    else:
        result["OutputFileTypes"] = [
            {"FileType": x.strip(), "Visible": "Y" if step.output.visible else "N"}
            for x in step.output.types.split(",")
        ]
    return result
