...
class search_func:
    def GET(self,*d):
        ...
        if 'key' in indata:
            print(indata['key'])
            if 'search_db' not in global_setting:
                global_setting['search_db'] = self.get_search_db()
            rr = global_setting['search_db'].find(indata['key'])
    ...
    def get_search_db(self):
        return yxsfile.file_database(dirname=global_setting['db_dirname'])
