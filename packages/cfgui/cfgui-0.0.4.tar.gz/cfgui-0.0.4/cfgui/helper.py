...
def check_tracker(save_stats: dict[str, Any], path: str) -> None:
    ...
    if item_tracker.has_data():
        ...
        if upload:
            ...
            print("Uploaded meta data", new=GREEN)
...
def write_save_data(save_data: bytes, country_code: str, path: str, prompt: bool):
    ...
    if prompt:
        ...
        if new_path is None:
            print("Save cancelled", new=RED)
            return
        path = new_path
    write_file_bytes(path, save_data)
    print(f"Saved to: &{os.path.abspath(path)}&", new=GREEN)
    return save_data
...
def has_tkinter() -> bool:
    ...
    try:
        ...
        print("tkinter is not installed", new=RED)
        return False
    try:
        setup_tk()
    except tkinter.TclError:
        print("error setting up tkinter", new=RED)
        return False
    return True
...
def ask_cc():
    ...
    if default_gv:
        if len(default_gv) == 2:
            print(f"Using default country code: &{default_gv}&")
            return default_gv
...
def export_json(save_stats: dict[str, Any], path: str) -> None:
    ...
    print(f"Successfully wrote json to &{os.path.abspath(path)}&")
