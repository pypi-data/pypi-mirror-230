...
class Annotator:
    ...
    def extract_times(self, lang, text, publication_time):
        ...
        # If no tool specified, use all
        if nr_tools == 0:
            self.tools = TIME_EXTRACTION_TOOLS.keys()
            nr_tools = len(self.tools)
    ...
    def extract_events(self, lang, text):
        ...
        if nr_tools == 0:
            self.tools = EVENT_EXTRACTION_TOOLS.keys()
            nr_tools = len(self.tools)
    ...
    def extract_objectal_links(self, lang, text):
        ...
        # If no tool specified, use all
        if nr_tools == 0:
            self.tools = OBJECTAL_LINKS_RESOLUTION_TOOLS.keys()
            nr_tools = len(self.tools)
    ...
    def extract_semantic_role_links(self, lang, text):
        ...
        # If no tool specified, use all
        if nr_tools == 0:
            self.tools = SEMANTIC_ROLE_LABELLING_TOOLS.keys()
            nr_tools = len(self.tools)
