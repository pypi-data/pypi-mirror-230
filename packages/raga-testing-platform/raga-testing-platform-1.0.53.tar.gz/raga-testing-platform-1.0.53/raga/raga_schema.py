...
class InferenceSchemaElement(RagaSchemaElement):
    def __init__(self, model:str):
        ...
        if not isinstance(model, str) or not model: 
            raise ValueError(f"{REQUIRED_ARG_V2.format('model', 'non-empty string')}")
        self.type = "inference"
        self.model = model
...
class FMARules():
    ...
    def add(self, 
        ...
        if background_label: rules.update({"background_label":background_label })
        if background_label: rules.update({"include_background":include_background })
...
class Embedding:
    def __init__(self, embedding_val: float):
        from raga import REQUIRED_ARG_V2
        assert isinstance(embedding_val, float), f"{REQUIRED_ARG_V2.format('embedding', 'float')}"
