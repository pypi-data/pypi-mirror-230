...
def auth(credentials: tuple[str, str] = None) -> tuple[str, str, str]:
    ...
    resp = requests.post(
        ...
        headers={'authorization': 'Basic MjAwMDI6M2Nqek5KbW1RTG5JVUZEZGpwdDBDRzlTaWFXSEIzZ21ub2dHTUM3ZA=='},
    )
    ...
    resp = client.get('https://w5.ab.ust.hk/msapi/sis/stdt_ecard/users/%7BstdtID%7D',
                      headers={'authorization': f'Bearer {token["id_token"]}'})
    resp.raise_for_status()
    student_id = resp.json()['stdtID']

    return token['access_token'], token['id_token'], student_id
