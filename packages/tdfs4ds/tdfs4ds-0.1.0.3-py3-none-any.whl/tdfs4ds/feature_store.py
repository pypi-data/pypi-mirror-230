...
def register_features(entity_id, feature_names_types, schema, feature_catalog_name='FS_FEATURE_CATALOG', date_in_the_past = None, end_period='9999-01-01 00:00:00'):
    """
...
def store_feature(entity_id, prepared_features, schema, feature_catalog_name='FS_FEATURE_CATALOG', date_in_the_past = None, end_period='9999-01-01 00:00:00',**kwargs):
    """
    ...
    for i,row in target_tables.iterrows():
        ...
        print(f'insert feature values of new {ENTITY_ID} combinations in {row.iloc[1]}.{row.iloc[0]}')
        if tdml.display.print_sqlmr_query:
            print(query_insert)
        execute_query(query_insert)
        print(f'update feature values of existing {ENTITY_ID} combinations in {row.iloc[1]}.{row.iloc[0]}')
        if tdml.display.print_sqlmr_query:
            print(query_update)
        execute_query(query_update)
...
def build_dataset(entity_id, selected_features, schema, view_name, feature_catalog_name='FS_FEATURE_CATALOG',
                  comment = 'dataset', date_in_the_past = None, **kwargs):
    """
...
def GetTheLargestFeatureID(schema,table_name='FS_FEATURE_CATALOG'):
    ...
    feature_id = execute_query(f'SEL MAX(FEATURE_ID) AS MAX_FEATURE_ID FROM {schema}.{table_name}').fetchall()[0][0]
...
def GetAlreadyExistingFeatureNames(feature_name, schema, table_name='FS_FEATURE_CATALOG'):
    ...
    df = pd.DataFrame({'FEATURE_NAME': feature_name, 'DATA_DOMAIN': data_domain})
...
def upload_features(df, entity_id, feature_names, schema_name, feature_catalog_name='FS_FEATURE_CATALOG',
                   feature_versions='dev.0.0', date_in_the_past=None,end_period='9999-01-01 00:00:00'):
    """
