...
def register_features(entity_id, feature_names_types, schema, feature_catalog_name='FS_FEATURE_CATALOG', date_in_the_past = None, end_period='9999-01-01 00:00:00'):
    ...
    if validtime_statement == 'CURRENT VALIDTIME':
        ...
        query_insert = f"""
        INSERT INTO {schema}.{feature_catalog_name} (FEATURE_ID, FEATURE_NAME, FEATURE_TABLE, FEATURE_DATABASE, FEATURE_VIEW, ENTITY_NAME,ValidStart,ValidEnd)
            SELECT
                NEW_FEATURES.FEATURE_ID
            ,   NEW_FEATURES.FEATURE_NAME
            ,   NEW_FEATURES.FEATURE_TABLE
...
def store_feature(entity_id, prepared_features, schema, feature_catalog_name='FS_FEATURE_CATALOG', date_in_the_past = None, end_period='9999-01-01 00:00:00',**kwargs):
    ...
    for i,row in target_tables.iterrows():
        ...
        if validtime_statement == 'CURRENT VALIDTIME':
            ...
            query_insert = f"""
            INSERT INTO {row[1]}.{row[0]} ({ENTITY_ID}, FEATURE_ID, FEATURE_VALUE, FEATURE_VERSION, ValidStart, ValidEnd)
                SELECT
                    NEW_FEATURES.{ENTITY_ID},
                    NEW_FEATURES.FEATURE_ID,
                    NEW_FEATURES.FEATURE_VALUE,
