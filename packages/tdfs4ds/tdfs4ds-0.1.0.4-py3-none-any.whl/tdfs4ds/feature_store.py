...
def feature_store_catalog_creation(if_exists = 'replace',comment='this table is a feature catalog'):
    ...
    try:
        ...
        if tdml.display.print_sqlmr_query:
            print(query)
        print(f'TABLE {schema}.{feature_catalog_name} has been created')
        ...
        if str(e).split('\n')[0].endswith('already exists.') and (if_exists == 'replace'):
            ...
            try:
                ...
                execute_query(query)
                print(f'TABLE {schema}.{feature_catalog_name} has been re-created')
                if tdml.display.print_sqlmr_query:
                    print(query)
                execute_query(query3)
            except Exception as e:
    ...
    try:
        ...
        if tdml.display.print_sqlmr_query:
            print(query)
        print(f'SECONDARY INDEX ON TABLE {schema}.{feature_catalog_name} has been created')
    except Exception as e:
        print(str(e).split('\n')[0])
...
def GetAlreadyExistingFeatureNames(feature_name):
    """
...
def Gettdtypes(tddf, features_columns):
    """
    ...
    existing_features = GetAlreadyExistingFeatureNames(tddf.columns)
