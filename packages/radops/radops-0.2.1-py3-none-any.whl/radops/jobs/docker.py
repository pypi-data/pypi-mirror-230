...
class DockerJobExecutor(ABC):
    ...
    @abstractmethod
    def run(self, image_name: str, name: str, command: Any) -> str:
        pass
...
class LocalDockerJobExecutor(DockerJobExecutor):
    ...
    def run(self, image_name: str, name: str, command: Any = None) -> None:
        # for some reason can't use the lower level APIClient for this? so we use `DockerClient` instead
        client = docker.from_env()
        client.containers.run(
            image_name, command=command, name=name, detach=True
        )
...
class RemoteDockerJobExecutor(DockerJobExecutor):
    ...
    def run(self, image_name: str, name: str, command: Any = None):
        cmd_str = f"{self.docker_path} run --name {name} -d {image_name}"
