...
class CommandTask(ITask):
    ...
    def __init__(self, interval:_TimeSpan, command:str|list[str]) -> None:
        ...
        self.task_id = md5(commandStringRepresentation.encode()).hexdigest()[:16]
...
class TaskSchedulerManager:
    ...
    def _RunTaskEvent(self, task: ITask, eventFunc: _Callable):
        ...
        taskTitle = task.task_id if not task.task_description else f'{task.task_id}({task.task_description})'
        try:
            output = eventFunc()
