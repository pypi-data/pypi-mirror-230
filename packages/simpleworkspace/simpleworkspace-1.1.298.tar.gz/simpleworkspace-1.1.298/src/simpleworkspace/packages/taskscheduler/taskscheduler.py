...
class ITask(_ABC):
    ...
    properties and methods for Derived classes:
        ...
        * On_Schedule 
    '''
    task_interval = None  # type: _TimeSpan|str
    '''
...
class CommandTask(ITask):
    ...
    def __init__(self, interval:_TimeSpan|str, command:str|list[str]) -> None:
    ...
    def _ScheduledTask_SetNextSchedule(self, task: ITask):
        ...
        if(isinstance(task.task_interval, str)):
            ...
            if not(hasattr(task, '_task_interval_croniter')):
                setattr(task, '_task_interval_croniter', croniter(task.task_interval))
            croniterInstance = getattr(task, '_task_interval_croniter') #type: croniter
            task._task_nextSchedule = croniterInstance.get_next(start_time=_datetime.now())
        else:
