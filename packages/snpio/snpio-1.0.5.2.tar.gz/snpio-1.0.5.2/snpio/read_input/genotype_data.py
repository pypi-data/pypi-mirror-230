...
class GenotypeData:
    ...
    def extract_iqtree_params(self, filename):
        ...
        for i, line in enumerate(lines):
            ...
            if "Rate matrix Q:" in line:
                rate_matrix_data = []
                for j in range(1, 6):  # Adjusting the index to capture 4 rows, allowing for an empty line
                    line_content = lines[i+j].strip()
                    if line_content:  # Skip empty lines
                        rate_matrix_data.append(list(map(float, re.split(r'\s+', line_content)[1:])))
                rate_matrix = pd.DataFrame(rate_matrix_data, columns=['A', 'C', 'G', 'T'], index=['A', 'C', 'G', 'T'])

            # Extract Rate Parameters R
            if "Rate parameter R:" in line:
                rate_params_data = []
                for j in range(1, 8):  # Allowing up to 7 lines to capture 6 values (including a potential empty line)
                    line_content = lines[i+j].strip()
                    if line_content:  # Skip empty lines
                        param, value = line_content.split(":")
                        rate_params_data.append([param.strip(), float(value.strip())])
                rate_params = pd.DataFrame(rate_params_data, columns=['Parameter', 'Value'])
            ...
            if "State frequencies:" in line:
                ...
                for j in range(2, 6):
                    ...
                    state_freq_data.append([param.strip()[3:].strip(')'), float(value.strip())])  # Removing extra characters
                state_freq = pd.DataFrame(state_freq_data, columns=['State', 'Frequency'])
    ...
    def iqtree(self):
        ...
        self._iqtree_params = self.extract_iqtree_params(self.iqtree_filename)
        return self._iqtree_params
    ...
    def iqtree(self, value):
        ...
        if not isinstance(dict, value):
            raise TypeError(f"iqtree property must be a dictionary, but got {type(value)}")
        
        keys = ["rate_matrix", "rate_params", "best_fit_model", "state_freq", "rate_heterogeneity"]

        if any(k not in value for k in keys):
            raise KeyError(f"iqtree object was missing one or more keys. The following keys are required: {keys}")
