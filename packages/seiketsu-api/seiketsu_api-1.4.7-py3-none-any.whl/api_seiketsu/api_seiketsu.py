...
class ApiSeiketsu:
    ...
    # Initialize the Firebase Storage client using service account credentials
    def _get_storage_client(self):
        try:
            credentials_file = os.path.join(os.path.dirname(__file__), 'data', 'firebase_storage_credentials.json')
            credentials = service_account.Credentials.from_service_account_file(credentials_file)
            return storage.Client(credentials=credentials, project=credentials.project_id)
        except Exception as e:
            # Handle any exceptions related to Firebase Storage client creation
            raise Exception(f"Firebase Storage client creation error: {str(e)}")

    # Upload an image to Firebase Storage with a random suffix and return the download link
    def upload_image(self, image_file):
        try:
            ...
            original_image_name = os.path.basename(image_file.name)

            # Combine the original name and random suffix
            image_name = f"{original_image_name}_{random_suffix}"

            # Upload the image to the 'images' folder in Firebase Storage
            blob = self.bucket.blob(f'images/{image_name}')
            blob.upload_from_file(image_file)

            # Generate a signed URL for the uploaded image
            url = blob.generate_signed_url()  # Set the expiration time as needed
            return url
        except Exception as e:
