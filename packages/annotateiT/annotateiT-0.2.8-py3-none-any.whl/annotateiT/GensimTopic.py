...
from nltk.tokenize import word_tokenize  # NLTK kütüphanesinden word_tokenize ekledik
import re
...
class GensimPreprocessor:
    ...
    def preprocess(self):
        self.processed_corpus = [self.custom_preprocess(doc) for doc in self.corpus]  # Özel preprocess işlemini kullan
        return self.processed_corpus

    def custom_preprocess(self, doc):
        # Özel preprocess işlemi, önce metni küçük harfe çevirir, stop kelimeleri kaldırır ve boşlukları temizler.
        # Ardından lemmatization uygularız.
        doc = doc.lower()
        doc = remove_stopwords(doc)
        doc = doc.strip()
        
        # Metni kelimelere ayır ve her kelimeyi lemmatize et
        words = word_tokenize(doc)  # Kelimeleri ayırmak için word_tokenize kullan
