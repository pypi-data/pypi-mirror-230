...
def detect_language(self, field: str):
    ...
    logger.info(f"Detected language in field {field}, added language metadata to '__language'.")
    return self
...
def calc_perplexity(self, field: str):
    ...
    logger.info(f"Calculated perplexity-per-byte in field {field}, added perplexity metadata to '__perplexity'.")
    return self
...
def detect_pii(self, fields: Sequence[str]):
    ...
    self.dataset = self.dataset.map(detect_)
    logger.info(f"Detected PII in fields: {fields}; added __pii__email, __pii__phone, __pii__credential, and __pii__any metadata.")
...
def count_tokens(self, fields: Sequence[str], tokenizer: Optional[str] = None):
    ...
    logger.info(f"Counted tokens in fields: {fields}, added metadata to {field_name}")
