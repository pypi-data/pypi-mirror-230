...
class _PyFuncModelWrapper:
    ...
    def predict(self, text):
        ...
        """
        TODO:
        1) support output level via first row inference and other params X DONE  hackable via nlu rc  --> ADD TO LIB AND RELASE?!
        
        2) support start session with GPU (ENV VAR?) X done hackable --> just matters which jars is in container! --> TEST
        3) support image predictions (OCR MODEL) --> send img as binary?? hackable in nlu rc
        
        
        OCR is integrated but it fails during container creation. Tons of conflicts
