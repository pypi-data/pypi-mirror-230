...
intrnum=CONDOBK{PLAT_BOOK}PG{PLAT_PAGE}",
    ...
    def get_street_address(self, parcel_data: dict) -> str:
        ...
        city = parcel_data.get("PROP_CITY", "")
        if house_number and city:
            street = primary_address.split(house_number)[-1].split(city)[0]
        elif house_number:
            street = primary_address.split(house_number)[-1]
        elif city:
            street = primary_address.split(city)[0]

        return street.replace(",", "").strip()
