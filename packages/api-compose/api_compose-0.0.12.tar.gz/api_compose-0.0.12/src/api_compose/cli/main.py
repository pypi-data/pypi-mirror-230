...
from api_compose import JinjaFunctionsRegistry, safe_import_module
...
from api_compose.cli.options import include_manifest_file_paths_option, include_tags_option, include_models_option, \
    ...
    extra_env_var_option, env_files_pack_option, selector_option
...
from api_compose.core.settings.exceptions import ConfigurationFileException
from api_compose.core.settings.settings import GlobalSettingsModelSingleton, SelectorsSettings, CONFIGURATION_FILE_PATH
...
from api_compose.services.common.exceptions import ManifestIdNotFoundException, ManifestFolderNotFoundException
...
{os.linesep.join(['- ' + entry.name + ' (alias: ' + os.linesep.join(entry.alias) + ') ' + os.linesep for entry in JinjaFunctionsRegistry.registry])}
...
@app.callback()
def init(
        env_files_pack: Annotated[
            str,
            env_files_pack_option,
        ] = None,
):
    ...
    for pack in GlobalSettingsModelSingleton.get().selectors.packs.values():
        pack: SelectorsSettings.SelectorPacksSettings = pack
        try:
    ...
    # Verify env
    if env_files_pack:
        GlobalSettingsModelSingleton.get().env_files.current = env_files_pack
    else:
        logger.warning(
            f'No env files pack is set. Defaulting to {GlobalSettingsModelSingleton.get().env_files.default}')

    # Display Env var
    logger.debug(f'Display Current Env Files Pack: {GlobalSettingsModelSingleton.get().env_files.current}',
                 DiscoveryEvent())
