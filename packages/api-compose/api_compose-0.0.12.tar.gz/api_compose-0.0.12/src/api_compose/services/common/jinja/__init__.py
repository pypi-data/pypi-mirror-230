...
def get_jinja_functions() -> Tuple[Dict, Dict, Dict]:
    globals = convert_dotted_string_to_nested_dict(
        [(name, model.func) for model in JinjaFunctionsRegistry.registry
         for name in model.all_names if model.func_type == JinjaFunctionType.Global])

    filters = {name: model.func for model in JinjaFunctionsRegistry.registry if model.func_type == JinjaFunctionType.Filter
               for name in model.all_names}
    tests = {name: model.func for model in JinjaFunctionsRegistry.registry if model.func_type == JinjaFunctionType.Test for
             name in model.all_names}

    if count_items(globals) + count_items(filters) + count_items(tests) != len([name for entry in JinjaFunctionsRegistry.registry for name in entry.all_names]):
        raise JinjaFunctionNamespaceClashException(JinjaFunctionsRegistry.registry)
