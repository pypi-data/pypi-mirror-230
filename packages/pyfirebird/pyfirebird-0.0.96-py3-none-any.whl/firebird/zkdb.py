...
class ZKDatabase:
    ...
    def register_pipeline(
        ...
        self.zk.create(f"{pipeline_path}/k8s_statefulset_id", "")
        self.zk.create(f"{pipeline_path}/k8s_deployment_id", "")
        return ZKError.OK
    ...
    def set_pipeline_is_running(self, pipeline_id:str, *, is_running:bool, statefulset_id:str=None, deployment_id:str=None) -> ZKError:
        pipeline_path = f"/pipelines/{pipeline_id}"
        if not self.zk.exists(pipeline_path):
            return ZKError.PIPELINE_NOT_REGISTERED
        value, _ = self.zk.get(f"{pipeline_path}/is_running")
        ...
        if is_running:
            self.zk.set(f"{pipeline_path}/k8s_statefulset_id", statefulset_id)
            self.zk.set(f"{pipeline_path}/k8s_deployment_id", deployment_id)
        else:
            self.zk.set(f"{pipeline_path}/k8s_statefulset_id", "")
            self.zk.set(f"{pipeline_path}/k8s_deployment_id", "")
