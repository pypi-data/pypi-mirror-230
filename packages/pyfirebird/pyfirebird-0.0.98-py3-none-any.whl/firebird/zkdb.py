...
class ZKDatabase:
    ...
    def register_pipeline(
        ...
        self.zk.create(f"{pipeline_path}/k8s_state", "{}".encode("utf-8"))
        return ZKError.OK
    ...
    def get_pipeline(self, pipeline_id:str) -> Tuple[ZKError, Any]:
        ...
        value, _ = self.zk.get(f"{pipeline_path}/k8s_state")
        k8s_state = json.loads(value.decode("utf-8"))
    ...
    def set_pipeline_is_running(self, pipeline_id:str, *, is_running:bool, k8s_state:dict={}) -> ZKError:
        pipeline_path = f"/pipelines/{pipeline_id}"
        if not self.zk.exists(pipeline_path):
            return ZKError.PIPELINE_NOT_REGISTERED
        value, _ = self.zk.get(f"{pipeline_path}/is_running")
        ...
        if is_running:
            self.zk.set(f"{pipeline_path}/k8s_state", json.dumps(k8s_state))
        else:
            self.zk.set(f"{pipeline_path}/k8s_state", "{}")
