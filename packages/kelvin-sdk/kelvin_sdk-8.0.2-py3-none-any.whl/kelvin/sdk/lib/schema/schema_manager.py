...
def generate_base_schema_template(project_creation_parameters_object: ProjectCreationParametersObject) -> dict:
    ...
    elif project_creation_parameters_object.app_type == ProjectType.kelvin_v2:
...
def build_kelvin_app_block(project_creation_parameters_object: ProjectCreationParametersObject) -> dict:
    ...
    if project_creation_parameters_object.app_flavour is ApplicationFlavour.default:
        demo_config = yaml.load(DEMO_APP_LITERAL, Loader=yaml.SafeLoader)
        kelvin_block.update(**demo_config)
...
# KEDGE-1015 FIX THIS
DEMO_APP_LITERAL: str = """
    # Asset Configurations
    assets:
      # Asset 1
      - name: demo-cwr-01-booster-compressor
        # Asset Metrics Configuration (remote: true == Cross Node Data Flow)
        metrics:
          demo-cwr-compressor-speed-sp:
            remote: true
          demo-cwr-compressor-temperature:
            remote: true
      ...
      # Asset 2
      - name: demo-cwr-01-gas-compressor
        # Asset Metrics Configuration (remote: true == Cross Node Data Flow)
        metrics:
          demo-cwr-compressor-speed-sp:
            remote: true
    ...
    # Application Inputs (Messages the Application is Consuming)
    inputs:
      - name: demo-cwr-compressor-temperature
        data_type: raw.float64

      # We need to keep track of the real setpoint value in order to avoid sending duplicate Control Changes
      - name: demo-cwr-compressor-speed-sp
        data_type: raw.float64

    # Application Outputs (Messages the Application is Producing)
    outputs:
      - name: demo-cwr-compressor-speed-sp
        data_type: raw.float64
        control_change: true

      - name: demo-cwr-compressor-random-value
        data_type: raw.float64

    # Application Global Configuration (Can be any structure)
    configuration:
      temperature_metric_name: demo-cwr-compressor-temperature
      speed_sp_metric_name: demo-cwr-compressor-speed-sp

    # Asset Parameters (Schema) Definition - Used to build the AI Control UI under Asset Controls (maps)
    parameters:
      - name: temperature_max_threshold
        ...
        schema:
          ...
          minimum: 0.0
          maximum: 200.0
      - name: temperature_min_threshold
        data_type: raw.float64
        default:
          value: 0
        schema:
          title: "Temperature Min Threshold"
          minimum: 0.0
          maximum: 100.0
      - name: recommended_speed_setpoint
        data_type: raw.float64
        default:
          value: 1000
        schema:
          title: "Recommended Speed SetPoint"
          minimum: 0
          maximum: 2000
      - name: closed_loop
