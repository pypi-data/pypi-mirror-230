...
from .constants import DEFAULT_SCHEMA, MATERIALIZED_VIEW, MATERIALIZED_VIEW_INDEX
...
def _get_partitions(
    ...
    statement = sa.text(
        f"SELECT inhrelid::regclass AS partition, inhparent::regclass AS table_name FROM pg_catalog.pg_inherits WHERE inhparent IN ({regclasses})")
...
def create_view(
    ...
    partitions_mapping = {}
    for partition_row in fetchall(_partitions(models, schema, tables)):
