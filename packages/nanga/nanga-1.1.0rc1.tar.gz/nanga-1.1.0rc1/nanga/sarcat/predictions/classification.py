...
from nanga.sarcat.models.legacy.modules import load_model
from nanga.sarcat.models.s2fos import S2FOS, LABELS
from nanga.sarcat.models.legacy.utils import get_mcat_dict
from nanga.sarcat.predictions.utils import data_loader
...
class Predictor(object):
    ...
    def __init__(self, model_name=None, model_path=None):
        ...
        if os.path.isdir(model_path) and model_name == 's2fos':
            print('load s2fos model')
            self.model = self._load_model(self.model_path)
        elif model_name == 'legacy' and os.path.isfile(model_path):
            self.model, _ = self._load_model(self.model_path)
        elif model_path is not None and not os.path.isfile(model_path):
    ...
    def _load_model(self, model_path=None):
        ...
        """
        if self.model_name == 's2fos':
            return S2FOS(self.model_path)
        return sarcat_model(self.model_path)
...
#         return (ids, labels[0], confidences[0])
    ...
    def get_labels(self, title=None, abstract=None, papers=None, n=1):
        """
        Given a title and an abstract, get the category.
        """
        if not papers is None and self.model_name=='s2fos':
            return self.model.predict(papers)
        if title is None or abstract is None:
            logging.error("Title or abstract must not be None.")
            return {"categories": None, "confidences":None}
        preds = self.model.predict(title, abstract)
...
class Predictors(Predictor):
    ...
    def predict_batch(self, papers: list = []):
        """
        Predictions from a list of papers.
         :param papers: list of dict
        """
        if self.model_name == 's2fos':
            categories = self.get_labels(papers=papers)
            print(categories)
            res = []
            for category in categories:
                try:
                    res.append(max(category, key=category.get))
                except:
                    res.append(None)
            return res
        if self.model_name == 'legacy':
            titles, abstracts = [], []
            for paper in papers:
                title = paper['title']
                titles.append(paper.get('title', ''))
                abstracts.append(paper.get('abstract', ''))
            return self.from_list(titles=titles, abstracts=abstracts)


   
        
        
    def from_list(self, titles:list = [], abstracts:list = [], batch_size:int = 16):
        """
