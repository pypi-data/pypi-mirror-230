...
class RnaView(GenomeView):
    ...
    def draw(self, individual, display_legend, outdir, title):
        ...
        self.ax.plot("grey", color="black", label="Coding RNA : " + str(number_is_coding))
        self.ax.plot("black", color="lightgrey", label="Non coding RNA : " + str(number_is_not_coding))

        # Display the legend
        if (display_legend == True):
            self.ax.legend(title="Genome length : " + str(individual.genome_length) + " bp", loc=(0.85, 0))
        ...
        if (rna.strand == "LEADING"):
            ...
            self.ax.add_patch(
                Arc(center, diameter + offset, diameter + offset,
                    angle=90, theta1=-end_angle, theta2=-begin_angle,
                    linewidth=3, color=color))
            # Draw decorator
            self.ax.add_patch(
                Arc(center, diameter + offset, diameter + offset,
                    angle=90, theta1=-end_angle, theta2=-term_angle + 1,
                    linewidth=5, color=color))
        else:
            # Draw main arc
            self.ax.add_patch(
                Arc(center, diameter + offset, diameter + offset,
                    angle=90, theta1=-begin_angle, theta2=-end_angle,
                    linewidth=3, color=color))
            # Draw decorator
            self.ax.add_patch(
                Arc(center, diameter + offset, diameter + offset,
                    angle=90, theta1=-term_angle - 1, theta2=-end_angle,
                    linewidth=5, color=color))
