...
class TestE2EConsolidationModeSnapshot(TestE2EBase):
  ...
  def test_insert(self, source_file_path, file_format, expected_rows):
      ...
      self.assertTrue(E2EUtils.get_table(self._spark, provider.database_name, f"{entity.table_name.lower()}validationerrors"))
  ...
  @parameterized.expand([
   ("parquet 2 assets",'files/countries_insert.parquet', "parquet", "countries.parquet", 2, 200),
   ("parquet 5 assets",'files/countries_insert.parquet', "parquet", "countries.parquet", 5, 500),
   ("csv 2 assets",'files/countries_insert.csv', "csv", "countries.csv", 2, 200),
   ("csv 5 assets",'files/countries_insert.csv', "csv", "countries.csv", 5, 500),
  ])  
  def _test_multiple_insert_full_load_same_content_different_date(self, _, source_file_path : str, file_format :str, destination_path : str, iterations : int, expected_rows : int):
     #ARRANGE
    provider, entity, destination_path = self._fixture._prepare_countries_metadata(source_file_path, file_format, recreate_tables = True, with_primary_key = True, consolidation_mode = Constants.CONSOLIDATION_MODE_SNAPSHOT)
    ...
    #different sources
    self._spark.catalog.setCurrentDatabase(provider.database_name)
    result = self._spark.sql(f"SELECT COUNT(DISTINCT IdSourceItem) as rows FROM {entity.table_name.lower()}")
    value = result.select("rows").collect()
    count = value[0].asDict()["rows"]
    self.assertEqual(count, iterations)
    
  @parameterized.expand([
  ('files/countries_insert.parquet', "parquet", "countries.parquet", 100),
  ('files/countries_insert.csv', "csv", "countries.csv", 100),
  ('files/countries_insert.xls', "xls", "countries.xls",   100),
  ('files/countries_insert.xlsm', "xlsm", "countries.xlsm", 100),
  ('files/countries_insert.xlsx', "xlsx", "countries.xlsx",  100)
  ])
  def _test_multiple_insert_full_load_same_content_same_date(self, source_file_path, file_format, destination_file_name, expected_rows):
    #ARRANGE
  ...
  def _test_update(self, source_file_path, file_format, destination_file_name, update_source, expected_rows):
    #ARRANGE
    provider, entity, destination_path = self._fixture._prepare_countries_metadata(source_file_path, file_format, destination_file_name = destination_file_name, recreate_tables = True, with_primary_key = True, consolidation_mode = Constants.CONSOLIDATION_MODE_SNAPSHOT)
  ...
  def _test_no_metadata_and_partition_fields(self, source_file_path, file_format, destination_file_name, update_source, expected_rows):
    #ARRANGE
    attributes = []
  ...
  @parameterized.expand([
  ('files/countries_insert.parquet', "parquet", 100),
  ])
  def _test_insert_with_assets_is_full_extract(self, source_file_path, file_format, expected_rows):
     #ARRANGE
    provider, entity, destination_path = self._fixture._prepare_countries_metadata(source_file_path, file_format, recreate_tables = True, with_primary_key = True, consolidation_mode = Constants.CONSOLIDATION_MODE_SNAPSHOT)
    #Data table is created
    self.assertTrue(E2EUtils.get_table(provider.database_name, entity.table_name.lower()))  
    #Validation errors table is created
    self.assertTrue(E2EUtils.get_table(provider.database_name, f"{entity.table_name.lower()}validationerrors"))
