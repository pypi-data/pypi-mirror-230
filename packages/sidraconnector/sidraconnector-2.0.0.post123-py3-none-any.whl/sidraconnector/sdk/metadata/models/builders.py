...
class EntityBuilder():
  ...
  def with_default_regular_expression(self, provider_name, entity_name, format):
    ...
    self.regular_expression = "^{provider_name}_{entity_name}_{time_stamp_regular_expression}?.{format}".format(provider_name=provider_name, entity_name=entity_name, time_stamp_regular_expression=regular_expression_datetime, format=format)
    return self
...
class ReaderPropertiesBuilder():
  def __init__(self):
    ...
    self.reader_options = json.loads('{}')
    self.header_lines = 0
  ...
  def with_reader_options(self, reader_options_json):
