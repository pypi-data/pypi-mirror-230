from promptmanager.runtime.flow import PMRuntime, PMNodeOutput
import logging
...
class PMPromptRunner:
    ...
    def exec(self) -> str:
        ...
        result = customLLM.request_result_by_message(prompt.message(self.prompt_variables), self.model_params)
        return result
...
def run(params: dict, inputs: dict, outputs: dict) -> PMNodeOutput:
    ...
    runner = PMPromptRunner(template_content, role_prompt, prompt_variables, model_conf, model_param_define,
                            model_params)
    result = runner.exec()
    # result = "resultresultresultresultresultresultresultresultresultresultresult"
