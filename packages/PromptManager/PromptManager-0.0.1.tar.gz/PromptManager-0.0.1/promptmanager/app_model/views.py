...
def get_model_config(request):
    ...
    model_config = base.MODEL_DEFAULT_CONFIG
...
def parse_model_params(request):
    ...
    type = post_params.get('type')  # OpenAI/Others判断
    ...
    param_result = []
    default_params = json.loads(base.MODEL_DEFAULT_PARAMS)
    if model_id:
...
def generate_template_model_param():
    try:
        default_model = Model.objects.get(is_default=1)
    except Exception:
    ...
    model_dict = {}
    if default_model.params:
        params = JsonUtil.json_to_dict(default_model.params)
        model_params_value.extend(params)

        model_dict["params_define"] = params

    if default_model.config:
        model_dict["config"] = default_model.config
