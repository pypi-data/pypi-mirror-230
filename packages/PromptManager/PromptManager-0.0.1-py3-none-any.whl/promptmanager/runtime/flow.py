...
class PMFlowInputNode(PMFlowNode):
    def __init__(self):
        ...
        self.name = "input"
        self.module_id = "00000000-0000-0000-aaaa-000000000002"
...
class PMFlowOutputNode(PMFlowNode):
    def __init__(self):
        self.id = str(uuid.uuid4())
        self.name = "output"
        self.module_id = "00000000-0000-0000-aaaa-000000000003"
...
class PMFlowTemplateNode(PMFlowNode):
    def __init__(self, id=None, name=None, template: PMPromptTemplate = None, model=None, model_params_value={},
                 **kwargs):
    ...
    def get_script_module_info(self):
        module_name = "define_prompt"
        module_path = Path(__file__).resolve().parent.parent / "script/define_prompt.py"
        return module_name, module_path

    @staticmethod
    def from_template(name=None, template: PMPromptTemplate = None, model=None, model_params_value={}):
...
class PMFlowScriptNode(PMFlowNode):
    ...
    def __init__(self, id=None, name=None, path=None, module_id=None, module_name=None, module_type=None, params=[],
                 inputs=[],
                 outputs=[],
                 **kwargs):
        super().__init__(kwargs)
    ...
    def get_script_path(self) -> str:
        return self.script_path if self.script_path else self.params['script']['value']

    def get_script_module_info(self):
        module_path = self.get_script_path()
...
class PMFlow(object):
    def __init__(self, id=None, name=None, nodes: list[PMFlowNode] = [], edges: list = []):
        if id:
            self.id = id
        else:
            self.id = str(uuid.uuid4())
    ...
    def get_flow_input_params(nodes, edges):
        ...
        for direct_connect_edge in direct_connect_edges:
            ...
            new_variable = {
                ...
                "defaultValue": target_input['defaultValue']
            }
            if params:
                for param in params:
    ...
    def _run_node_down(self, current_node, node_output_dict: dict):
        for current_edge in self.edges:
            if current_edge.source_node == current_node.id:
                ...
                # run node
                if self.__get_node_status(target_node.id) == PMNodeStatus.QUEUED.name:
    ...
    def create_pmruntime_and_run(self, node, node_output_dict) -> PMNodeOutput:
        ...
        module_name, module_path = node.get_script_module_info()
        ...
        try:
            ...
            self.__set_node_status(node.id, PMNodeStatus.FAILED.name, error_msg=getattr(e, 'message', ''))
            logger.error(getattr(e, 'message', ''))
            raise flow_exception.FLOW_RUN_EXCEPTION

    def generate_flow_result_path(self):
        return Path(__file__).resolve().parent.parent / ("flow/%s/flow_running.info" % self.id)

    def generate_flow_output_path(self):
        return Path(__file__).resolve().parent.parent / ("flow/%s/output.result" % self.id)

    def write_flow_result(self):
        file_path = self.generate_flow_result_path()
        FileUtil.write(file_path, PMCommonUtil.object_to_json(self.flow_result))
    ...
    def run_output_node(self, node, node_output_dict: dict):
        ...
        # write to file
        output_result_path = Path(__file__).resolve().parent.parent / ("flow/%s/output.result" % self.id)
        ...
        self.flow_result.end_time = time.time()
        self.flow_result.status = PMFlowStatus.SUCCESS.name
        self.flow_result.outputs = node.inputs
    ...
    def check_run_variables(params: list, variables: list[dict]):
        ...
        if params:
            for param in params:
                ...
                elif param['type'] != variable_dict[param_name]['type']:
                    is_pass = False
                    illegal_variables.append(param_name)
                else:
                    pass
    ...
    def get_result(self, output_name=None, wait_finish=True):
        while wait_finish and (
                self.flow_result.status != PMFlowStatus.SUCCESS.name or self.flow_result.status != PMFlowStatus.FAILED.name):
            # running_node = None
    ...
    def load(file_path):
        ...
        edges = flow_dict['config']['edges']
        nodes = flow_dict['config']['nodes']
        return PMFlow(name=flow_dict['name'], nodes=nodes, edges=edges)
...
class PMFlowResult(object):
    def __init__(self, flow_id=None, flow_name=None, start_time=None, end_time=None, status=None,
                 nodes_info: list[PMFlowNodeRunningInfo] = [], outputs: list[PMNodeOutput] = []):
