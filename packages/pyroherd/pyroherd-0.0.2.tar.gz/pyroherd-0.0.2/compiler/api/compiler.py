...
def get_docstring_arg_type(t: str, is_list: bool = False, is_pyroherd_type: bool = False):
    if t in CORE_TYPES:
        if t == "long":
            return "``int`` ``64-bit``"
        elif "int" in t:
        ...
        return "Any method from :obj:`~pyroherd.raw.functions`"
...
def get_references(t: str, kind: str):
    ...
    if t:
        return "\n            ".join(
            f"- :obj:`{i} <pyroherd.raw.functions.{i}>`"
            for i in t
        ), len(t)
...
def start(format: bool = False):
    ...
    for qualtype in types_to_constructors:
        ...
        items = "\n            ".join([f"- :obj:`{c} <pyroherd.raw.types.{c}>`" for c in constructors])
        ...
        for i, arg in enumerate(sorted_args):
            ...
            docstring_args.append(
                "{}{}: {}".format(
                    ...
                    get_docstring_arg_type(arg_type, is_pyroherd_type=c.namespace == "pyroherd")
                )
            )

        if c.section == "types":
            docstring += f"This object is a constructor of the base type :obj:`~pyroherd.raw.base.{c.qualtype}`.\n\n"
        else:
