...
class Hurdle_for_AdaSTEM(BaseEstimator):
    ...
    def predict(self, 
                ...
                njobs:int = 1,
                verbosity: int=1,
                ensmeble_index: Union[None, int]=None) -> np.ndarray:
        """Predict
        ...
        """
        if verbosity==0:
            cls_res = self.classifier.predict(X_test, njobs=njobs, verbosity=0, ensmeble_index=ensmeble_index)
            reg_res = self.regressor.predict(X_test, njobs=njobs, verbosity=0, ensmeble_index=ensmeble_index)
        else:
            cls_res = self.classifier.predict(X_test, njobs=njobs, verbosity=1, ensmeble_index=ensmeble_index)
            reg_res = self.regressor.predict(X_test, njobs=njobs, verbosity=1, ensmeble_index=ensmeble_index)
        # reg_res = np.where(reg_res>=0, reg_res, 0) ### we constrain the reg value to be positive
    ...
    def predict_proba(self, 
        ...
        return self.predict(self, X_test, njobs=njobs, verbosity=verbosity, ensmeble_index=ensmeble_index)