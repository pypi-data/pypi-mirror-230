...
class AdaSTEM(BaseEstimator):
    ...
    def predict_proba(self,
                      ...
                      ensmeble_index: Union[None, int]=None) -> Union[np.ndarray, Tuple[np.ndarray]]:
        """Predict probability
        ...
        Args:
            ...
            ensmeble_index (Union[None, int], optional):
                Test function. Only predict on ensemble X.
        ...
        Unique_Ensemble_Indexes = list(self.ensemble_df.ensemble_index.unique()) if ensmeble_index is None else [ensmeble_index]
        
        for ensemble in Unique_Ensemble_Indexes:
        ...
        if not ensmeble_index is None:
            pred_mean = res_not_nan_count.values
            pred_std = np.array([np.nan] * len(res_mean.values))
        else:
    ...
    def predict(self,
        ...
        return self.predict_proba(X_test, verbosity=verbosity, return_std=return_std, njobs=njobs, aggregation=aggregation, ensmeble_index=ensmeble_index)
...
class AdaSTEMClassifier(AdaSTEM):
    ...
    def predict(self, 
        ...
        if return_std:
            mean, std = self.predict_proba(X_test, verbosity=verbosity, return_std=True, njobs=njobs, aggregation=aggregation, ensmeble_index=ensmeble_index)
            mean = np.where(mean<cls_threashold, 0, mean)
            mean = np.where(mean>=cls_threashold, 1, mean)
            return mean, std
        else:
            mean = self.predict_proba(X_test, verbosity=verbosity, return_std=False, njobs=njobs, aggregation=aggregation, ensmeble_index=ensmeble_index)
            mean = np.where(mean<cls_threashold, 0, mean)
            mean = np.where(mean>=cls_threashold, 1, mean)
            return mean
