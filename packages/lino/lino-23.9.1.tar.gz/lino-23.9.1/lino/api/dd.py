...
def schedule_often(every=10):
    def decorator(func):
        if settings.SITE.use_linod:
            ...
            Procedures.add_item_lazy(func.__name__, func.__name__, func.__name__, every_unit='secondly',
                                                 every_value=every, func=func)
...
def schedule_daily():
    def decorator(func):
        if settings.SITE.use_linod:
            from lino.modlib.linod.choicelists import Procedures
            Procedures.add_item_lazy(func.__name__, func.__name__, func.__name__, every_unit='daily',
                                                 every_value=1, func=func)
        return func
