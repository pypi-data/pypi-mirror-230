...
def get_embedding_for_tflite_with_label_taekwondo(poses, embedding, labels):
    embedding_data = []
    pose_data = []
    label_data = []
    for label, pose in zip(labels, poses):
...
def get_embedding_for_tflite_taekwondo(pose, embedding):
    return getPoseEmbeddingList(pose, embedding.upper())
