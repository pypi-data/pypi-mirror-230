...
def test_cut(df0_long, column, bins, right, labels, retbins, device):
    tdf = df0_long.to(device)
    df = tdf.to_pandas()
    
    output = tdfl.cut(tdf[column], bins, right=right, labels=labels, retbins=retbins)
    ...
    if labels is None:
        ...
        expected = expected.cat.codes.astype(float)
        expected[expected == -1] = np.nan

    np.testing.assert_allclose(ar.Array(output).detach().cpu().numpy(), expected.to_numpy())


@pytest.mark.parametrize(
    'ltdf, rtdf, on, left_on, right_on, suffixes, grads',
    [
        ('df0', 'df1a', None, None, None, ('_x', '_y'), (True, True, False, True, True, True)),
        ('df0', 'df1a', ('a', 'b'), None, None, ('_0', '_1'), (True, True, False, True, True, False, True)),
        ('df0', 'df1b', None, None, None, ('_x', '_y'), (True, False, False, True, True, False, False)),
        ('df0', 'df1b', None, ('a', 'b'), ('b', 'a'), ('_x', '_y'), (True, False, False, True, True, False, False)),
    ],
)
@pytest.mark.parametrize(
    'how',
...
@pytest.mark.parametrize('device', devices)
def test_merge(request, ltdf, rtdf, on, left_on, right_on, suffixes, how, sort, indicator, grads, device):
    ...
    tdf = ltdf.merge(rtdf, how=how, on=on, left_on=left_on, right_on=right_on, sort=sort, suffixes=suffixes, indicator=indicator)
