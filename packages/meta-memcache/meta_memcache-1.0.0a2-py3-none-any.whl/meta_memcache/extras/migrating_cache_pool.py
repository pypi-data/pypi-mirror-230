...
class MigratingCachePool(HighLevelCommandsMixin):
    """
    ...
    def __init__(
        self,
        origin_pool: CacheApiProtocol,
        destination_pool: CacheApiProtocol,
        migration_mode_config: Union[MigrationMode, Dict[MigrationMode, int]],
        default_read_backfill_ttl: int = 3600,
    ) -> None:
        # Share the WriteFailureEvent across all pools
        self.on_write_failure = WriteFailureEvent()
        origin_pool.on_write_failure = self.on_write_failure
        destination_pool.on_write_failure = self.on_write_failure

        self._origin_pool: CacheApiProtocol = origin_pool
        self._destination_pool: CacheApiProtocol = destination_pool
    ...
    def meta_multiget(
        ...
        if migration_mode >= MigrationMode.USE_DESTINATION_UPDATE_ORIGIN:
            return self._destination_pool.meta_multiget(
                keys=keys,
                flags=flags,
                int_flags=int_flags,
            ...
            results = self._origin_pool.meta_multiget(
                keys=keys,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
            ...
            return self._origin_pool.meta_multiget(
                keys=keys,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
    ...
    def meta_get(
        ...
        migration_mode = self.get_migration_mode()
        if migration_mode >= MigrationMode.USE_DESTINATION_UPDATE_ORIGIN:
            return self._destination_pool.meta_get(
                key=key,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
            ...
            result = self._origin_pool.meta_get(
                key=key,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
            ...
            return self._origin_pool.meta_get(
                key=key,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
    ...
    def meta_set(
        ...
        if migration_mode < MigrationMode.ONLY_DESTINATION:
            origin_response = self._origin_pool.meta_set(
                key=key,
                value=value,
                ttl=ttl,
                flags=flags,
        ...
        if migration_mode > MigrationMode.ONLY_ORIGIN:
            destination_response = self._destination_pool.meta_set(
                key=key,
                value=value,
                ttl=ttl,
                flags=flags,
    ...
    def meta_delete(
        ...
        migration_mode = self.get_migration_mode()
        if migration_mode < MigrationMode.ONLY_DESTINATION:
            origin_response = self._origin_pool.meta_delete(
                key=key,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
        ...
        if migration_mode > MigrationMode.ONLY_ORIGIN:
            destination_response = self._destination_pool.meta_delete(
                key=key,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
    ...
    def meta_arithmetic(
        ...
        migration_mode = self.get_migration_mode()
        if migration_mode >= MigrationMode.USE_DESTINATION_UPDATE_ORIGIN:
            return self._destination_pool.meta_arithmetic(
                key=key,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
            ...
            return self._origin_pool.meta_arithmetic(
                key=key,
                flags=flags,
                int_flags=int_flags,
                token_flags=token_flags,
    ...
    def touch(
        ...
        it is used to ensure cache consistency, so we will send it to both pools.
        """
        origin_response = destination_response = None
        migration_mode = self.get_migration_mode()
        if migration_mode < MigrationMode.ONLY_DESTINATION:
            origin_response = self._origin_pool.touch(
                key=key, ttl=ttl, no_reply=no_reply
            )
        if migration_mode > MigrationMode.ONLY_ORIGIN:
            destination_response = self._destination_pool.touch(
                key=key, ttl=ttl, no_reply=no_reply
            )
        if migration_mode >= MigrationMode.USE_DESTINATION_UPDATE_ORIGIN:
            assert destination_response is not None  # noqa: S101
    ...
    def get_counters(self) -> Dict[ServerAddress, PoolCounters]:
        counters = self._origin_pool.get_counters()
        counters.update(self._destination_pool.get_counters())
        return counters
