...
def annotate_genes(input_faa, output_dir='.', bit_score_threshold=60, rbh_bit_score_threshold=350,
    ...
    # Remove files matching the pattern "*mmsdb*" from the sample directory
    for fasta_loc in fasta_locs:
        fasta_name = get_fa_db_name(fasta_loc)
        fasta_dir = path.join(tmp_dir, fasta_name)
        print(f"Removing '*mmsdb*' files from {fasta_dir}:")
    ...
    # Debug print to verify directory contents
    print(f"Contents of {fasta_dir} after removal:")
