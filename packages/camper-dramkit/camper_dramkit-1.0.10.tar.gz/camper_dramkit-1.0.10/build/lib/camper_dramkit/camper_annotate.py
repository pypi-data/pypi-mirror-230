...
def annotate_genes(input_faa, output_dir='.', bit_score_threshold=60, rbh_bit_score_threshold=350,
    ...
    # Now, let's remove the files from the copied directory
    copied_fasta_dir = path.join(tmp_dir, f"{fasta_name} (copy)")
    print(f"Removing '*mmsdb*' files from {copied_fasta_dir} (copy):")

    for file_name in os.listdir(copied_fasta_dir):
        if "mmsdb" in file_name:
            file_path = os.path.join(copied_fasta_dir, file_name)
            if os.path.isfile(file_path):
                os.remove(file_path)
                print(f"Removed: {file_name}")
