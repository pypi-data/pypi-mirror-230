...
class SampleHandler:
    ...
    def split_train_val_data_of_yolov8(all_image_path, all_label_path, dest_path, class_name, train_radio, valid_radio,
        ...
        SampleHandler.copy_data_to_dest(train_img_list, "train", all_label_path, yolo_image_dir, yolo_label_dir)
        SampleHandler.copy_data_to_dest(valid_img_list, "val", all_label_path, yolo_image_dir, yolo_label_dir)
        SampleHandler.copy_data_to_dest(test_img_list, "test", all_label_path, yolo_image_dir, yolo_label_dir)
    ...
    @staticmethod
    def copy_data_to_dest(img_list, img_usage, all_label_path, yolo_image_dir, yolo_label_dir):
        # 复制数据到指定目录
        for img_path in img_list:
            img_name_prefix = FileHelper.get_file_name_prefix(img_path)
            label_txt_path = os.path.join(all_label_path, img_name_prefix + ".txt")
            shutil.copy(img_path, os.path.join(yolo_label_dir, img_usage + "2017"))
            shutil.copy(label_txt_path, os.path.join(yolo_image_dir, img_usage + "2017"))
