...
class BaseClassifierConfig:
    ...
    Args:
        ...
        max_epochs (int): max epoch for training
        seed (int): random seed
        num_class (int): number of class
        output_dir (str): output directory
        device (str): device name, e.g. 'cpu', 'gpu'
        use_device_list (Union[str, List[int]]): device list
        num_workers (int): number of workers
        fc_classifier (Union[str, nn.Module]): fully connected classifier
        label_smoothing_factor (float): label smoothing factor
        gradient_accumulation_steps (int): gradient accumulation steps
        print_topk_list (list): topk list for printing
        monitor_topk (Union[int, str]): topk for monitoring
        save_ckpt_topk (int): topk for saving checkpoint
        check_val_every_n_epoch (int): check validation every n epoch
        num_sanity_val_steps (int): number of sanity validation steps
        precision (str): precision, e.g. '16-mixed', '32'
        label_name_list (list): label name list
        criterion (str): criterion name
        problem_type (str): problem type, e.g. 'single_label_classification', 'multi_label_classification'
    """
