...
class BiFPNBlock(nn.Module):
    ...
    def forward(self, inputs: list[Tensor]) -> list[Tensor]:
        ...
        for i in range(num_inputs - 1):
            assert inputs[i].shape[2] == inputs[i + 1].shape[2] * 2
            assert inputs[i].shape[3] == inputs[i + 1].shape[3] * 2
        ...
        features = [feature]
        for i in range(num_inputs - 1):
            feature = self.in_convs[i](w1[0, i] * inputs[i + 1] + w1[1, i] * F.interpolate(feature, scale_factor=0.5))
            features.append(feature)

        # Computes upward path.
        out_features = [feature]
        for i in range(num_inputs - 2, -1, -1):
            feature = self.out_convs[i](
                w2[0, i] * inputs[i] + w2[1, i] * features[i] + w2[2, i] * F.interpolate(feature, scale_factor=2)
            )
...
class BiFPN(nn.Module):
    ...
    def __init__(self, input_size: list[int], feature_size: int = 64, num_layers: int = 2) -> None:
        super().__init__()
        ...
        for _ in range(num_layers):
            bifpns.append(BiFPNBlock(feature_size, len(input_size)))
