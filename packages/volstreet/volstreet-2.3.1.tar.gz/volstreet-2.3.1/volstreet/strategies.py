    ...
    for index in indices:
        try:
            vs.load_combine_save_json_data(
                ...
                file_name=f"{index.name}_intraday_strangle.json",
                user_id=user,
            ...
            vs.notifier(
                f"Appending strangle data failed for {index.name}: {e}",
                discord_webhook_url,
            )
...
def overnight_straddle_nifty(
    ...
    try:
        vs.load_combine_save_json_data(
            new_data=nifty.strategy_log[strategy_tags[0]],
            file_name=f"NIFTY_overnight_short_straddle.json",
            user_id=user,
            notification_url=discord_webhook_url,
            default_structure=list,
        )
        vs.load_combine_save_json_data(
            new_data=nifty.strategy_log[strategy_tags[1]],
            file_name=f"NIFTY_weekly_hedge.json",
            user_id=user,
            notification_url=discord_webhook_url,
            default_structure=list,
        )
    ...
    for index in indices:
        try:  # Remove this try except block after testing
            vs.append_data_to_json(
                index.strategy_log[strategy_tag],
                f"{user}_{index.name}_intraday_trend.json",
            )
            vs.load_combine_save_json_data(
                new_data=index.strategy_log[strategy_tag],
                file_name=f"{index.name}_intraday_trend.json",
                user_id=user,
                notification_url=discord_webhook_url,
                default_structure=list,
            )
