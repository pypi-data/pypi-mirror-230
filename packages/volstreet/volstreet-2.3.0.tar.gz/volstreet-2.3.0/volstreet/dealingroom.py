...
class Index:
    ...
    def overnight_straddle(
        ...
        if (
            ...
            notifier(
                f"Invalid strike found for {self.name}.", notification_url, "ERROR"
            )
    ...
    def intraday_strangle(
        ...
        def justify_stop_loss(info_dict, side):
            ...
            unjust_increase = (
                actual_price / simulated_option_price
...
def combine_data(
    ...
    if not existing_data and isinstance(existing_data, list):
...
def notifier(message, webhook_url=None, level="INFO"):
    ...
    if levels.index(level) < levels.index(NOTIFIER_LEVEL):
        return
