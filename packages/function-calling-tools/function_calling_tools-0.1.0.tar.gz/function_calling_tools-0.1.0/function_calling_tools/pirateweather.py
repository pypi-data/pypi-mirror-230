...
def fetch_weather_info(latitude, longitude, unix_timestamp):
    ...
    url = f"{API_ENDPOINT}{PIRATE_WEATHER_API_KEY}/{latitude},{longitude},{unix_timestamp}?exclude=minutely,hourly&units=si"
...
class WeatherFetcher(BaseTool):
    ...
    def _run(self, latitude: float, longitude: float, unix_timestamp: float):
        return fetch_weather_info(latitude, longitude, unix_timestamp)
