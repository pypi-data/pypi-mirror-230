    ...
    fn py_optimise_predictions(predictions: PyReadonlyArray3<f64>,
        ...
        for (i, p) in pricing_models.into_iter().enumerate() {
            let model = match p {
                ...
                "RI1Y" => PricingModel::Reservations(Term::OneYear, prices.slice(s![i, ..]), null_base_level.view()),
                "RI3Y" => PricingModel::Reservations(Term::ThreeYears, prices.slice(s![i, ..]), null_base_level.view()),
                "CurrentSP" => PricingModel::SavingsPlans(Term::AlreadyPayed, prices.slice(s![i, ..]), sp_levels.slice(s![.., sp_index])),
                "SP1Y" => PricingModel::SavingsPlans(Term::OneYear, prices.slice(s![i, ..]), null_base_level.slice(s![.., 0])),
                "SP3Y" => PricingModel::SavingsPlans(Term::ThreeYears, prices.slice(s![i, ..]), null_base_level.slice(s![.., 0])),
                _ => panic!("Not a known priving model")
            };

            match model {
                PricingModel::Reservations(Term::OneYear, _, _) | PricingModel::Reservations(Term::ThreeYears, _, _) => {
                    space.reserve(n);
                    for k in max_usage.iter() {space.push(Range{ start: 0., end: k / period + 0.0000001})};
                    j += n;
                },
                PricingModel::SavingsPlans(Term::OneYear, sp_prices, _) |  PricingModel::SavingsPlans(Term::ThreeYears, sp_prices, _) => {
                    j += 1;
        ...
        starts.axis_iter(Axis(1))
            ...
            // .expect("hjhjkg")
            .map_init(
                || CostPredictionFunction::new(predictions.clone(), &models, b, record),
        ...
        let coverages: Array1<f64> = predictions.axis_iter(Axis(0))
                .map(|pred| final_coverage(pred.view(), &models, x.view(), b, &mut dump))
                .collect();
