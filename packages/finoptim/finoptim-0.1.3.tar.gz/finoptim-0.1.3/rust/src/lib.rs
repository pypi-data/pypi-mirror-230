...
use crate::cost_utils::{cost, cost_final, coverage, final_coverage};
use crate::optimisers::{best_optimiser, best_optimiser_with_details, inertial_optimiser, Results};
...
fn rust_as_backend(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    ...
    fn py_simple_optimiser<'py>(
        ...
        let res = match convergence_details {
            Some(b) if b => {
                best_optimiser_with_details(usage.to_owned(), prices.to_owned(), t, Some(start))
            }
    ...
    fn py_optimise_predictions(
        ...
        for (i, p) in pricing_models.into_iter().enumerate() {
            let model = match p {
                ...
                "CurrentSP" => PricingModel::SavingsPlans(
                    Term::AlreadyPayed,
                    prices.slice(s![i, ..]),
    ...
    fn py_coverage(
        ...
        let z = match levels.dim().into_dimension().ndim() {
            1 => {
                ...
                let one_dim_levels = levels.into_dimensionality::<Ix1>().unwrap();
                let two_dim_levels = Array2::zeros((i, j + 1)) + &one_dim_levels;
                Some(coverage(usage.view(), prices.view(), two_dim_levels.view()))
            }
            2 => Some(coverage(
                usage.view(),
                prices.view(),
