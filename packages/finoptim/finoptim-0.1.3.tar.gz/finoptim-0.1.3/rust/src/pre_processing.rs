    ...
    fn record(&mut self, x: ArrayView1<f64>, c: f64, speed: Option<f64>) {
        ...
        if self.record {
            ...
            self.convergence
                ...
                .as_mut()
                .expect("correct initialisation")
                .push(final_underutilization_cost(
                    self.usage.view(),
                    &self.models,
                    x,
                    self.day,
...
impl<'a> Optimisable for CostPredictionFunction<'a> {
    fn call(&mut self, x: ArrayView1<f64>) -> f64 {
        let pred_costs: Array1<f64> = self
            .prediction
            .axis_iter(Axis(0))
            .map(|pred| cost_final(pred.view(), &self.models, x, self.day, &mut self.usage_copy))
            .collect();
    ...
    fn record(&mut self, x: ArrayView1<f64>, c: f64, speed: Option<f64>) {
        ...
        if self.record {
            let pred_coverages: Array1<f64> = self
                ...
                .axis_iter(Axis(0))
                .map(|pred| {
                    final_coverage(pred.view(), &self.models, x, self.day, &mut self.usage_copy)
                })
                .collect();
            ...
            .axis_iter(Axis(0))
            .map(|pred| {
                final_underutilization_cost(pred.view(), &self.models, x, self.day, &mut self.usage_copy)
            })
            .collect();
