...
use crate::cost_utils::{cost_scalar, coverage, underutilisation};
...
pub fn best_optimiser_with_details(
    usage: Array2<f64>,
    prices: Array2<f64>,
    step_size: f64,
    ...
    let mut returned_levels: ArrayBase<ndarray::OwnedRepr<f64>, Dim<[usize; 1]>> =
        Array::zeros(n + 1);
    ...
    while j < above {
        ...
        underutilisations.push(underutilisation(
            usage.view(),
            prices.view(),
            levels.view(),
            &mut dump,
    ...
    Results {
        ...
        minimum: cost_scalar(
            usage.view(),
            prices.view(),
            argmin.mapv(|x: usize| x as f64).view(),
            &mut dump.clone(),
        ),
