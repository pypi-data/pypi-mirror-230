...
pub fn cost_scalar(
    ...
    *dump = &usage - &reservations;
    ...
    let mut s: ArrayBase<ndarray::OwnedRepr<f64>, Dim<[usize; 1]>> =
        Array::ones(timespan) * s * 24.;
...
pub fn coverage(usage: ArrayView2<f64>, prices: ArrayView2<f64>, levels: ArrayView2<f64>) -> f64 {
    let od_price = prices.slice(s![0, ..]);
    let sp_price = prices.slice(s![1, ..]);
    ...
    let mut dump = &usage - &reservations;
    dump.mapv_inplace(|d| if d < 0. { 0. } else { d });
...
pub fn final_underutilization_cost(
    usage: ArrayView2<f64>,
    models: &Vec<PricingModel>,
    x: ArrayView1<f64>,
    days: bool,
