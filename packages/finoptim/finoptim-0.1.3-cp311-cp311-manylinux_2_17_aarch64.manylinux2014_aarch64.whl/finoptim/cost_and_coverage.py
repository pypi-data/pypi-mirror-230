...
def cost(usage: Union[pd.DataFrame, np.ndarray],
         prices: Union[dict, list, pd.DataFrame],
    ...
    Args:
        ...
        prices (Union[dict, list, pd.DataFrame]): Prices associated with different pricing models.
...
def underutilization(usage: Union[pd.DataFrame, np.ndarray],
             prices: Union[dict, list, pd.DataFrame],
             commitments: Optional[dict] = None,
             savings_plans: Union[None, float, int, np.ndarray] = None,
             reservations: Union[None, np.ndarray, dict] = None,
    ...
    Args:
        usage (Union[pd.DataFrame, np.ndarray]): _description_
        prices (Union[dict, list, pd.DataFrame]): _description_
        commitments (Optional[dict], optional): _description_. Defaults to None.
        savings_plans (Union[None, float, int, np.ndarray], optional): _description_. Defaults to None.
        reservations (Union[None, np.ndarray, dict], optional): _description_. Defaults to None.
        period (Optional[str], optional): _description_. Defaults to None.
...
def __general_entries__(
    ...
    if commitments is not None:
        ...
        assert set(prices.keys()).issubset(
        ...
        match action:
            case "cost":
                return rs.final_cost_or_coverage(
                    ...
                    np.hstack([np.zeros((timespan, 1 + (n - 1) * ('RI' == k[:2]))) + np.array(commitments[k])
                              for k in models if k in commitments.keys()]),  # this syntax is to ensure the same order
                    models,
                    period,
                    True
                )
    ...
    if isinstance(prices, dict):
        prices = pd.DataFrame(prices).values.T
