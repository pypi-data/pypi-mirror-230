...
# compute original cell expressions for all cells, then concatenate them into a matrix
def generate_original_expression(imc_array, mask):
    # Initialize the expression matrix
    exp_matrix = np.empty((0, imc_array.shape[0]))

    # For each cell
    for label in np.unique(mask)[1:]:
        # Create a temporary mask with only one cell
        tmp_mask = np.zeros_like(mask)
        tmp_mask[mask == label] = 1

        # Extract the indices of the cell
