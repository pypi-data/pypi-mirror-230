...
def generate_aug(imc_image, mask, prob, num_to_gen):
    ...
    for lab in select_cells: # iterates over unique non-zero labels found in mask - exlcude 0 (background)
        ...
        try:
            if choose_trans == 0:
                    ...
                    aug_row_array = compute_cell_expression(imc_image, aug_index)
    ...
    exp_matrix = generate_original_expression(imc_image, mask)
    ...
    cell_id_lst = np.unique(mask)[1:].tolist() + cell_id_lst
