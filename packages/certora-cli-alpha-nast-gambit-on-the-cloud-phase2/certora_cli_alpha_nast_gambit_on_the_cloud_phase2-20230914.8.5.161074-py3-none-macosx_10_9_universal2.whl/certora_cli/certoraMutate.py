...
class WebUtils:
    def __init__(self, args: argparse.Namespace):
        ...
        if self.server == "staging":
            self.mutation_test_id_url = "https://vaas-stg.certora.com/mutationTesting/requestId/"
    ...
    def get_response_with_headers_and_timeout(self, url: str,
        ...
        ----
            ...
            headers (dict[str, str]): a dict of headers
        ...
        for i in range(self.max_timeout_attempts_count):
            try:
                resp = requests.get(url, timeout=self.request_timeout, headers=headers)
                return resp
            except Exception:
...
def submit(args: argparse.Namespace) -> None:
    ...
    mutation_test_id = get_mutation_test_id_request(web_utils, len(all_mutants_with_target_dir))
    ...
    with multiprocessing.Pool(processes=num_processes_for_mp, maxtasksperchild=max_task_per_worker) as pool:
        mutant_runs = pool.starmap(run_mutant,
                                   [(mutant, sources_dir, trg_dir, args.prover_conf, args, mutation_test_id) for mutant, trg_dir in
    ...
    if not args.sync:
        ...
        response = send_get_request(web_utils, type=FlowType.ASYNC, mutation_test_id=mutation_test_id)
...
def run_certora_prover(conf_file: Path, args: argparse.Namespace, mutation_test_id: str = "") -> Tuple[bool, Optional[CertoraRunResult]]:
    with conf_file.open() as conf_file_handle:
        ...
        if mutation_test_id:
            certora_args.append("--mutation_test_id")
            certora_args.append(mutation_test_id)
...
def get_mutation_test_id_request(web_utils: WebUtils, mutants_number: int) -> str:
    url = web_utils.mutation_test_id_url + f"?mutants_number={mutants_number}"
    resp = web_utils.get_response_with_headers_and_timeout(url, default_headers)
    if resp is None:
        print("Failed to get a the mutation test ID")
        sys.exit(1)
    ...
    return resp_obj[Constants.ID]


def send_get_request(web_utils: WebUtils, type: str = "", mutation_test_id: str = "") -> ServerResponse:
    url = web_utils.ui_submit_url + default_cookies[Constants.CERTORA_KEY]
    if type == FlowType.ASYNC:
        url += f"&id={mutation_test_id}&type={type}"
...
default_headers = {"Authorization": f"Bearer {os.getenv(KEY_ENV_VAR, '')}"}
