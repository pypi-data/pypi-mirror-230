...
Sources:
- https://github.com/joke2k/faker (MIT)
- https://github.com/kevinwuhoo/randomcolor-py (MIT)
- https://github.com/davidmerfield/randomColor (CC0 1.0)
...
class ColorInfo(TypedDict):
    ...
    lower_bounds: Sequence[Tuple[int, int]]
...
class RandomColor:
    ...
    def generate_rgb_float(
        ...
        """
        hsv = self.generate_hsv(hue=hue, luminosity=luminosity)
        return self.hsv_to_rgb_float(hsv)
    ...
    def pick_hue(self, hue: Optional[HueType]) -> int:
        ...
        hue_ = self.random_within(hue_range)
    ...
    def get_hue_range(self, color_input: Optional[HueType] = None) -> Tuple[int, int]:
        """
        ...
        if isinstance(color_input, (int, float)) and 0 <= color_input <= 360:
            return int(color_input), int(color_input)

        if (
            isinstance(color_input, (list, tuple))
            and all(isinstance(c, (float, int)) for c in color_input)
            and len(color_input) == 2
        ):
            v1, v2 = (int(c) for c in color_input)
        ...
        raise TypeError(
            "Hue must be a string, numeric type, or a tuple/list of 2 numeric types."
        )
    ...
    def get_color_info(self, hue: int) -> ColorInfo:
        """
        ...
        for color_name, color in self.colormap.items():
            hue_range_min, hue_range_max = sorted(color["hue_range"])
            if hue_range_min <= hue <= hue_range_max:
                return color
