    ...
    def _is_finetuning_supported(self):
        ...
        if self.task_name in [
            ...
            try:
                ...
                error_str = f"The model {self.model_name_or_path} cannot be loaded. Only models that can be\
                loaded with transformers.AutoModelForImageClassification are supported for finetuning."
            ...
            if ImageDataItemLiterals.HF_LABELS_KEY not in params:
                error_str = f"Could not find {ImageDataItemLiterals.HF_LABELS_KEY}\
                in signature parameters of model.forward."
