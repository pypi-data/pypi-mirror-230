...
class BotStatsUpdateJob(BaseBackgroundJob, ABC):
    ...
    def exec(self, data: BotStatsUpdateJobData):
        ...
        test_auth_response_data = test_auth_response.json()
        log.info(test_auth_response_data)
        if test_auth_response_data.get('team'):
            bot.source.source_name = get_formatted_bot_name(test_auth_response_data.get('team'))
        bot.slack_url = bot.registration_link = test_auth_response_data.get('url')
