...
class PractitionerMerger:
    ...
    def merge_practitioner(
        ...
        # if no combined_names matched then just use names without any extension
        if len(combined_names) == 0:
            names_without_data_set: List[HumanName] = [
                cast(HumanName, name)
                for name in practitioner.name
                if cast(HumanName, name).extension is None
                or not any(
                    extension
                    for extension in cast(HumanName, name).extension
                    if cast(Extension, extension).url == CLIENT_EXTENSION_URL
            ...
            combined_names.extend(names_without_data_set)
