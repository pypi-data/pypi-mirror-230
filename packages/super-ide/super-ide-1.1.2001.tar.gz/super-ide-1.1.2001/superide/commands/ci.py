...
from superide.exception import CIBuildEnvsEmpty
from superide.project.commands.init import project_init_cmd, validate_boards
...
def _copy_project_conf(build_dir, project_conf):
    ...
    if config.has_section("superide"):
        config.remove_section("superide")
    config.save(os.path.join(build_dir, __configfile__))
