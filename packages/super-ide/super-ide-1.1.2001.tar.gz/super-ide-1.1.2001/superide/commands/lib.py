...
from superide import exception, fs, __configfile__
...
from superide.package.exception import NotGlobalLibDir
from superide.package.manager.library import LibraryPackageManager
from superide.package.meta import PackageItem, PackageSpec
...
def get_project_global_lib_dir():
    return ProjectConfig.get_instance().get("superide", "globallib_dir")
...
@click.option(
    "-g", "--global", is_flag=True, help="Manage global superide library storage"
)
@click.option(
    "-e",
    "--environment",
    ...
    if not storage_dirs:
        if is_platformio_project():
            ...
            click.secho(
                ...
                "Please use `superide lib --global %s` command to remove "
    ...
    for storage_dir in storage_dirs:
        ...
        with fs.cd(storage_dir):
            config = ProjectConfig.get_instance(
                os.path.join(storage_dir, __configfile__)
            )
