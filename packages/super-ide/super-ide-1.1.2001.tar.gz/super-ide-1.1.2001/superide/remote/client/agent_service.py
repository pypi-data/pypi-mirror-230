...
from superide import proc, __configfile__
from superide.device.list.util import list_serial_ports
from superide.project.config import ProjectConfig
from superide.project.exception import NotPlatformIOProjectError
from superide.remote.ac.process import ProcessAsyncCmd
from superide.remote.ac.psync import ProjectSyncAsyncCmd
from superide.remote.ac.serial import SerialPortAsyncCmd
...
class RemoteAgentService(RemoteClientBase):
    def __init__(self, name, share, working_dir=None):
        ...
        self.working_dir = working_dir or os.path.join(
            ProjectConfig.get_instance().get("superide", "core_dir"), "remote"
        )
    ...
    def _process_cmd_run_or_test(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        origin_pio_ini = os.path.join(project_dir, __configfile__)
        ...
        try:
            ...
            # cleanup
            if conf.has_section("superide"):
                for opt in conf.options("superide"):
                    if opt.endswith("_dir"):
                        conf.remove_option("superide", opt)
            else:
                conf.add_section("superide")
            conf.set("superide", "build_dir", ".pio/build")
        ...
        cmd_args = ["superide", "--force", command, "-d", project_dir]
        ...
        return self._defer_async_cmd(
            ProcessAsyncCmd(
                {"executable": proc.where_is_program("superide"), "args": cmd_args},
                on_end_callback=_cb_on_end,
            )
        )

    def _process_cmd_update(self, options):
        cmd_args = ["superide", "--force", "update"]
