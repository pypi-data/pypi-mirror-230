...
@click.option(
    "--command",
    default="/root/.platformio/penv/bin/pio run -d ",
    help=(
        "command"
    ),
)
def cli(project_dir,docker_name,command):
    try:
        result = subprocess.run(["docker", "run","-it","--rm", "-v", project_dir+":"+project_dir, docker_name ]+(command+project_dir).split(), 
                                  text=True)
        if result.stdout:
