...
from superide.compat import get_object_members, load_python_module
...
class DeviceMonitorFilterBase(miniterm.Transform):
    def __init__(self, options=None):
        """Called by superide to pass context"""
...
def register_filters(platform=None, options=None):
    ...
    load_monitor_filters(
        ProjectConfig.get_instance().get("superide", "monitor_dir"),
        prefix="filter_",
        options=options,
    )
    # platform filters
...
def load_monitor_filter(path, options=None):
    ...
    name = name[: name.find(".")]
    module = load_python_module("superide.device.monitor.filters.%s" % name, path)
