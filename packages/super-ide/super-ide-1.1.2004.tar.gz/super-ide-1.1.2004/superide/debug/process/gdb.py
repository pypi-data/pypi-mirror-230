...
class GDBClientProcess(DebugClientProcess):
    ...
    INIT_COMPLETED_BANNER = "superide: Initialization completed"
    ...
    def generate_init_script(self, dst):
        ...
        banner = [
            "echo superide Unified Debugger -> https://bit.ly/pio-debug\\n",
            "echo superide: debug_tool = %s\\n" % self.debug_config.tool_name,
            "echo superide: Initializing remote target...\\n",
        ]
    ...
    def _auto_exec_continue(self):
        ...
        self.console_log(
            "superide: Resume the execution to `debug_init_break = %s`\n"
            % self.debug_config.init_break
        )
        self.console_log(
            "superide: More configuration options -> https://bit.ly/pio-debug\n"
        )
        if self.debug_config.platform.is_embedded():
