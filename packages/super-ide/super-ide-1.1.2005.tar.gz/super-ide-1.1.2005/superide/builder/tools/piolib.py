...
from superide.compat import IS_WINDOWS, hashlib_encode_data, string_types
from superide.http import HTTPClientError, InternetConnectionError
from superide.package.exception import (
    MissingPackageManifestError,
    UnknownPackageError,
)
from superide.package.manager.library import LibraryPackageManager
from superide.package.manifest.parser import (
    ManifestParserError,
    ManifestParserFactory,
)
from superide.package.meta import PackageCompatibility, PackageItem
    ...
    def _mbed_conf_append_macros(self, mbed_config_path, macros):
        ...
        with open(mbed_config_path, encoding="utf8") as fp:
            for line in fp.readlines():
                line = line.strip()
                if line == "#endif":
                    lines.append("// superide Library Dependency Finder (LDF)")
                    lines.extend(
                        [
...
class PlatformIOLibBuilder(LibBuilderBase):
    ...
    def get_include_dirs(self):
        ...
        # backwards compatibility with superide 2.0
        if (
