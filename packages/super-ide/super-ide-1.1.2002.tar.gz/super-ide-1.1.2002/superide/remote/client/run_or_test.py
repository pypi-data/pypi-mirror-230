...
from superide import fs, __configfile__
from superide.compat import hashlib_encode_data
from superide.project.config import ProjectConfig
from superide.remote.client.async_base import AsyncClientBase
    ...
    def add_project_items(self, psync):
        with fs.cd(self.options["project_dir"]):
            cfg = ProjectConfig.get_instance(
                os.path.join(self.options["project_dir"], __configfile__)
            )
            psync.add_item(cfg.path, __configfile__)
            psync.add_item(cfg.get("superide", "shared_dir"), "shared")
            psync.add_item(cfg.get("superide", "boards_dir"), "boards")
            ...
            if self.command == "test":
                psync.add_item(cfg.get("superide", "test_dir"), "test")

    def _add_project_source_items(self, cfg, psync):
        psync.add_item(cfg.get("superide", "lib_dir"), "lib")
        psync.add_item(
            cfg.get("superide", "include_dir"),
            "include",
            cb_filter=self._cb_tarfile_filter,
        )
        psync.add_item(
            cfg.get("superide", "src_dir"), "src", cb_filter=self._cb_tarfile_filter
        )
        if set(["buildfs", "uploadfs", "uploadfsota"]) & set(
            self.options.get("target", [])
        ):
            psync.add_item(cfg.get("superide", "data_dir"), "data")

    @staticmethod
    def _add_project_binary_items(cfg, psync):
        build_dir = cfg.get("superide", "build_dir")
