...
from superide.package.commands.install import install_project_dependencies
...
def update_board_envs(project_dir, boards, extra_project_options, env_prefix):
    config = ProjectConfig(
        os.path.join(project_dir, __configfile__), parse_extra=False
    )
    used_boards = []
...
def update_project_env(environment, extra_project_options=None):
    ...
    option_to_sections = {"superide": [], env_section: []}
    for item in extra_project_options:
        assert "=" in item
