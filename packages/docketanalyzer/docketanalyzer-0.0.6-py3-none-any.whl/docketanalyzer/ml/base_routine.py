...
class BaseRoutine:
    ...
    def process_merge_args(self, merged_model_path=None, merged_name=None):
        ...
        return merged_model_path, merged_name

    def push_merged_model(self, merged_model_path=None, merged_name=None):
        merged_model_path, merged_name = self.process_merge_args(merged_model_path, merged_name)
    ...
    def merge(self, merged_model_path=None, merged_name=None, push=True):
        merged_model_path, merged_name = self.process_merge_args(merged_model_path, merged_name)

        print("Merging and saving model, this can take a while")
        model = AutoPeftModelForCausalLM.from_pretrained(self.adapters_name, device_map='auto')
