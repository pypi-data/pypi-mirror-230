...
class YandexParser(object):
    ...
    def _get_title(self, sn, infected):
        ...
        if not h2:
            ...
            if 'SocialSnippetHeader' in html:
                match = re.search(
                    r'<a class="Link SocialSnippetHeader-Link" href="([^"]+?)"',
                    html,
                    flags=re.S
                )
                if not match:
                    raise YandexParserError('SocialSnippetHeader-Link not found')
                url = match.group(1)

                match = re.search(
                    r'class="SocialSnippetHeader-Name">([^<]+?)</div>',
                    html,
                    flags=re.S
                )
                if not match:
